...
public class Menu_CreateScenario_HRE_Princes extends SliderMenu {
   public Menu_CreateScenario_HRE_Princes() {
      ...
      for (int i = 0; i < CFG.holyRomanEmpire_Manager.getHRE().getPrincesSize(); i++) {
         menuElements.add(
            new Button_New_Game_Players_CivID_LEFT(
               ...
               CFG.PADDING + 2,
               tY,
               tempW - 3 - CFG.PADDING * 2 - (int)(CFG.BUTTON_HEIGHT * 0.6F) * 2,
               true
            ) {
               @Override
         ...
         menuElements.add(
            new Button_RandomGame_Players_Elector(
               CFG.holyRomanEmpire_Manager.getHRE().getPrince(i),
               "",
               -1,
               tempW - 2 - CFG.PADDING - (int)(CFG.BUTTON_HEIGHT * 0.6F) * 2,
               tY,
      ...
      if (menuElements.size() > 0) {
         menuElements.add(
            new Button_Transparent(
               0, 0, tempW, menuElements.get(menuElements.size() - 1).getPosY() + menuElements.get(menuElements.size() - 1).getHeight() + CFG.PADDING, true
            )
         );
      } else {
      ...
      this.initMenu(
         new SliderMenuTitle(null, CFG.BUTTON_HEIGHT * 3 / 4, false, false) {
            ...
            public void draw(SpriteBatch oSB, int iTranslateX, int nPosX, int nPosY, int nWidth, boolean sliderMenuIsActive) {
               ...
               ImageManager.getImage(Images.hre_flag)
                  .draw(
                     oSB,
                     nPosX + (int)(nWidth - this.getTextWidth() * 0.8F) / 2 - (CFG.PADDING + CFG.CIV_FLAG_WIDTH) / 2 + iTranslateX,
                     2 + nPosY - this.getHeight() + (this.getHeight() - CFG.CIV_FLAG_HEIGHT) / 2 - ImageManager.getImage(Images.hre_flag).getHeight(),
                     CFG.CIV_FLAG_WIDTH,
                     CFG.CIV_FLAG_HEIGHT
                  );
               ImageManager.getImage(Images.flag_rect)
                  .draw(
                     oSB,
                     nPosX + (int)(nWidth - this.getTextWidth() * 0.8F) / 2 - (CFG.PADDING + CFG.CIV_FLAG_WIDTH) / 2 + iTranslateX,
                     2 + nPosY - this.getHeight() + (this.getHeight() - CFG.CIV_FLAG_HEIGHT) / 2
                  );
               ...
               CFG.drawText(
                  ...
                  nPosX
                     ...
                     - (CFG.PADDING + CFG.CIV_FLAG_WIDTH) / 2
                     + CFG.PADDING
                     + CFG.CIV_FLAG_WIDTH
                     + iTranslateX,
         ...
         CFG.BUTTON_HEIGHT + CFG.PADDING * 2 + CFG.PADDING * 2 + CFG.BUTTON_HEIGHT * 3 / 4,
         tempW,
         Math.min(
   ...
   public void updateLanguage() {
      this.getTitle()
         .setText(
            ""
               + (CFG.holyRomanEmpire_Manager.getHRE().getPrincesSize() > 0 ? CFG.holyRomanEmpire_Manager.getHRE().getPrincesSize() + " " : "")
               + CFG.langManager.get("Princes")
         );
   }
