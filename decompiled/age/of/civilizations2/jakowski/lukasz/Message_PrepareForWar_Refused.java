...
public class Message_PrepareForWar_Refused extends Message {
   public Message_PrepareForWar_Refused(int fromCivID, int iValue) {
      super(fromCivID, iValue);
      this.messageType = Message_Type.PREPARE_FOR_WAR_REFUSED;
      ...
      if (CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID() == iCivID) {
         ...
         lMess.add(
            CFG.game.getCiv(this.iFromCivID).getCivName()
               + ": "
               + CFG.langManager.get("RefusedToJoinWarPreparationsAgainst")
               + ": "
   ...
   public MenuElement_Hover_v2 getHover() {
      ...
      nData.add(new MenuElement_Hover_v2_Element_Type_Text(CFG.langManager.get("RefusedToJoinWarPreparationsAgainst") + ":", CFG.COLOR_TEXT_MODIFIER_NEGATIVE2));
