...
public class Message_Relations_Increase extends Message {
   public Message_Relations_Increase(int byCivID) {
      super(byCivID, 0);
      this.messageType = Message_Type.RELATIONS_INCREASE;
   ...
   public void onAccept(int iCivID) {
      if (CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID() == iCivID) {
         CFG.toast.setInView(CFG.langManager.get("ImprovingRelationsFrom") + ": " + CFG.game.getCiv(iCivID).getCivName(), CFG.COLOR_TEXT_NUM_OF_PROVINCES);
      }
   }
