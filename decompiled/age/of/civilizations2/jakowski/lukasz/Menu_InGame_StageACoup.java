...
class Menu_InGame_StageACoup
extends SliderMenu {
    private int iOnCivID = -1;

    public Menu_InGame_StageACoup() {
    ...
    public Menu_InGame_StageACoup(int onCivID) {
        ...
        menuElements.add(new Button_FlagActionSliderStyle(CFG.langManager.get("Cancel"), -1, 2 + CFG.PADDING, tY += CFG.PADDING, CFG.BUTTON_WIDTH, true){
            ...
            @Override
            public int getWidth() {
                return Menu_InGame_StageACoup.this.getElementW() - CFG.PADDING - CFG.PADDING / 2;
            }
        });
        menuElements.add(new Button_FlagActionSliderStyle(CFG.langManager.get("Start"), -1, 2, tY, CFG.BUTTON_WIDTH, true){

            @Override
            public int getPosX() {
                return Menu_InGame_StageACoup.this.getElementW() + CFG.PADDING / 2;
            }
            ...
            public void buildElementHover() {
                ...
                nData.add(new MenuElement_Hover_v2_Element_Type_Text(CFG.langManager.get("StartOperation") + ":", CFG.COLOR_BUTTON_GAME_TEXT_ACTIVE));
                nData.add(new MenuElement_Hover_v2_Element_Type_Flag(Menu_InGame_StageACoup.this.iOnCivID, CFG.PADDING, CFG.PADDING));
                nData.add(new MenuElement_Hover_v2_Element_Type_Text(CFG.game.getCiv(Menu_InGame_StageACoup.this.iOnCivID).getCivName()));
                ...
                nData.add(new MenuElement_Hover_v2_Element_Type_Text("-1.6", CFG.COLOR_TEXT_MODIFIER_NEGATIVE2));
            ...
            public boolean getClickable() {
                return CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getDiplomacyPoints() >= 16;
            }
        ...
        int tempMenuPosY = ImageManager.getImage(Images.top_left2).getHeight() + CFG.PADDING * 2 + CFG.BUTTON_HEIGHT * 3 / 5;
        this.initMenu(new SliderMenuTitle(CFG.langManager.get("StageACoup"), CFG.BUTTON_HEIGHT * 3 / 5, true, true){
    ...
    public final void actionElement(int iID) {
        if (iID == this.getMenuElementsSize() - 1) {
            if (CFG.game.getCiv((int)this.iOnCivID).civGameData.leaderData == null) {
                CFG.game.getCiv((int)this.iOnCivID).civGameData.leaderData = new LeaderOfCiv_GameData();
            }
            CFG.game.getCiv((int)this.iOnCivID).civGameData.leaderData.setName("Coup Government");
            CFG.game.getCiv((int)this.iOnCivID).civGameData.leaderData.setImage("coup.png");
            DiplomacyManager.CoupSuccessful(this.iOnCivID, CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(), this.getMenuElement(1).getCurrent());
