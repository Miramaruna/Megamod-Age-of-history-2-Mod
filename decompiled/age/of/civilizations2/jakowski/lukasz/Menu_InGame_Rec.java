...
class Menu_InGame_Rec
extends SliderMenu {
    private int iOnCivID = -1;

    public Menu_InGame_Rec(int onCivID) {
        ...
        menuElements.add(new Button_NS_Opinion(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(), CFG.game.getCiv(CFG.getActiveCivInfo()).getPuppetOfCivID(), Images.diplo_lord, 0, 2, 2, tY, CFG.BUTTON_WIDTH * 2){
            ...
            @Override
            public int getWidth() {
                return Menu_InGame_Rec.this.getElementW() * 2;
            }
        });
        ...
        menuElements.add(new Button_FlagActionSliderStyle(CFG.langManager.get("Cancel"), -1, 2 + CFG.PADDING, tY += CFG.PADDING, CFG.BUTTON_WIDTH, true){
            ...
            @Override
            public int getWidth() {
                return Menu_InGame_Rec.this.getElementW() - CFG.PADDING - CFG.PADDING / 2;
            }
        });
        menuElements.add(new Button_FlagActionSliderStyle(CFG.langManager.get("Confirm"), -1, 2, tY, CFG.BUTTON_WIDTH, true){
            ...
            @Override
            public int getPosX() {
                return Menu_InGame_Rec.this.getElementW() + CFG.PADDING / 2;
            }
            ...
            public void buildElementHover() {
                ...
                nData.add(new MenuElement_Hover_v2_Element_Type_Flag(Menu_InGame_Rec.this.iOnCivID, CFG.PADDING, CFG.PADDING));
                nData.add(new MenuElement_Hover_v2_Element_Type_Text(CFG.game.getCiv(Menu_InGame_Rec.this.iOnCivID).getCivName()));
        ...
        this.initMenu(new SliderMenuTitle(CFG.langManager.get("Mobilization"), CFG.BUTTON_HEIGHT * 3 / 5, true, true){
    ...
    public final void actionElement(int i) {
        if (i == this.getMenuElementsSize() - 1) {
            int civID = CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID();
            CFG.game_NextTurnUpdate.updateCivRecruitableArmy(civID);
            int size = CFG.game.getCiv((int)civID).lProvincesWithMoreRecruitableArmy.size();
            if (size > 0) {
                for (i = 0; i < size; ++i) {
                    int intValue = CFG.game.getCiv((int)civID).lProvincesWithMoreRecruitableArmy.get(i);
                    if (!CFG.game.getCiv(civID).recruitArmy(intValue, CFG.gameAction.getRecruitableArmy(intValue, civID))) break;
                }
            }
        ...
        } else if (i == this.getMenuElementsSize() - 2) {
            this.setVisible(false);
        }
    }
