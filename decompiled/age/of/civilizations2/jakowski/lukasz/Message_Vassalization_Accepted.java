...
public class Message_Vassalization_Accepted extends Message {
   ...
   public void onAccept(int iCivID) {
      if (CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID() == iCivID) {
         ...
         lMess.add(CFG.game.getCiv(this.iFromCivID).getCivName() + ": " + CFG.langManager.get("OurProposalWasAccepted"));
   ...
   public MenuElement_Hover_v2 getHover() {
      ...
      nData.add(new MenuElement_Hover_v2_Element_Type_Text(CFG.langManager.get("OfferVassalization"), CFG.COLOR_TEXT_NUM_OF_PROVINCES));
      ...
      nData.add(new MenuElement_Hover_v2_Element_Type_Text(CFG.langManager.get("OurProposalWasAccepted"), CFG.COLOR_TEXT_MODIFIER_POSITIVE));
      nElements.add(new MenuElement_Hover_v2_Element2(nData));
      nData.clear();
