...
public class Button_BotBar_InvestDevelopment extends Button_BotBar {
   ...
   public void drawText(SpriteBatch oSB, int iTranslateX, int iTranslateY, boolean isActive) {
      ImageManager.getImage(Images.development)
         .draw(
            ...
            this.getPosY() + (this.getHeight() - ImageManager.getImage(Images.development).getHeight()) / 2 + iTranslateY
         );
      CFG.fontMain.getData().setScale(this.FONT_SCALE);
      CFG.drawTextWithShadow(
         oSB,
         this.getTextToDraw(),
         this.getPosX() + this.getTextPos() + CFG.PADDING + ImageManager.getImage(Images.development).getWidth() + iTranslateX,
   ...
   public Color getColor(boolean isActive) {
      return isActive
         ...
         : (
            this.getClickable()
               ? (this.getIsHovered() ? CFG.COLOR_BUTTON_GAME_TEXT_HOVERED : CFG.COLOR_TEXT_MODIFIER_NEUTRAL2)
