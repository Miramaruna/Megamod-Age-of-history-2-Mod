...
public class Message_Hungry extends Message {
   public Message_Hungry(int fromCivID) {
      super(fromCivID, 0);
      this.messageType = Message_Type.HUNGRY;
   ...
   public void onAction(int iMessageID) {
      CFG.toast
         .setInView(CFG.langManager.get("Hungry") + ": " + CFG.game.getCiv(this.iFromCivID).getHunger() * 100.0F + "%", CFG.COLOR_TEXT_MODIFIER_NEGATIVE2);
      CFG.game
   ...
   public MenuElement_Hover_v2 getHover() {
      ...
      nData.add(new MenuElement_Hover_v2_Element_Type_Text(CFG.langManager.get("Hungry") + ": ", CFG.COLOR_TEXT_NUM_OF_PROVINCES));
      nData.add(new MenuElement_Hover_v2_Element_Type_Text("" + CFG.game.getCiv(this.iFromCivID).getHunger() * 100.0F + "%", CFG.COLOR_TEXT_MODIFIER_NEGATIVE2));
