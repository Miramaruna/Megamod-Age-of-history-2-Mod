...
public class Editor_ProvinceTexture extends Editor {
   public int button;
   public int iBrushScale = 1;
   public boolean theDoubleMode = false;
   ...
   public void keyDown(int keycode) {
      ...
      if (Gdx.input.isKeyPressed(49)
         ...
         && (provID = CFG.game.getActiveProvinceID()) >= 0
         && !CFG.game.getProvince(provID).getSeaProvince()) {
         Pixmap wtf = PixmapIO.readCIM(
            Gdx.files
               .local(
                  "map/" + CFG.map.getFile_ActiveMap_Path() + "data/scales/provinces/" + CFG.map.getMapDefaultScale(CFG.map.getActiveMapID()) + "/" + provID
               )
         );
         ...
         for (int yi = 0; yi < pixmap.getHeight(); yi++) {
            for (int xi = 0; xi < pixmap.getWidth(); xi++) {
               if (omg.getPixel(0, 0)
                  == wtf.getPixel(
                     (int)(xi * ((float)CFG.map.getMapDefaultScale(CFG.map.getActiveMapID()) / CFG.map.getMapBG().getMapScale())),
                     (int)(yi * ((float)CFG.map.getMapDefaultScale(CFG.map.getActiveMapID()) / CFG.map.getMapBG().getMapScale()))
                  )) {
         ...
         PixmapIO.writeCIM(
            Gdx.files.local("map/" + CFG.map.getFile_ActiveMap_Path() + "data/scales/provinces/" + CFG.map.getMapBG().getMapScale() + "/" + provID), pixmap
         );
      }
   }
   ...
   public final void dragged(int provID, int screenX, int screenY, boolean type) {
      if (provID >= 0) {
         Pixmap wtf = PixmapIO.readCIM(
            Gdx.files.local("map/" + CFG.map.getFile_ActiveMap_Path() + "data/scales/provinces/" + CFG.map.getMapBG().getMapScale() + "/" + provID)
         );
         Pixmap pixmap = null;
         screenX = (int)(screenX / CFG.map.getMapScale().getCurrentScale());
         screenY = (int)(screenY / CFG.map.getMapScale().getCurrentScale());
