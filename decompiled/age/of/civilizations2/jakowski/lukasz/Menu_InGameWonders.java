...
public class Menu_InGameWonders extends SliderMenu {
   public Menu_InGameWonders(int tInit) {
   ...
   public Menu_InGameWonders() {
      ...
      for (int i = 0; i < CFG.game.wondersManager.wonders_Provinces.size(); i++) {
         for (int j = 0; j < CFG.game.getProvince(CFG.game.wondersManager.wonders_Provinces.get(i)).getWonderSize(); j++) {
            if (CFG.game.getProvince(CFG.game.wondersManager.wonders_Provinces.get(i)).getWonder(j).isAvailable) {
               menuElements.add(
                  new Button_NS_Wonder_2(
                     CFG.game.getPlayer(CFG.PLAYER_TURNID).getMetProvince(CFG.game.wondersManager.wonders_Provinces.get(i))
                        ? new Color(
                           CFG.game.getCiv(CFG.game.getProvince(CFG.game.wondersManager.wonders_Provinces.get(i)).getCivID()).getR() / 255.0F,
                           CFG.game.getCiv(CFG.game.getProvince(CFG.game.wondersManager.wonders_Provinces.get(i)).getCivID()).getG() / 255.0F,
                           CFG.game.getCiv(CFG.game.getProvince(CFG.game.wondersManager.wonders_Provinces.get(i)).getCivID()).getB() / 255.0F,
                           1.0F
                        )
                        : new Color(0.09411765F, 0.3137255F, 0.43137255F, 0.5F),
                     CFG.game.wondersManager.wonders_Provinces.get(i),
                     j,
                     0,
                     tY,
                     tempWidth
      ...
      this.initMenu(
         new SliderMenuTitle(CFG.langManager.get("Wonders"), CFG.BUTTON_HEIGHT * 3 / 5, true, true) {
            @Override
