...
public class Button_View extends Button {
   public Button_View(String sText, int iPosX, int iPosY, int iWidth, int iHeight, boolean isClickable) {
   ...
   public void drawText(SpriteBatch oSB, int iTranslateX, int iTranslateY, boolean isActive) {
      if (isActive) {
         ...
         CFG.drawText(
            oSB,
            this.getText(),
            this.getPosX() + (this.getTextPos() < 0 ? this.getWidth() / 2 - (int)(this.getTextWidth() * 0.6F / 2.0F) : this.getTextPos()) + iTranslateX,
            this.getPosY()
               + ImageManager.getImage(Images.top_left2).getHeight()
               + (this.getHeight() - ImageManager.getImage(Images.top_left2).getHeight() - 2) / 2
               - (int)(CFG.TEXT_HEIGHT * 0.6F / 2.0F)
               + iTranslateY,
