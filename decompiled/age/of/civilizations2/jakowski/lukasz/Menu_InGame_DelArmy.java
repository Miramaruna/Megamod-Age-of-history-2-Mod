...
public class Menu_InGame_DelArmy extends SliderMenu {
   public int iOnCivID = -1;

   public Menu_InGame_DelArmy(int onCivID) {
      ...
      menuElements.add(
         new Button_Build_Building_Destroy(
            ...
            CFG.langManager.get("DestroyEntireArmy") + ": " + CFG.game.getCiv(this.iOnCivID).getCivName(),
            Images.diplo_army,
            0,
            tY,
            CFG.BUTTON_WIDTH * 2,
            ...
            @Override
            public int getWidth() {
               return Menu_InGame_DelArmy.this.getElementW() * 2;
            }
      ...
      menuElements.add(new Button_FlagActionSliderStyle(CFG.langManager.get("Cancel"), -1, 2 + CFG.PADDING, var7 = tY + CFG.PADDING, CFG.BUTTON_WIDTH, true) {
         @Override
         public int getWidth() {
            return Menu_InGame_DelArmy.this.getElementW() - CFG.PADDING - CFG.PADDING / 2;
         }
      });
      menuElements.add(
         new Button_FlagActionSliderStyle(CFG.langManager.get("DeletArmy"), -1, 2, var7, CFG.BUTTON_WIDTH, true) {
            @Override
            public int getPosX() {
               return Menu_InGame_DelArmy.this.getElementW() + CFG.PADDING / 2;
            }
            ...
            public void buildElementHover() {
               ...
               nData.add(new MenuElement_Hover_v2_Element_Type_Text(CFG.langManager.get("DeletArmy") + "!", CFG.COLOR_TEXT_MODIFIER_NEGATIVE2));
      ...
      this.initMenu(
         new SliderMenuTitle(CFG.langManager.get(CFG.langManager.get("DeletArmy")), CFG.BUTTON_HEIGHT * 3 / 5, true, true) {
            @Override
            public void draw(SpriteBatch oSB, int iTranslateX, int nPosX, int nPosY, int nWidth, boolean sliderMenuIsActive) {
               ...
               CFG.game
                  ...
                  .draw(
                     oSB,
                     Menu_InGame_DelArmy.this.getPosX() + CFG.PADDING * 2 + iTranslateX,
                     Menu_InGame_DelArmy.this.getPosY()
                        - this.getHeight() / 2
               ...
               ImageManager.getImage(Images.flag_rect)
                  .draw(
                     oSB,
                     Menu_InGame_DelArmy.this.getPosX() + CFG.PADDING * 2 + iTranslateX,
                     Menu_InGame_DelArmy.this.getPosY() - this.getHeight() / 2 - CFG.CIV_FLAG_HEIGHT / 2
                  );
               CFG.fontMain.getData().setScale(0.8F);
               CFG.drawText(
                  oSB,
   ...
   public final void actionElement(int iID) {
      if (iID != this.getMenuElementsSize() - 1) {
         ...
         if (CFG.game.getCiv(CFG.game.getProvince(CFG.game.getActiveProvinceID()).getCivID()).getNumOfProvinces() > 0) {
            for (int i = 0; i < CFG.game.getCiv(CFG.game.getProvince(CFG.game.getActiveProvinceID()).getCivID()).getNumOfProvinces(); i++) {
               CFG.game.getProvince(CFG.game.getCiv(CFG.game.getProvince(CFG.game.getActiveProvinceID()).getCivID()).getProvinceID(i)).updateArmy(0);
            }
         }
