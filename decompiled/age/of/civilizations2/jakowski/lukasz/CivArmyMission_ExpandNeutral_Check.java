...
public class CivArmyMission_ExpandNeutral_Check extends CivArmyMission {
   public int iTurnsRequiredToMoveToFrontLine = 1;

   public CivArmyMission_ExpandNeutral_Check(int nCivID, int fromProvinceID, int toProvinceID, int iArmy) {
      this.iArmy = iArmy;
   ...
   public boolean action(int nCivID) {
      RegroupArmy_Data_ExpandCheck tryRegroupArmy;
      if (this.iProvinceID == this.toProvinceID
         || (tryRegroupArmy = new RegroupArmy_Data_ExpandCheck(nCivID, this.iProvinceID, this.toProvinceID)).getRouteSize() <= 0) {
         return true;
