...
public class Button_BotBar_NotSupplied extends Button_BotBar {
   public Button_BotBar_NotSupplied(String sText, float FONT_SCALE, int iPosX, int iPosY, int iMinWidth, boolean isClickable, boolean isVisible) {
   ...
   public void drawText(SpriteBatch oSB, int iTranslateX, int iTranslateY, boolean isActive) {
      ImageManager.getImage(Images.diplo_festival)
         .draw(
            ...
            this.getPosY() + (this.getHeight() - ImageManager.getImage(Images.diplo_festival).getHeight()) / 2 + iTranslateY
         );
      CFG.fontMain.getData().setScale(this.FONT_SCALE);
      CFG.drawTextWithShadow(
         oSB,
         this.getTextToDraw(),
         this.getPosX() + this.getTextPos() + CFG.PADDING + ImageManager.getImage(Images.diplo_festival).getWidth() + iTranslateX,
   ...
   public Color getColor(boolean isActive) {
      return isActive
         ...
         : (this.getClickable() ? (this.getIsHovered() ? CFG.COLOR_TEXT_HAPPINESS_HOVER : CFG.COLOR_TEXT_HAPPINESS_MAX) : new Color(0.78F, 0.78F, 0.78F, 0.7F));
   }
}
