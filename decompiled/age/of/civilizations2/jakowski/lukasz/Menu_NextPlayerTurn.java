...
public class Menu_NextPlayerTurn extends SliderMenu {
   ...
   public static String worldsProvinces = "";
   ...
   public Menu_NextPlayerTurn() {
      ...
      menuElements.add(
         new Button_Flag_JustFrame(CFG.CIV_FLAG_WIDTH / 2, CFG.GAME_HEIGHT / 2 - ImageManager.getImage(Images.top_flag_frame).getHeight() / 2, true) {
            @Override
   ...
   public static final void updateData() {
      if (CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID() > 0 && !CFG.SPECTATOR_MODE) {
         if (CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getNumOfProvinces() > 0) {
            worldsProvinces = CFG.langManager
               .get(
                  "XOfAllProvines",
                  ""
                     + CFG.getPercentage(
                        CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getNumOfProvinces(), CFG.game.countLandProvinces_NotWasteland(), 4
                     )
               );
      ...
      if (this.getMenuElement(0).getVisible()) {
         ...
         ImageManager.getImage(Images.slider_gradient)
            .draw(
               ...
               this.getPosY() + this.getHeight() / 2 - CFG.BUTTON_HEIGHT - ImageManager.getImage(Images.slider_gradient).getHeight() + iTranslateY,
               iCivNameWidth + ImageManager.getImage(Images.top_flag_frame).getWidth() + CFG.CIV_FLAG_WIDTH + CFG.PADDING * 2,
               CFG.BUTTON_HEIGHT * 2
            );
         ...
         ImageManager.getImage(Images.slider_gradient)
            .draw(
               oSB,
               this.getPosX() + iTranslateX,
               this.getPosY() + 1 + this.getHeight() / 2 - CFG.BUTTON_HEIGHT - ImageManager.getImage(Images.slider_gradient).getHeight() + iTranslateY,
               iCivNameWidth + ImageManager.getImage(Images.top_flag_frame).getWidth() + CFG.CIV_FLAG_WIDTH + CFG.PADDING * 2,
               1
            );
         ImageManager.getImage(Images.slider_gradient)
            .draw(
               oSB,
               this.getPosX() + iTranslateX,
               this.getPosY() - 2 + this.getHeight() / 2 + CFG.BUTTON_HEIGHT - ImageManager.getImage(Images.slider_gradient).getHeight() + iTranslateY,
               iCivNameWidth + ImageManager.getImage(Images.top_flag_frame).getWidth() + CFG.CIV_FLAG_WIDTH + CFG.PADDING * 2,
               1
            );
         ...
         CFG.game
            ...
            .draw(
               oSB,
               this.getPosX() + CFG.CIV_FLAG_WIDTH / 2 + iTranslateX,
               this.getPosY()
                  + this.getHeight() / 2
                  - CFG.game.getPlayer(CFG.PLAYER_TURNID).getFlag().getHeight()
         ...
         if (CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).civGameData.leaderData != null
            && CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).civGameData.leaderData.getName().length() > 0) {
            CFG.drawTextWithShadow(
               oSB,
               CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).civGameData.leaderData.getName(),
               CFG.CIV_FLAG_WIDTH / 2 + ImageManager.getImage(Images.top_flag_frame).getWidth() + CFG.PADDING + iTranslateX,
               CFG.GAME_HEIGHT / 2 + CFG.PADDING / 2,
               CFG.COLOR_TEXT_CIV_NAME
            );
         } else {
            CFG.drawTextWithShadow(
               oSB,
               this.sPlayer + " " + (CFG.PLAYER_TURNID + 1),
               CFG.CIV_FLAG_WIDTH / 2 + ImageManager.getImage(Images.top_flag_frame).getWidth() + CFG.PADDING + iTranslateX,
               CFG.GAME_HEIGHT / 2 + CFG.PADDING / 2,
               CFG.COLOR_TEXT_CIV_NAME
            );
         ...
         CFG.drawTextWithShadow(
            ...
            CFG.CIV_FLAG_WIDTH / 2 + ImageManager.getImage(Images.top_flag_frame).getWidth() + CFG.PADDING + iTranslateX,
            CFG.GAME_HEIGHT / 2 - CFG.TEXT_HEIGHT - CFG.PADDING / 2,
            CFG.COLOR_TEXT_RANK
         );
         if (lTime > System.currentTimeMillis() - 3000L) {
            ...
            CFG.drawText(
               ...
               CFG.GAME_HEIGHT - (int)(CFG.TEXT_HEIGHT * 0.8F) - CFG.PADDING * 2,
               new Color(CFG.COLOR_TEXT_NUM_OF_PROVINCES.r, CFG.COLOR_TEXT_NUM_OF_PROVINCES.g, CFG.COLOR_TEXT_NUM_OF_PROVINCES.b, tAlpha)
            );
      ...
      try {
         if (RTS.isEnabled() && !RTS.PAUSE) {
            ...
            if (CFG.game.getPlayer(CFG.PLAYER_TURNID).iBefore_PosY == -999999) {
               ...
               if (CFG.game.getProvince(CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getCapitalProvinceID()).getCivID()
         ...
         if (CFG.game.getPlayer(CFG.PLAYER_TURNID).visible_UpgradingArmy) {
   ...
   public static final void clickEnd() {
      ...
      if (CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getAllianceID() > 0) {
         for (int j = 0;
            ...
            for (int i2 = 0;
               i2
                  < CFG.game
                     .getCiv(CFG.game.getAlliance(CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getAllianceID()).getCivilization(j))
                     .getMoveUnitsSize();
               i2++
            ) {
               CFG.game
      ...
      for (int j = CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID() - 1; j > 0; j--) {
         if (CFG.game.getCiv(j).getPuppetOfCivID() == CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()) {
            for (int i2 = 0; i2 < CFG.game.getCiv(j).getMoveUnitsSize(); i2++) {
               CFG.game.getCiv(j).getMoveUnits(i2).getMoveUnitsLine().updateMoveTime();
            }
         }
      }

      for (int var7 = CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID() + 1; var7 < CFG.game.getCivsSize(); var7++) {
         if (CFG.game.getCiv(var7).getPuppetOfCivID() == CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()) {
            for (int i2 = 0; i2 < CFG.game.getCiv(var7).getMoveUnitsSize(); i2++) {
               if (CFG.game.getCiv(var7).getMoveUnits(i2).getMoveUnitsLine() != null) {
                  CFG.game.getCiv(var7).getMoveUnits(i2).getMoveUnitsLine().updateMoveTime();
               }
            }
         }
      }
