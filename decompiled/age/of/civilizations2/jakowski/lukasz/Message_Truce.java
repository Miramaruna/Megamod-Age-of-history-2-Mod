...
public class Message_Truce extends Message {
   public Message_Truce(int fromCivID) {
      super(fromCivID, 0);
      this.messageType = Message_Type.TRUCE;
   ...
   public void onAccept(int iCivID) {
      if (CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID() == iCivID) {
         ...
         lMess.add(CFG.langManager.get("TruceWith") + ": " + CFG.game.getCiv(this.iFromCivID).getCivName());
