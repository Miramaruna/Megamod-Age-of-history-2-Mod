...
public class Event_Conditions_IsCapital extends Event_Conditions {
   public int iCivID = -1;
   ...
   public boolean outCondition() {
      ...
      try {
         for (int i = 0; i < this.lProvinces.size(); i++) {
            if (CFG.game.getProvince(this.lProvinces.get(i)).getIsCapital()
               && (this.getCivID() <= 0 || CFG.game.getProvince(this.lProvinces.get(i)).getCivID() == this.getCivID())) {
      ...
      return !((float)numOfControlledProvinces / this.lProvinces.size() > this.getValue() / 100.0F);
   }
