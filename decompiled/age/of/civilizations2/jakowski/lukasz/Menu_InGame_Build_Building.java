...
class Menu_InGame_Build_Building extends SliderMenu {
   private int iProvinceID = -1;
   private int iN = 0;

   public Menu_InGame_Build_Building() {
   ...
   public Menu_InGame_Build_Building(int nProvinceID, final int n) {
      this.iN = n;
      ArrayList<MenuElement> menuElements = new ArrayList<>();
      ...
      menuElements.add(
         new Button_Build_Building(
            CFG.langManager.get(CFG.game.getProvince(this.iProvinceID).getLevelOfBuilding(n) == 0 ? "BuildCastleIn" : "BuildFortressIn") + ": ",
            ...
            CustomBuildingsManager.getBuildings_Image(CFG.game.getProvince(BuildingsManager.iBuildInProvinceID).getLevelOfBuilding(n), n),
            CustomBuildingsManager.getBuilding_BuildCost(CFG.game.getProvince(this.iProvinceID).getLevelOfBuilding(n) + 1, this.iProvinceID, n),
            CustomBuildingsManager.getBuilding_BuildMovementCost(CFG.game.getProvince(this.iProvinceID).getLevelOfBuilding(n) + 1, n),
            0,
            tY,
            CFG.BUTTON_WIDTH * 2
         ) {
            @Override
            public int getWidth() {
               return Menu_InGame_Build_Building.this.getElementW() * 2;
            }
                  ...
                  new MenuElement_Hover_v2_Element_Type_Text(
                     "+"
                        + CustomBuildingsManager.getBuilding_DefenseBonus(
                           CFG.game.getProvince(Menu_InGame_Build_Building.this.iProvinceID).getLevelOfBuilding(n) + 1, n
                        )
                        + "%",
      ...
      menuElements.get(menuElements.size() - 1)
         .setMin((int)(CustomBuildingsManager.getBuilding_TechLevel(CFG.game.getProvince(this.iProvinceID).getLevelOfBuilding(n) + 1, n) * 100.0F));
      tY += menuElements.get(menuElements.size() - 1).getHeight();
      int var8;
      menuElements.add(new Button_FlagActionSliderStyle(CFG.langManager.get("Cancel"), -1, 2 + CFG.PADDING, var8 = tY + CFG.PADDING, CFG.BUTTON_WIDTH, true) {
         @Override
         public int getWidth() {
            return Menu_InGame_Build_Building.this.getElementW() - CFG.PADDING - CFG.PADDING / 2;
         }
      });
      menuElements.add(
         new Button_FlagActionSliderStyle(CFG.langManager.get("Construct"), -1, 2, var8, CFG.BUTTON_WIDTH, true) {
            @Override
            public int getPosX() {
               return Menu_InGame_Build_Building.this.getElementW() + CFG.PADDING / 2;
            }
            ...
            public void buildElementHover() {
               ...
               nData.add(
                  new MenuElement_Hover_v2_Element_Type_Text(
                     CFG.langManager
                           .get(
                              CFG.game.getProvince(Menu_InGame_Build_Building.this.iProvinceID).getLevelOfBuilding(n) == 0
                                 ? "BuildCastleIn"
                                 : "BuildFortressIn"
                           )
                        + ": ",
               ...
               nData.add(
                  new MenuElement_Hover_v2_Element_Type_Text(
                     CFG.game.getProvince(Menu_InGame_Build_Building.this.iProvinceID).getName().length() > 0
                        ? CFG.game.getProvince(Menu_InGame_Build_Building.this.iProvinceID).getName()
                        : CFG.langManager.get("Province")
                  )
               );
               nData.add(
                  new MenuElement_Hover_v2_Element_Type_Flag(CFG.game.getProvince(Menu_InGame_Build_Building.this.iProvinceID).getCivID(), CFG.PADDING, 0)
               );
               nElements.add(new MenuElement_Hover_v2_Element2(nData));
               nData.clear();
                  ...
                  new MenuElement_Hover_v2_Element_Type_Text(
                     ""
                        + CustomBuildingsManager.getBuilding_BuildCost(
                           CFG.game.getProvince(Menu_InGame_Build_Building.this.iProvinceID).getLevelOfBuilding(n) + 1,
                           Menu_InGame_Build_Building.this.iProvinceID,
                           n
                        ),
      ...
      this.initMenu(
         new SliderMenuTitle(
            CustomBuildingsManager.getBuilding_Name(CFG.game.getProvince(this.iProvinceID).getLevelOfBuilding(n) + 1, n), CFG.BUTTON_HEIGHT * 3 / 5, true, true
         ) {
            @Override
            public void draw(SpriteBatch oSB, int iTranslateX, int nPosX, int nPosY, int nWidth, boolean sliderMenuIsActive) {
               ...
               CFG.game
                  ...
                  .draw(
                     oSB,
                     Menu_InGame_Build_Building.this.getPosX() + CFG.PADDING * 2 + iTranslateX,
                     Menu_InGame_Build_Building.this.getPosY()
                        - this.getHeight() / 2
               ...
               ImageManager.getImage(Images.flag_rect)
                  .draw(
                     oSB,
                     Menu_InGame_Build_Building.this.getPosX() + CFG.PADDING * 2 + iTranslateX,
                     Menu_InGame_Build_Building.this.getPosY() - this.getHeight() / 2 - CFG.CIV_FLAG_HEIGHT / 2
                  );
               CFG.fontMain.getData().setScale(0.8F);
               CFG.drawText(
                  oSB,
   ...
   public final void actionElement(int iID) {
      if (iID == this.getMenuElementsSize() - 1) {
         if (CustomBuildingsManager.constructBuilding(this.iProvinceID, CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(), this.iN)) {
