...
public class Civilization_Diplomacy_GameData implements Serializable {
   ...
   public List<Civilization_Diplomacy_ImproveRelations_GameData> lImproveRelations = new ArrayList<>();
   public List<Civilization_ClosedEmbassy> lEmabassyClosed = new ArrayList<>();
   ...
   public final void addEmbeassyClosed(Civilization_ClosedEmbassy nData) {
   ...
   public final boolean isEmassyClosed(int nCivID) {
      for (int i = 0; i < this.iEmbassyClosedSize; i++) {
         if (this.lEmabassyClosed.get(i).iCivID == nCivID) {
            return true;
         }
