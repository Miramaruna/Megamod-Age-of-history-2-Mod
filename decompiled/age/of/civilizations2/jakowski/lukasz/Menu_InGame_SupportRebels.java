...
public class Menu_InGame_SupportRebels extends SliderMenu {
   ...
   public Menu_InGame_SupportRebels() {
      ...
      this.initMenu(
         new SliderMenuTitle(CFG.langManager.get("SupportRebels"), CFG.BUTTON_HEIGHT * 3 / 5, true, true),
   ...
   public Menu_InGame_SupportRebels(int onCivID, int iRebelsID) {
      ...
      int tY = 0;
      menuElements.add(
         new Button_NS_Opinion(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(), onCivID, Images.diplo_revolution, 0, 34, 2, tY, CFG.BUTTON_WIDTH * 2) {
            @Override
            public int getWidth() {
      ...
      if (supportData.lMovements.size() > 0) {
         ...
         menuElements.add(
            new Text_AlliesNotInWar(
               CFG.langManager.get("SelectMovement"), -1, CFG.PADDING, var9 = tY + CFG.PADDING, tempWidth - CFG.PADDING * 2, CFG.TEXT_HEIGHT + CFG.PADDING * 3
            ) {
               @Override
               public int getPosX() {
                  return 0;
         ...
         for (int i = 0; i < supportData.lMovements.size(); i++) {
            menuElements.add(
               new Button_Diplomacy_SupportRebels(
                  i,
                  supportData.lMovements.get(i),
                  supportData.lPopulation.get(i),
                  (int)((float)supportData.lUnrest.get(i).intValue() / supportData.lProvinces.get(i).intValue()),
                  supportData.lProvinces.get(i),
                  2,
                  tY,
                  CFG.BUTTON_WIDTH * 2
               ) {
            ...
            tY += menuElements.get(menuElements.size() - 1).getHeight();
            if (menuElements.get(menuElements.size() - 1).getCurrent() == iRebelsID) {
      ...
      menuElements.add(
         new Button_FlagActionSliderStyle(CFG.langManager.get("SupportRebels"), -1, 2, var12, CFG.BUTTON_WIDTH, supportData.lMovements.size() > 0) {
            @Override
            public int getPosX() {
            ...
            public void drawText(SpriteBatch oSB, int iTranslateX, int iTranslateY, boolean isActive) {
               ImageManager.getImage(Images.diplo_revolution)
                  .draw(
                     ...
                     this.getPosX()
                        + this.getWidth() / 2
                        - (int)((this.getTextWidth() * 0.8F + ImageManager.getImage(Images.diplo_revolution).getWidth() + CFG.PADDING) / 2.0F)
                        + iTranslateX,
                     this.getPosY() + this.getHeight() / 2 - ImageManager.getImage(Images.diplo_revolution).getHeight() / 2 + iTranslateY
                  );
                  ...
                  this.getPosX()
                     + (
                        this.getTextPos() < 0
                           ? this.getWidth() / 2
                              - (int)((this.getTextWidth() * 0.8F + ImageManager.getImage(Images.diplo_revolution).getWidth() + CFG.PADDING) / 2.0F)
                              + ImageManager.getImage(Images.diplo_revolution).getWidth()
                              + CFG.PADDING
                           : this.getTextPos()
                     )
                     + iTranslateX,
            ...
            public boolean getClickable() {
               return !CFG.SPECTATOR_MODE
                  && CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getDiplomacyPoints() >= 34
                  && super.getClickable();
            }
      ...
      int tempMenuPosY = ImageManager.getImage(Images.top_left2).getHeight() + CFG.PADDING * 2 + CFG.BUTTON_HEIGHT * 3 / 5;
      this.initMenu(
         new SliderMenuTitle(CFG.langManager.get("RevolutionaryMovements"), CFG.BUTTON_HEIGHT * 3 / 5, true, true) {
            @Override
   ...
   public final void actionElement(int iID) {
      if (iID == this.getMenuElementsSize() - 1) {
         ...
         } else if (this.getMenuElement(this.getMenuElementsSize() - 3).getCurrent() <= 0) {
            CFG.toast.setInView(CFG.langManager.get("SelectTheAmountOfMoneyToSend") + "!", CFG.COLOR_TEXT_MODIFIER_NEGATIVE2);
            CFG.toast.setTimeInView(4500);
         } else {
            if (DiplomacyManager.supportRebels(
               CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(), iOnCivID, iSelectedCivID, this.getMenuElement(this.getMenuElementsSize() - 3).getCurrent()
            )) {
         ...
         this.setVisible(false);
      } else if (iID != this.getMenuElementsSize() - 3 && iID > 1) {
         ...
         this.getMenuElement(this.getMenuElementsSize() - 3)
            .setMax(
               Math.max(
                  0,
                  Math.min(
                     (int)CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getMoney(), DiplomacyManager.supportRebels_MaxGold(rebelsProvinces)
                  )
               )
            );
         ...
         if (centerToProv && rebelsProvinces.size() > 0) {
            ...
            for (int ix = 1; ix < rebelsProvinces.size(); ix++) {
               if (CFG.game.getPlayer(CFG.PLAYER_TURNID).getMetProvince(rebelsProvinces.get(ix))
                  && CFG.game.getProvince(rebelsProvinces.get(tBest)).getPopulationData().getPopulationOfCivID(this.getMenuElement(iID).getCurrent())
                     < CFG.game.getProvince(rebelsProvinces.get(ix)).getPopulationData().getPopulationOfCivID(this.getMenuElement(iID).getCurrent())) {
                  tBest = ix;
               }
            }
