...
public class MessageBox_GameData implements Serializable {
   ...
   public List<Message> lMessages = new ArrayList<>();
   ...
   public final void addMessage(Message nMessage) {
      if (nMessage.messageType == Message_Type.DISEASE) {
         ...
         for (int ix = 0; ix < this.getMessagesSize(); ix++) {
            if (this.lMessages.get(ix).iFromCivID == nMessage.iFromCivID && this.lMessages.get(ix).messageType == nMessage.messageType) {
               if (nMessage.messageType == Message_Type.ULTIMATUM) {
                  this.removeMessage(ix);
                  break;
               }

               if (nMessage.messageType == Message_Type.TECHNOLOGY_RESEARCHED) {
                  this.lMessages.get(ix).iNumOfTurnsLeft = nMessage.iNumOfTurnsLeft;
                  return;
               }
               ...
               if (nMessage.messageType == Message_Type.TRANSFER_CONTROL && nMessage.iValue == this.lMessages.get(ix).iValue) {
                  return;
               }
            }
         }
   ...
   public final void removeMessage_TypeFrom(int nFromCivID, Message_Type nType) {
      try {
