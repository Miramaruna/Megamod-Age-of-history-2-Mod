...
public class Menu_InGame_PrepareForWar extends SliderMenu {
   ...
   public Menu_InGame_PrepareForWar(int onCivID) {
      ...
      int tY = 0;
      menuElements.add(
         new Button_Diplomacy_CallToArms(
            Images.diplo_rivals, CFG.langManager.get("RequestWarPreparationsAgainst") + ":", onCivID, 2, tY, CFG.BUTTON_WIDTH * 2, 0
         ) {
            @Override
            public int getWidth() {
      ...
      if (CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getAllianceID() > 0) {
         for (int i = 0;
            ...
            if (CFG.game
                     ...
                     .getNumOfProvinces()
                  > 0
               && onCivID != CFG.game.getAlliance(CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getAllianceID()).getCivilization(i)
      ...
      for (int ix = 1; ix < CFG.game.getCivsSize(); ix++) {
         if (ix != CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()
            ...
            && ix != onCivID
            && !CFG.game.getCivsAtWar(onCivID, ix)) {
      ...
      for (int var11 = alliesToCall.size() - 1; var11 >= 0; var11--) {
         if (CFG.game
            .getCiv(alliesToCall.get(var11))
            .civGameData
            .civPlans
            .isPreparingForTheWar(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(), this.iOnCivID)) {
            alliesToCall.remove(var11);
         }
      }
      ...
      if (alliesToCall.size() == 0) {
         ...
         for (int var12 = 0; var12 < alliesToCall.size(); var12++) {
            menuElements.add(new Buton_Diplomacy_CallAllies(var12, alliesToCall.get(var12), onCivID, 2, tY, CFG.BUTTON_WIDTH * 2) {
               @Override
               public int getWidth() {
                  return Menu_InGame_PrepareForWar.this.getElementW() * 2;
               }
      ...
      menuElements.add(
         new Button_FlagActionSliderStyle(CFG.langManager.get("Confirm"), -1, 2, var15, CFG.BUTTON_WIDTH, canSend) {
            @Override
            public int getPosX() {
            ...
            public void drawText(SpriteBatch oSB, int iTranslateX, int iTranslateY, boolean isActive) {
               ImageManager.getImage(Images.diplo_war_preparations)
                  .draw(
                     ...
                     this.getPosX()
                        + this.getWidth() / 2
                        - (int)((this.getTextWidth() * 0.8F + ImageManager.getImage(Images.diplo_war_preparations).getWidth() + CFG.PADDING) / 2.0F)
                        + iTranslateX,
                  ...
                  this.getPosX()
                     + (
                        this.getTextPos() < 0
                           ? this.getWidth() / 2
                              - (int)((this.getTextWidth() * 0.8F + ImageManager.getImage(Images.diplo_war_preparations).getWidth() + CFG.PADDING) / 2.0F)
                              + ImageManager.getImage(Images.diplo_war_preparations).getWidth()
                              + CFG.PADDING
                           : this.getTextPos()
                     )
                     + iTranslateX,
   ...
   public final void actionElement(int iID) {
      if (iID != this.getMenuElementsSize() - 1) {
         ...
         for (int i = 1; i < this.getMenuElementsSize() - 3; i++) {
         ...
         if (selectedAnyCiv) {
            for (int var4 = 1; var4 < this.getMenuElementsSize() - 3; var4++) {
               if (this.getMenuElement(var4).getCurrent() > 0 && this.getMenuElement(var4).getCheckboxState()) {
                  DiplomacyManager.sendPrepareForWar(
                     this.getMenuElement(var4).getCurrent(),
                     CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(),
                     this.iOnCivID,
