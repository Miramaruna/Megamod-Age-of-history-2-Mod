...
public class Button_Casualties extends Button {
   ...
   public void drawButtonBG(SpriteBatch oSB, int iTranslateX, int iTranslateY, boolean isActive) {
      ...
      ImageManager.getImage(Images.difficulty_hell)
         .draw(
            oSB,
            this.getPosX() + CFG.PADDING * 3 + (int)(this.getTextWidth() * 0.8F) + (int)(this.iPopulationWidth * 0.8F) + iTranslateX,
            this.getPosY()
               + 1
               + this.getHeight() / 2
               - (int)(
                     ImageManager.getImage(Images.difficulty_hell).getHeight() * this.getImageScale(ImageManager.getImage(Images.difficulty_hell).getHeight())
                  )
                  / 2
               + iTranslateY
               - ImageManager.getImage(Images.difficulty_hell).getHeight(),
            (int)(ImageManager.getImage(Images.difficulty_hell).getWidth() * this.getImageScale(ImageManager.getImage(Images.difficulty_hell).getHeight())),
            (int)(ImageManager.getImage(Images.difficulty_hell).getHeight() * this.getImageScale(ImageManager.getImage(Images.difficulty_hell).getHeight()))
         );
   }
   ...
   public void drawText(SpriteBatch oSB, int iTranslateX, int iTranslateY, boolean isActive) {
      ...
      CFG.drawTextWithShadow(
         oSB,
         this.sPopulation,
         this.getPosX() + CFG.PADDING * 2 + (int)(this.getTextWidth() * 0.8F) + iTranslateX,
         this.getPosY() + this.getHeight() / 2 - (int)(this.getTextHeight() * 0.8F / 2.0F) + iTranslateY,
   ...
   public void buildElementHover() {
      try {
         ...
         if (tempCas > 0) {
            ...
            for (int ix = 0; ix < CFG.game.getWar(CFG.peaceTreatyData.peaceTreatyGameData.iWarID).getDefendersSize(); ix++) {
               tempCas = CFG.game.getWar(CFG.peaceTreatyData.peaceTreatyGameData.iWarID).getDefenderID(ix).getCasualties()
                  + CFG.game.getWar(CFG.peaceTreatyData.peaceTreatyGameData.iWarID).getDefenderID(ix).getCivilianDeaths();
               if (tempCas > 0) {
                  if (added && !added2) {
                     nData.add(new MenuElement_Hover_v2_Element_Type_Space());
                     nElements.add(new MenuElement_Hover_v2_Element2(nData));
                  ...
                  nData.add(
                     new MenuElement_Hover_v2_Element_Type_Flag(CFG.game.getWar(CFG.peaceTreatyData.peaceTreatyGameData.iWarID).getDefenderID(ix).getCivID())
                  );
                  nData.add(new MenuElement_Hover_v2_Element_Type_Text(CFG.langManager.get("Casualties") + ": "));
                  nData.add(new MenuElement_Hover_v2_Element_Type_Text("" + CFG.getNumberWithSpaces("" + tempCas), CFG.COLOR_TEXT_MODIFIER_NEGATIVE2));
                  nData.add(new MenuElement_Hover_v2_Element_Type_Image(Images.difficulty_hell, CFG.PADDING, 0));
   ...
   public final float getImageScale(int nImageID) {
      return (CFG.TEXT_HEIGHT + CFG.PADDING * 2) * 0.8F / ImageManager.getImage(nImageID).getHeight() < 1.0F
         ? (CFG.TEXT_HEIGHT + CFG.PADDING * 2) * 0.8F / ImageManager.getImage(nImageID).getHeight()
         : 1.0F;
   }
         ...
         CFG.PADDING * 4
            + (int)(
               this.getTextWidth() * 0.8F
                  + this.iPopulationWidth * 0.8F
                  + (int)(
                     ImageManager.getImage(Images.difficulty_hell).getWidth() * this.getImageScale(ImageManager.getImage(Images.difficulty_hell).getHeight())
                  )
            )
            + CFG.PADDING * 2
      );
