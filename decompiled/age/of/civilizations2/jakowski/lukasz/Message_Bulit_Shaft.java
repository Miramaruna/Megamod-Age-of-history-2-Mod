...
public class Message_Bulit_Shaft extends Message {
   protected Message_Bulit_Shaft(int var1, int var2) {
      super(var1, var2);
      this.messageType = Message_Type.BUILT_SHAFT;
   ...
   protected MenuElement_Hover_v2 getHover() {
      ...
      var2.add(
         new MenuElement_Hover_v2_Element_Type_Text(
            CFG.langManager.get(Shaft.getShaft_Name(CFG.game.getProvince(this.iValue).getLevelOfSupply())), CFG.COLOR_TEXT_NUM_OF_PROVINCES
         )
      );
      var2.add(new MenuElement_Hover_v2_Element_Type_Image(Images.b_supply, CFG.PADDING, 0));
      var1.add(new MenuElement_Hover_v2_Element2(var2));
      var2.clear();
      ...
      var2.add(new MenuElement_Hover_v2_Element_Type_Text(CFG.langManager.get("MilitaryUpkeep") + ": "));
      var2.add(
         new MenuElement_Hover_v2_Element_Type_Text(
            "-" + (int)(Shaft.getShaft_Bonus(CFG.game.getProvince(this.iValue).getLevelOfSupply()) * 100.0F) + "%", CFG.COLOR_TEXT_MODIFIER_POSITIVE
         )
      );
   ...
   protected void onAccept(int var1) {
      if (CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID() == var1) {
         ...
         var2.add(var4.append(var5).append(CFG.langManager.get(Shaft.getShaft_Name(CFG.game.getProvince(this.iValue).getLevelOfSupply()))).toString());
