...
public class Message_Union_Refused extends Message {
   public Message_Union_Refused(int fromCivID, int iValue) {
      super(fromCivID, iValue);
      this.messageType = Message_Type.UNION_REFUSED;
   ...
   public void onAccept(int iCivID) {
      if (CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID() == iCivID) {
         ...
         lMess.add(CFG.langManager.get("XRefusedToFormUnion", CFG.game.getCiv(this.iFromCivID).getCivName()));
   ...
   public MenuElement_Hover_v2 getHover() {
      ...
      nData.add(
         new MenuElement_Hover_v2_Element_Type_Text(
            CFG.langManager.get("XRefusedToFormUnion", CFG.game.getCiv(this.iFromCivID).getCivName()), CFG.COLOR_TEXT_MODIFIER_NEGATIVE2
         )
      );
