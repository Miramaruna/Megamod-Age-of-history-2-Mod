...
public class CivArmyMission_RegroupAfterRecruitment_War extends CivArmyMission {
   public int iTurnsRequiredToMoveToFrontLine = 1;

   public CivArmyMission_RegroupAfterRecruitment_War(int nCivID, int fromProvinceID, int toProvinceID, int iArmy) {
      this.iArmy = iArmy;
   ...
   public boolean action(int nCivID) {
      RegroupArmy_Data_ToTheFront tryRegroupArmy;
      if (this.iProvinceID == this.toProvinceID
         || (tryRegroupArmy = new RegroupArmy_Data_ToTheFront(nCivID, this.iProvinceID, this.toProvinceID)).getRouteSize() <= 0) {
         return true;
