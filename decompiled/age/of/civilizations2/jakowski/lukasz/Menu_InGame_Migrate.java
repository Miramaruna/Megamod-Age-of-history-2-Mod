...
public class Menu_InGame_Migrate extends SliderMenu {
   ...
   public Menu_InGame_Migrate() {
      ...
      this.initMenu(
         new SliderMenuTitle(CFG.langManager.get("Migrate"), CFG.BUTTON_HEIGHT * 3 / 5, true, true),
   ...
   public Menu_InGame_Migrate(int nFromProvinceID, int nToProvinceID) {
      ...
      int tY = 0;
      menuElements.add(
         new Button_Diplomacy_Migrate2(this.fromProvinceID, nToProvinceID, CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(), 2, tY, CFG.BUTTON_WIDTH * 2, true) {
            @Override
            public int getWidth() {
      ...
      menuElements.add(new Button_FlagActionSliderStyle(CFG.langManager.get("Cancel"), -1, 2 + CFG.PADDING, var8 = tY + CFG.PADDING, CFG.BUTTON_WIDTH, true) {
         @Override
         public int getWidth() {
            return Menu_InGame_Migrate.this.getElementW() - CFG.PADDING - CFG.PADDING / 2;
         }
      });
      menuElements.add(
         new Button_FlagActionSliderStyle(CFG.langManager.get("Confirm"), -1, 2, var8, CFG.BUTTON_WIDTH, true) {
            @Override
            public int getPosX() {
               return Menu_InGame_Migrate.this.getElementW() + CFG.PADDING / 2;
            }
            ...
            public void buildElementHover() {
               ...
               nData.add(new MenuElement_Hover_v2_Element_Type_Text(CFG.langManager.get("MigrateTo") + ": "));
               nData.add(
                  new MenuElement_Hover_v2_Element_Type_Text(
                     CFG.game.getProvince(Menu_InGame_Migrate.this.toProvinceID).getName().length() > 0
                        ? CFG.game.getProvince(Menu_InGame_Migrate.this.toProvinceID).getName()
                        : CFG.langManager.get("NewLand"),
               ...
               nData.add(
                  new MenuElement_Hover_v2_Element_Type_Text(
                     "+" + (int)((CFG.game.getProvince(Menu_InGame_Migrate.this.fromProvinceID).getIsCapital() ? 0.1F : 0.05F) * 100.0F) + "%",
            ...
            @Override
            public int getSFX() {
               return SoundsManager.SOUND_MOVE_ARMY2;
            }
         }
      );
      int tempMenuPosY = ImageManager.getImage(Images.top_left2).getHeight() + CFG.PADDING * 2 + CFG.BUTTON_HEIGHT * 3 / 5;
         ...
         new SliderMenuTitle(CFG.langManager.get("Migrate"), CFG.BUTTON_HEIGHT * 3 / 5, true, true) {
            ...
            public void draw(SpriteBatch oSB, int iTranslateX, int nPosX, int nPosY, int nWidth, boolean sliderMenuIsActive) {
               ...
               CFG.ideologiesManager
                  ...
                  .draw(
                     oSB,
                     Menu_InGame_Migrate.this.getPosX() + CFG.PADDING * 2 + iTranslateX,
                     Menu_InGame_Migrate.this.getPosY()
                        - this.getHeight() / 2
                        - CFG.ideologiesManager
                              .getIdeology(CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getIdeologyID())
                              .getCrownImageScaled()
   ...
   public final void actionElement(int iID) {
      if (iID == this.getMenuElementsSize() - 1) {
         CFG.gameAction.migrateToProvince(this.fromProvinceID, this.toProvinceID, CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(), true);
         ...
         for (int i = 0; i < CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getMigrateSize(); i++) {
            if (CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getMigrate(i).getFromProvinceID()
                  == CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getCapitalProvinceID()
               && CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getMigrate(i).getToProvinceID() == this.toProvinceID) {
               CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).removeMigrate(i);
               CFG.game
         ...
         CFG.game.setActiveProvinceID(this.fromProvinceID);
