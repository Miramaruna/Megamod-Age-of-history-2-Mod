...
public class Menu_NewGame_Players extends SliderMenu {
   ...
   public List<MenuElement> buildMenuElements() {
      ...
      menuElements.add(
         new Button_Menu_LR(
            ...
            CFG.GAME_WIDTH,
            CFG.BUTTON_HEIGHT,
            CFG.game.getPlayersSize() < CFG.game.getAvailableCivilizations()
               && CFG.game.getPlayersSize() < CFG.game.getGameScenarios().getNumOfCivs(CFG.game.getScenarioID())
         )
      );
      menuElements.add(
         new Button_Flag(
            CFG.game.getPlayer(0).getCivID(),
            0,
      ...
      menuElements.add(
         new Button_Menu_Classic(
            "P1 | "
               ...
               + ": "
               + (
                  CFG.game.getPlayer(0).getCivID() < 0
                     ? CFG.langManager.get("RandomCivilization")
                     : CFG.game.getCiv(CFG.game.getPlayer(0).getCivID()).getCivName()
               ),
            CFG.PADDING * 2,
            CFG.PADDING * 4 + (int)(CFG.CIV_COLOR_WIDTH * CFG.GUI_SCALE) + CFG.CIV_FLAG_WIDTH,
            CFG.BUTTON_HEIGHT + CFG.PADDING * 2,
            CFG.GAME_WIDTH - CFG.PADDING * 4 - (int)(CFG.CIV_COLOR_WIDTH * CFG.GUI_SCALE) - CFG.CIV_FLAG_WIDTH,
            CFG.BUTTON_HEIGHT,
            true
         )
      );
      ...
      for (int i = 1; i < CFG.game.getPlayersSize(); i++) {
         ...
         menuElements.add(
            new Button_Menu_Classic(
               ...
               CFG.PADDING * 4 + (int)(CFG.CIV_COLOR_WIDTH * CFG.GUI_SCALE) + CFG.CIV_FLAG_WIDTH,
               CFG.BUTTON_HEIGHT * (i + 1) + CFG.PADDING * (i + 2),
               CFG.GAME_WIDTH - CFG.BUTTON_WIDTH - CFG.BUTTON_WIDTH / 2 - CFG.PADDING * 4 - (int)(CFG.CIV_COLOR_WIDTH * CFG.GUI_SCALE) - CFG.CIV_FLAG_WIDTH,
               CFG.BUTTON_HEIGHT,
               true
            )
         );
