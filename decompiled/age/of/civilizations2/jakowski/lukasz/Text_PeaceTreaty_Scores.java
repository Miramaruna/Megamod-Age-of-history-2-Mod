...
public class Text_PeaceTreaty_Scores extends Text {
   public static final float FONT_SCALE = 0.8F;
   public boolean row = false;
   public int iCivID;
   public Color colorWarScore;

   public Text_PeaceTreaty_Scores(int nCivID, int iVicPointsLeft, int iPosX, int iPosY, int iWidth) {
      super("" + iVicPointsLeft, CFG.PADDING * 2, iPosX, iPosY, iWidth, Math.max(CFG.TEXT_HEIGHT + CFG.PADDING * 4, CFG.CIV_FLAG_HEIGHT + CFG.PADDING * 4));
      this.iCivID = nCivID;
      this.colorWarScore = iVicPointsLeft > 0
         ? CFG.COLOR_TEXT_NUM_OF_PROVINCES
         : (iVicPointsLeft < 0 ? CFG.COLOR_TEXT_MODIFIER_NEGATIVE2 : CFG.COLOR_TEXT_MODIFIER_NEUTRAL);
   ...
   public void draw(SpriteBatch oSB, int iTranslateX, int iTranslateY, boolean isActive, boolean scrollableY) {
      ...
      ImageManager.getImage(Images.victoryPoints)
         .draw(
            ...
            this.getPosY()
               + this.getHeight() / 2
               - (int)(ImageManager.getImage(Images.victoryPoints).getHeight() * this.getImageScale(ImageManager.getImage(Images.victoryPoints).getHeight()))
                  / 2
               + iTranslateY
               - ImageManager.getImage(Images.victoryPoints).getHeight(),
            (int)(ImageManager.getImage(Images.victoryPoints).getWidth() * this.getImageScale(ImageManager.getImage(Images.victoryPoints).getHeight())),
            (int)(ImageManager.getImage(Images.victoryPoints).getHeight() * this.getImageScale(ImageManager.getImage(Images.victoryPoints).getHeight()))
         );
      CFG.game
         .getCiv(this.iCivID)
      ...
      CFG.drawTextWithShadow(
         oSB,
         this.sText,
         this.getPosX() + CFG.PADDING + CFG.CIV_FLAG_WIDTH + CFG.PADDING + iTranslateX,
         this.getPosY() + this.getHeight() / 2 - (int)(this.iTextHeight * 0.8F / 2.0F) + iTranslateY,
         this.colorWarScore
      );
   ...
   public void drawActive(SpriteBatch oSB, int iTranslateX, int iTranslateY, boolean isActive, boolean scrollableY) {
      if (CFG.peaceTreatyData.iBrushCivID == this.getCurrent()) {
   ...
   public void buildElementHover() {
      ...
      nData.add(new MenuElement_Hover_v2_Element_Type_Text("" + this.getText(), this.colorWarScore));
      ...
      for (int i = 0; i < CFG.peaceTreatyData.peaceTreatyGameData.lCivsDemands_Defenders.size(); i++) {
         if (CFG.peaceTreatyData.peaceTreatyGameData.lCivsDemands_Defenders.get(i).iCivID == this.iCivID) {
            for (int j = 0; j < CFG.peaceTreatyData.peaceTreatyGameData.lCivsDemands_Defenders.get(i).lDemands.size(); j++) {
               nData.add(new MenuElement_Hover_v2_Element_Type_Flag(this.iCivID));
               nData.add(
                  new MenuElement_Hover_v2_Element_Type_Text(
                     CFG.game.getProvince(CFG.peaceTreatyData.peaceTreatyGameData.lCivsDemands_Defenders.get(i).lDemands.get(j)).getName()
                  )
               );
               nData.add(new MenuElement_Hover_v2_Element_Type_Image(Images.victoryPoints, CFG.PADDING, CFG.PADDING));
               nData.add(
                  new MenuElement_Hover_v2_Element_Type_Text(
                     ""
                        + CFG.peaceTreatyData.drawProvinceOwners.get(CFG.peaceTreatyData.peaceTreatyGameData.lCivsDemands_Defenders.get(i).lDemands.get(j)).iProvinceValue,
                     CFG.COLOR_TEXT_NUM_OF_PROVINCES
                  )
               );
               nElements.add(new MenuElement_Hover_v2_Element2(nData));
      ...
      for (int var6 = 0; var6 < CFG.peaceTreatyData.peaceTreatyGameData.lCivsDemands_Aggressors.size(); var6++) {
         if (CFG.peaceTreatyData.peaceTreatyGameData.lCivsDemands_Aggressors.get(var6).iCivID == this.iCivID) {
            for (int j = 0; j < CFG.peaceTreatyData.peaceTreatyGameData.lCivsDemands_Aggressors.get(var6).lDemands.size(); j++) {
               nData.add(new MenuElement_Hover_v2_Element_Type_Flag(this.iCivID));
               nData.add(
                  new MenuElement_Hover_v2_Element_Type_Text(
                     CFG.game.getProvince(CFG.peaceTreatyData.peaceTreatyGameData.lCivsDemands_Aggressors.get(var6).lDemands.get(j)).getName()
                  )
               );
               nData.add(new MenuElement_Hover_v2_Element_Type_Image(Images.victoryPoints, CFG.PADDING, CFG.PADDING));
               nData.add(
                  new MenuElement_Hover_v2_Element_Type_Text(
                     ""
                        + CFG.peaceTreatyData.drawProvinceOwners.get(CFG.peaceTreatyData.peaceTreatyGameData.lCivsDemands_Aggressors.get(var6).lDemands.get(j)).iProvinceValue,
                     CFG.COLOR_TEXT_NUM_OF_PROVINCES
                  )
               );
               nElements.add(new MenuElement_Hover_v2_Element2(nData));
   ...
   public void actionElement(int iID) {
      CFG.peaceTreatyData.iBrushCivID = this.getCurrent();

      try {
         CFG.toast.setInView(CFG.game.getCiv(CFG.peaceTreatyData.iBrushCivID).getCivName(), CFG.COLOR_TEXT_MODIFIER_POSITIVE);
