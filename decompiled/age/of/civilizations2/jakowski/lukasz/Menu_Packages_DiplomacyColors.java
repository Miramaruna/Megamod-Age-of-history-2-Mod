...
public class Menu_Packages_DiplomacyColors extends SliderMenu {
   ...
   public Menu_Packages_DiplomacyColors() {
      ...
      for (int i = 0; i < tagsSPLITED.length; i++) {
         menuElements.add(
            new Button_Menu(
               CFG.getPackageDiplomacyColorsDataName(tagsSPLITED[i]),
         ...
         menuElements.add(
            new Button_Menu_Classic_ReflectedCheckbox(
               tagsSPLITED[i].equals(CFG.sACTIVE_DIPLOMACY_COLORS_TAG) ? CFG.langManager.get("Active") : CFG.langManager.get("Enable"),
               (int)(50.0F * CFG.GUI_SCALE),
               CFG.GAME_WIDTH - CFG.BUTTON_WIDTH * 2,
               CFG.BUTTON_HEIGHT * (i + 1) + CFG.PADDING * (i + 2),
               CFG.BUTTON_WIDTH * 2,
               CFG.BUTTON_HEIGHT,
               true,
               tagsSPLITED[i].equals(CFG.sACTIVE_DIPLOMACY_COLORS_TAG)
            )
         );
      ...
      switch (iID) {
         ...
         default:
            if (iID % 2 == 0) {
               ...
               CFG.menuManager.getColorPicker().setVisible(true, ColorPicker_AoC.PickerAction.COLOR_DIPLOMACY_OWN_PROVINCES);
            } else if (!CFG.sACTIVE_DIPLOMACY_COLORS_TAG.equals(this.lTags.get((iID - 2) / 2))) {
