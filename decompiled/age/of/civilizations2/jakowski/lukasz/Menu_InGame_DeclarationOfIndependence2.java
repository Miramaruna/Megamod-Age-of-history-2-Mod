...
class Menu_InGame_DeclarationOfIndependence2 extends SliderMenu {
   private int iOnCivID = -1;

   protected Menu_InGame_DeclarationOfIndependence2(int var1) {
      ...
      var2.add(
         new Button_NS_Opinion(this, var1, CFG.game.getCiv(CFG.getActiveCivInfo()).getPuppetOfCivID(), Images.b_workshop, 0, 0, 0, 0, CFG.BUTTON_WIDTH * 0) {
            final Menu_InGame_DeclarationOfIndependence2 this$0;
            ...
            protected void drawText(SpriteBatch var1, int var2x, int var3x, boolean var4x) {
               ImageManager.getImage(Images.b_workshop)
                  .draw(
                     ...
                     this.getPosX()
                        + this.getWidth() / 2
                        - (int)((this.getTextWidth() * 0.8F + ImageManager.getImage(Images.b_workshop).getWidth() + CFG.PADDING) / 2.0F)
                        + var2x,
                     this.getPosY() + this.getHeight() / 2 - ImageManager.getImage(Images.b_workshop).getHeight() / 2 + var3x
                  );
               ...
               if (this.getTextPos() < 0) {
                  var7 = this.getWidth() / 2
                     - (int)((this.getTextWidth() * 0.8F + ImageManager.getImage(Images.b_workshop).getWidth() + CFG.PADDING) / 2.0F)
                     + ImageManager.getImage(Images.b_workshop).getWidth()
                     + CFG.PADDING;
               } else {
      ...
      String var6;
      if (CFG.game.getCiv(var1).isBuildBuildingsForPlayer <= 0) {
         var5 = CFG.langManager;
         var6 = "Automaticconstruction";
      } else {
         var5 = CFG.langManager;
         var6 = "StopAutomaticconstruction";
      }
   ...
   protected final void actionElement(int var1) {
      if (var1 == this.getMenuElementsSize() - 1) {
         var1 = CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID();
         if (CFG.game.getCiv(var1).isBuildBuildingsForPlayer <= 0) {
            CFG.oAI.getAI_Style(CFG.game.getCiv(var1).getAI_Style()).buildBuildings_ForPlayer(var1);
            CFG.game.getCiv(var1).isBuildBuildingsForPlayer = 1;
            CFG.menuManager.updateInGame_TOP_All(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID());
         } else {
            CFG.game.getCiv(var1).isBuildBuildingsForPlayer = 0;
         }
   ...
   protected void draw(SpriteBatch var1, int var2, int var3, boolean var4) {
      if (Menu_InGame_OfferAlliance.lTime + 200L >= System.currentTimeMillis()) {
         ...
         var7 = this.getPosY();
         int var12 = ImageManager.getImage(Images.new_game_top_edge).getHeight();
         var8 = ImageManager.getImage(Images.new_game_top_edge).getWidth();
         var6 = this.getHeight();
         var14.draw2(var1, var9 + 2 + var10 - var11 + var2, var7 - var12 + var3, var8, CFG.PADDING + var6, true, true);
         ...
         var15.draw2(var1, var27 - 2 + var2, var21 - var18 + var3, var30 - var24 + 4, CFG.PADDING + var33, false, true);
         var15 = ImageManager.getImage(Images.new_game_top_edge);
         var30 = this.getPosX();
         var27 = this.getWidth();
         int var35 = ImageManager.getImage(Images.new_game_top_edge).getWidth();
         var33 = this.getPosY();
         var24 = ImageManager.getImage(Images.new_game_top_edge).getHeight();
         var18 = ImageManager.getImage(Images.new_game_top_edge).getWidth();
         var21 = this.getHeight();
         var15.draw2(var1, var30 + 2 + var27 - var35 + var2, var33 - var24 + var3, var18, CFG.PADDING + var21, true, true);
         var1.setColor(new Color(0.0F, 0.0F, 0.0F, 0.45F));
         ImageManager.getImage(Images.gradient)
            .draw(
               var1,
