...
public class Menu_Packages_PalletOfCivsColors extends SliderMenu {
   public Menu_Packages_PalletOfCivsColors() {
      ...
      for (int i = 0; i < CFG.palletManager.getNumOfPallets(); i++) {
         menuElements.add(
            new Button_Menu("", (int)(50.0F * CFG.GUI_SCALE), 0, CFG.BUTTON_HEIGHT * (i + 1) + CFG.PADDING * (i + 2), CFG.GAME_WIDTH, CFG.BUTTON_HEIGHT, true) {
               int iCurrent = 0;
               ...
               public void drawText(SpriteBatch oSB, int iTranslateX, int iTranslateY, boolean isActive) {
                  CFG.palletManager
                     .drawSampleColors(
                        oSB,
                        this.getPosX() + this.getWidth() / 2 - CFG.BUTTON_WIDTH + iTranslateX,
                        this.getPosY() + this.getHeight() / 2 - this.getHeight() / 4 + Menu_Packages_PalletOfCivsColors.this.getMenuPosY(),
                        CFG.BUTTON_WIDTH * 2,
                        this.getHeight() / 2,
                        this.iCurrent,
                        isActive
