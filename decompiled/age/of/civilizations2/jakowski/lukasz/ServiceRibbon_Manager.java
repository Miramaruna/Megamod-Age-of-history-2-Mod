...
public class ServiceRibbon_Manager {
   public List<ServiceRibbon_GameData> lSR;
   public List<String> lTags;
   public int iSRSize;
   public List<List<Image>> lSRImages = null;
   public int iSRImagesSize;
   public List<Image> lSROverlayImages = null;
   public int iSROverlayImagesSize;

   public final Statistics_Civ_GameData loadStatistics_Civ(String nTag) {
      try {
   ...
   public final void drawSROverlay(SpriteBatch oSB, int nPosX, int nPosY, ServiceRibbon_Overlay_GameData nSROverlay, Color nColor, int nExtraScale) {
      ...
      if (nSROverlay.getReflected()) {
         ImageManager.getImage(Images.pix255_255_255)
            .draw(
               oSB,
               nPosX + (int)(nSROverlay.getPosX() * CFG.GUI_SCALE * nExtraScale),
               nPosY - ImageManager.getImage(Images.pix255_255_255).getHeight(),
               (int)(nSROverlay.getWidth() * CFG.GUI_SCALE * nExtraScale),
         ...
         ImageManager.getImage(Images.pix255_255_255)
            .draw(
               ...
               nPosX
                  ...
                  - (int)(nSROverlay.getPosX() * CFG.GUI_SCALE * nExtraScale)
                  - (int)(nSROverlay.getWidth() * CFG.GUI_SCALE * nExtraScale),
               nPosY - ImageManager.getImage(Images.pix255_255_255).getHeight(),
               (int)(nSROverlay.getWidth() * CFG.GUI_SCALE * nExtraScale),
               CFG.SERVICE_RIBBON_HEIGHT * nExtraScale
   ...
   public final void loadSRImages() {
      ...
      for (int i2 = 0; i2 < tagsSPLITED.length; i2++) {
         try {
            ArrayList<Image> tempSRImages = new ArrayList<>();

            for (int j = 1; j < 6; j++) {
               tempSRImages.add(
                  new Image(new Texture("UI/" + CFG.getRescouresPath() + "sr/" + tagsSPLITED[i2] + "/" + j + ".png"), Texture.TextureFilter.Linear)
               );
            }
      ...
      for (int i3 = 0; i3 < tagsSPLITED2.length; i3++) {
         try {
            this.lSROverlayImages
               .add(new Image(new Texture("UI/" + CFG.getRescouresPath() + "sr_over/" + tagsSPLITED2[i3] + ".png"), Texture.TextureFilter.Linear));
