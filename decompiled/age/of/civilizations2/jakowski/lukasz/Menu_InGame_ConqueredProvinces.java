...
public class Menu_InGame_ConqueredProvinces extends SliderMenu {
   ...
   public Menu_InGame_ConqueredProvinces(int tInit) {
      ...
      this.initMenu(null, CFG.GAME_WIDTH * 3 / 4 - tempWidth / 2, tempMenuPosY, tempWidth, CFG.GAME_HEIGHT * 3 / 5, menuElements, false, false);
   }
   ...
   public Menu_InGame_ConqueredProvinces() {
      ...
      menuElements.add(new Button_Statistics_Title(CFG.langManager.get("Civilizations"), CFG.PADDING * 2, 2, 0, CFG.BUTTON_WIDTH * 2, tElemHeight) {
         @Override
         public int getWidth() {
            return Menu_InGame_ConqueredProvinces.this.getElementW() * 2 + CFG.PADDING * 2 - 2;
         }

         @Override
         public Color getColor(boolean isActive) {
            return Menu_InGame_ConqueredProvinces.iSort == 0 ? CFG.COLOR_TEXT_NUM_OF_PROVINCES : super.getColor(isActive);
         }
      ...
      menuElements.add(
         new Button_Statistics_Title(
            CFG.langManager.get("ConqueredProvinces"), CFG.PADDING, CFG.PADDING * 2 + CFG.BUTTON_WIDTH * 5, 0, CFG.BUTTON_WIDTH, tElemHeight
         ) {
            @Override
            public int getPosX() {
               return Menu_InGame_ConqueredProvinces.this.getElementW() * 2 + CFG.PADDING * 2;
            }

            @Override
            public int getWidth() {
               return Menu_InGame_ConqueredProvinces.this.getW() - Menu_InGame_ConqueredProvinces.this.getElementW() * 2 + CFG.PADDING * 2 - 2;
            }

            @Override
            public Color getColor(boolean isActive) {
               return Menu_InGame_ConqueredProvinces.iSort == 1 ? CFG.COLOR_TEXT_NUM_OF_PROVINCES : super.getColor(isActive);
            }
      ...
      if (iSort == 0) {
         ...
         for (int i2 = 1; i2 < CFG.game.getCivsSize(); i2++) {
            ...
            tempScore.add(CFG.game.getCiv(CFG.game.getSortedCivsAZ(i2 - 1)).civGameData.iNumOfConqueredProvinces);
         }
      ...
      for (int i = 0; i < iSize; i++) {
         ...
         menuElements.add(
            new Button_Statistics(
               CFG.getNumberWithSpaces("" + CFG.game.getCiv(tSorted.get(i)).civGameData.iNumOfConqueredProvinces),
               CFG.PADDING,
   ...
   public void updateLanguage() {
      try {
         this.getTitle().setText(CFG.langManager.get("ConqueredProvinces"));
