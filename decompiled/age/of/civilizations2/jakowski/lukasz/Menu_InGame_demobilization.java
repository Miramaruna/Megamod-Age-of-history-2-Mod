...
public class Menu_InGame_demobilization extends SliderMenu {
   public int iOnCivID = -1;

   public Menu_InGame_demobilization(int onCivID) {
      ...
      menuElements.add(
         new Button_Build_Building_Destroy(
            CFG.langManager.get("demobilisation"),
            CFG.langManager.get("demobilisation") + ": " + CFG.game.getCiv(this.iOnCivID).getCivName(),
            ...
            @Override
            public int getWidth() {
               return Menu_InGame_demobilization.this.getElementW() * 2;
            }
      ...
      menuElements.add(new Button_FlagActionSliderStyle(CFG.langManager.get("Cancel"), -1, 2 + CFG.PADDING, var7 = tY + CFG.PADDING, CFG.BUTTON_WIDTH, true) {
         @Override
         public int getWidth() {
            return Menu_InGame_demobilization.this.getElementW() - CFG.PADDING - CFG.PADDING / 2;
         }
      });
      menuElements.add(
         new Button_FlagActionSliderStyle(CFG.langManager.get("demobilisation"), -1, 2, var7, CFG.BUTTON_WIDTH, true) {
            @Override
            public int getPosX() {
               return Menu_InGame_demobilization.this.getElementW() + CFG.PADDING / 2;
            }
            ...
            public void buildElementHover() {
               ...
               nData.add(new MenuElement_Hover_v2_Element_Type_Text(CFG.langManager.get("demobilisation") + "!", CFG.COLOR_TEXT_MODIFIER_NEGATIVE2));
      ...
      this.initMenu(
         new SliderMenuTitle(CFG.langManager.get("demobilisation"), CFG.BUTTON_HEIGHT * 3 / 5, true, true) {
            @Override
            public void draw(SpriteBatch oSB, int iTranslateX, int nPosX, int nPosY, int nWidth, boolean sliderMenuIsActive) {
               ...
               CFG.game
                  ...
                  .draw(
                     oSB,
                     Menu_InGame_demobilization.this.getPosX() + CFG.PADDING * 2 + iTranslateX,
                     Menu_InGame_demobilization.this.getPosY()
                        - this.getHeight() / 2
               ...
               ImageManager.getImage(Images.flag_rect)
                  .draw(
                     oSB,
                     Menu_InGame_demobilization.this.getPosX() + CFG.PADDING * 2 + iTranslateX,
                     Menu_InGame_demobilization.this.getPosY() - this.getHeight() / 2 - CFG.CIV_FLAG_HEIGHT / 2
                  );
               CFG.fontMain.getData().setScale(0.8F);
               CFG.drawText(
                  oSB,
   ...
   public final void actionElement(int paramInt) {
      if (paramInt >= 0 && paramInt < this.getMenuElementsSize()) {
         if (paramInt == this.getMenuElementsSize() - 1) {
            int civId = CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID();
            CFG.game_NextTurnUpdate.updateCivRecruitableArmy(civId);

            for (int provinceIndex : CFG.game.getCiv(civId).lProvincesWithMoreRecruitableArmy) {
               int recruitableArmy = CFG.game.getCiv(civId).lProvincesWithMoreRecruitableArmy.get(0);
               CFG.gameAction.disbandArmy(provinceIndex, recruitableArmy, civId);
