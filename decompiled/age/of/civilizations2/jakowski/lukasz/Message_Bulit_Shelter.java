...
public class Message_Bulit_Shelter extends Message {
   public Message_Bulit_Shelter(int fromCivID, int iValue) {
      super(fromCivID, iValue);
      this.messageType = Message_Type.BUILT_NUCLEAR_REACTOR;
      ...
      if (CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID() == iCivID) {
         ...
         lMess.add(
            (CFG.game.getProvince(this.iValue).getName().length() > 0 ? CFG.game.getProvince(this.iValue).getName() + ": " : "")
               + CFG.langManager.get(BuildingsManager.getShelter_Name(CFG.game.getProvince(this.iValue).getLevelOfShelter()))
         );
   ...
   public MenuElement_Hover_v2 getHover() {
      ...
      nData.add(
         new MenuElement_Hover_v2_Element_Type_Text(
            CFG.langManager.get(BuildingsManager.getShelter_Name(CFG.game.getProvince(this.iValue).getLevelOfShelter())), CFG.COLOR_TEXT_NUM_OF_PROVINCES
         )
      );
      nData.add(new MenuElement_Hover_v2_Element_Type_Image(Images.b_shelter, CFG.PADDING, 0));
      nElements.add(new MenuElement_Hover_v2_Element2(nData));
      nData.clear();
      ...
      nData.add(
         new MenuElement_Hover_v2_Element_Type_Text(
            "-" + (int)(BuildingsManager.getShelter_Bonus(CFG.game.getProvince(this.iValue).getLevelOfShelter()) * 100.0F) + "%",
