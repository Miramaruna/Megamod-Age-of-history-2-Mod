...
public class Message_InvestDone_Development extends Message {
   public Message_InvestDone_Development(int fromCivID, int iValue) {
      super(fromCivID, iValue);
      this.messageType = Message_Type.INVEST_IS_OVER_DEVELOPMENT;
   ...
   public MenuElement_Hover_v2 getHover() {
      ...
      nData.add(
         new MenuElement_Hover_v2_Element_Type_Text(
            "" + String.format("%.2f", CFG.game.getProvince(this.iValue).getDevelopmentLevel()).replace(',', '.'), CFG.COLOR_TEXT_MODIFIER_NEUTRAL2
         )
      );
      nData.add(new MenuElement_Hover_v2_Element_Type_Image(Images.development, CFG.PADDING, 0));
      nElements.add(new MenuElement_Hover_v2_Element2(nData));
      nData.clear();
      if (CFG.game.getCiv(this.iFromCivID).getTechnologyLevel() - 0.01F <= CFG.game.getProvince(this.iValue).getDevelopmentLevel()) {
         ...
         nData.add(
            new MenuElement_Hover_v2_Element_Type_Text(
               "" + (int)(CFG.game.getCiv(this.iFromCivID).getTechnologyLevel() * 100.0F) / 100.0F, CFG.COLOR_TEXT_TECHNOLOGY
            )
         );
