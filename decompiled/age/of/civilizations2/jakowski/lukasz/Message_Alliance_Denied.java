...
public class Message_Alliance_Denied extends Message {
   public Message_Alliance_Denied(int fromCivID) {
      super(fromCivID, 0);
      this.messageType = Message_Type.ALLIANCE_DENIED;
   ...
   public void onAccept(int iCivID) {
      if (CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID() == iCivID) {
         ...
         lMess.add(CFG.langManager.get("AllianceProposalWasRejected"));
   ...
   public MenuElement_Hover_v2 getHover() {
      ...
      nData.add(new MenuElement_Hover_v2_Element_Type_Text(CFG.langManager.get("AllianceProposalWasRejected"), CFG.COLOR_TEXT_MODIFIER_NEGATIVE2));
