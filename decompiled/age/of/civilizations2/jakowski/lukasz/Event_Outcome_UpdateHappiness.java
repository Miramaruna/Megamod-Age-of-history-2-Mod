...
public class Event_Outcome_UpdateHappiness extends Event_Outcome {
   public int iCivID = -1;
   public List<Integer> lProvinces = new ArrayList<>();
   public int iValue = 0;

   Event_Outcome_UpdateHappiness() {
   }
         ...
         for (int i = 0; i < this.getProvinces().size(); i++) {
            if (this.canMakeAction(i)) {
               CFG.game
                  ...
                  .setHappiness(CFG.game.getProvince(this.getProvinces().get(i)).getHappiness() + this.getValue() / 100.0F);
            }
         }
      }
   }
