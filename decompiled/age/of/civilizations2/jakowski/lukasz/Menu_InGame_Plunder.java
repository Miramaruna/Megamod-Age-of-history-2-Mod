...
public class Menu_InGame_Plunder extends SliderMenu {
   ...
   public Menu_InGame_Plunder() {
      ...
      this.initMenu(
         new SliderMenuTitle(CFG.langManager.get("Plunder"), CFG.BUTTON_HEIGHT * 3 / 5, true, true),
   ...
   public Menu_InGame_Plunder(final int nProvinceID) {
      ...
      int tY = 0;
      menuElements.add(
         new Button_Diplomacy_Plunder(
            "" + CFG.ideologiesManager.getIdeology(CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getIdeologyID()).COST_OF_PLUNDER / 10.0F,
            CFG.game.getProvince(nProvinceID).getPopulationData().getPopulation(),
            DiplomacyManager.plunderProvinceIncome(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(), nProvinceID, 0),
            2,
            tY,
            CFG.BUTTON_WIDTH * 2
         ) {
            @Override
            public int getWidth() {
               return Menu_InGame_Plunder.this.getElementW() * 2;
            }

            @Override
            public void actionElement(int iID) {
               CFG.game.setActiveProvinceID(Menu_InGame_Plunder.iProvinceID);
               CFG.map.getMapCoordinates().centerToProvinceID(CFG.game.getActiveProvinceID());
            }
         }
      );
      tY += menuElements.get(menuElements.size() - 1).getHeight();
      int currArmyPlunder = 0;

      for (int i = 0; i < CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getMoveUnitsPlunderSize(); i++) {
         if (CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getMoveUnits_Plunder(i).getFromProvinceID() == nProvinceID) {
            currArmyPlunder = CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getMoveUnits_Plunder(i).getNumOfUnits();
            break;
         }
      }
      ...
      menuElements.add(
         new Slider_FlagAction_Clear(
            ...
            0,
            CFG.game.getProvince(nProvinceID).getArmyCivID(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()) + currArmyPlunder,
            currArmyPlunder == 0 ? CFG.game.getProvince(nProvinceID).getArmyCivID(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()) : currArmyPlunder,
            0.65F
         ) {
            @Override
            public int getWidth() {
               return Menu_InGame_Plunder.this.getElementW() * 2 - CFG.PADDING * 4;
            }
            ...
            @Override
            public void actionElement(int iID) {
               Menu_InGame_Plunder.this.getMenuElement(0)
                  .setCurrent(
                     (int)(DiplomacyManager.plunderEfficiency(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(), nProvinceID, this.getCurrent()) * 100.0F)
                  );
            }
         }
      );
      ...
      menuElements.add(new Button_FlagActionSliderStyle(CFG.langManager.get("Cancel"), -1, 2 + CFG.PADDING, var9 = tY + CFG.PADDING, CFG.BUTTON_WIDTH, true) {
         @Override
         public int getWidth() {
            return Menu_InGame_Plunder.this.getElementW() - CFG.PADDING - CFG.PADDING / 2;
         }
      });
      menuElements.add(
         new Button_FlagActionSliderStyle(CFG.langManager.get("Confirm"), -1, 2, var9, CFG.BUTTON_WIDTH, true) {
            @Override
            public int getPosX() {
               return Menu_InGame_Plunder.this.getElementW() + CFG.PADDING / 2;
            }
            ...
            public void buildElementHover() {
               ...
               nData.add(new MenuElement_Hover_v2_Element_Type_Flag(CFG.game.getProvince(Menu_InGame_Plunder.iProvinceID).getTrueOwnerOfProvince()));
               nData.add(new MenuElement_Hover_v2_Element_Type_Text(Menu_InGame_Plunder.this.getTitle().getText(), CFG.COLOR_TEXT_NUM_OF_PROVINCES));
               nData.add(new MenuElement_Hover_v2_Element_Type_Image(Images.diplo_plunder, CFG.PADDING, 0));
               nElements.add(new MenuElement_Hover_v2_Element2(nData));
               nData.clear();
               nData.add(new MenuElement_Hover_v2_Element_Type_Text(CFG.langManager.get("Army") + ": "));
               nData.add(
                  new MenuElement_Hover_v2_Element_Type_Text("" + Menu_InGame_Plunder.this.getMenuElement(1).getCurrent(), CFG.COLOR_BUTTON_GAME_TEXT_ACTIVE)
               );
               ...
               nData.add(
                  new MenuElement_Hover_v2_Element_Type_Text(
                     "+"
                        + DiplomacyManager.plunderTreasuryIncome(
                           CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(),
                           Menu_InGame_Plunder.iProvinceID,
                           Menu_InGame_Plunder.this.getMenuElement(1).getCurrent()
                        ),
               ...
               nElements.add(new MenuElement_Hover_v2_Element2(nData));
               nData.clear();
               nData.add(new MenuElement_Hover_v2_Element_Type_Text(CFG.langManager.get("Food") + ": "));
               nData.add(
                  new MenuElement_Hover_v2_Element_Type_Text(
                     "+"
                        + DiplomacyManager.plunderTreasuryIncome(
               ...
               nData.add(
                  new MenuElement_Hover_v2_Element_Type_Text(
                     "-"
                        + (int)(
                              DiplomacyManager.plunder_LossesEconomy_Perc(
                                    CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(),
                                    Menu_InGame_Plunder.iProvinceID,
                                    Menu_InGame_Plunder.this.getMenuElement(1).getCurrent()
                                 )
               ...
               nData.add(
                  new MenuElement_Hover_v2_Element_Type_Text(
                     "-"
                        + (int)(
                              DiplomacyManager.plunder_Happiness(
                                    CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(),
                                    Menu_InGame_Plunder.iProvinceID,
                                    Menu_InGame_Plunder.this.getMenuElement(1).getCurrent()
                                 )
            ...
            public void drawText(SpriteBatch oSB, int iTranslateX, int iTranslateY, boolean isActive) {
               ImageManager.getImage(Images.diplo_plunder)
                  .draw(
                     ...
                     this.getPosX()
                        + this.getWidth() / 2
                        - (int)((this.getTextWidth() * 0.8F + ImageManager.getImage(Images.diplo_plunder).getWidth() + CFG.PADDING) / 2.0F)
                        + iTranslateX,
                     this.getPosY() + this.getHeight() / 2 - ImageManager.getImage(Images.diplo_plunder).getHeight() / 2 + iTranslateY
                  );
                  ...
                  this.getPosX()
                     + (
                        this.getTextPos() < 0
                           ? this.getWidth() / 2
                              - (int)((this.getTextWidth() * 0.8F + ImageManager.getImage(Images.diplo_plunder).getWidth() + CFG.PADDING) / 2.0F)
                              + ImageManager.getImage(Images.diplo_plunder).getWidth()
                              + CFG.PADDING
                           : this.getTextPos()
                     )
                     + iTranslateX,
      ...
      this.initMenu(
         new SliderMenuTitle(
            ...
            public void draw(SpriteBatch oSB, int iTranslateX, int nPosX, int nPosY, int nWidth, boolean sliderMenuIsActive) {
               ...
               CFG.game
                  .getCiv(CFG.game.getProvince(Menu_InGame_Plunder.iProvinceID).getTrueOwnerOfProvince())
                  .getFlag()
                  .draw(
                     oSB,
                     ...
                     nPosY
                        ...
                        - CFG.game.getCiv(CFG.game.getProvince(Menu_InGame_Plunder.iProvinceID).getTrueOwnerOfProvince()).getFlag().getHeight(),
                     CFG.CIV_FLAG_WIDTH,
                     CFG.CIV_FLAG_HEIGHT
                  );
      ...
      this.getMenuElement(0)
         .setCurrent(
            (int)(
               DiplomacyManager.plunderEfficiency(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(), nProvinceID, this.getMenuElement(1).getCurrent()) * 100.0F
            )
         );
   ...
   public final void actionElement(int iID) {
      if (iID == this.getMenuElementsSize() - 1) {
         if (this.getMenuElement(1).getCurrent() >= 0) {
            DiplomacyManager.plunderProvince(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(), iProvinceID, this.getMenuElement(1).getCurrent());
