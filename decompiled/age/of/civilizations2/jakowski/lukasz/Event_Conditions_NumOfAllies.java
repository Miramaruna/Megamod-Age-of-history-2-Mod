...
public class Event_Conditions_NumOfAllies extends Event_Conditions {
   public int iCivID = -1;
   public int iValue = 0;

   Event_Conditions_NumOfAllies() {
   }
   ...
   public boolean outCondition() {
      try {
         return CFG.game.getCiv(this.getCivID()).getAllianceID() > 0
            ? CFG.game.getAlliance(CFG.game.getCiv(this.getCivID()).getAllianceID()).getCivilizationsSize() >= this.getValue()
            : 0 >= this.getValue();
