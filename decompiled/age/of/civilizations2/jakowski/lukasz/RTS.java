...
public class RTS {
   ...
   public static void updateTimePast_AfterAction(float fPerc) {
      if ((TIME_PAST = TIME_PAST - (int)(TIME_REQUIRED_TO_ACTION[SPEED] * fPerc)) < 0L) {
         TIME_PAST = 0L;
      }
   }
   ...
   public static final void updateTime() {
      ...
      if (timePasted()) {
         ...
         if (CFG.gameAction.getActiveTurnState() == Game_Action.TurnStates.INPUT_ORDERS) {
            ...
            && CFG.menuManager.getInGame_ProvinceInfo().getMenuElement(0).getClickable()) {
            CFG.gameAction.tryToTakeNexTurn();
            resetTime();
         }
      }
   ...
   public static final boolean timePasted() {
      return TIME_PAST > getRequiredTime();
   }
   ...
   public static final int getRequiredTime() {
      if (CFG.gameAction.getActiveTurnState() == Game_Action.TurnStates.INPUT_ORDERS) {
         ...
         return CFG.gameAction.getActiveTurnState() != Game_Action.TurnStates.LOAD_AI_RTO
               && CFG.gameAction.getActiveTurnState() != Game_Action.TurnStates.LOADING_NEXT_TURN
            ? TIME_REQUIRED_TO_ACTION[SPEED]
            : TIME_REQUIRED_TO_ACTION_LOADAI[SPEED];
      }
   }
         ...
         if (!CFG.SPECTATOR_MODE
            && (
               CFG.viewsManager.getActiveViewID() >= 0
                     && !CFG.viewsManager.getActiveView().canMoveArmy
                     && CFG.viewsManager.getActiveViewID() != ViewsManager.VIEW_DISEASES_MODE
                  ...
                  || CFG.menuManager.getInGame_ProvinceBuild_Visible()
                  || CFG.menuManager.getInGame_ProvinceChooseProvince_Visible()
                  || CFG.menuManager.getVisible_InGame_FlagAction()
                  || CFG.menuManager.getVisible_InGame_Budget()
                  || CFG.menuManager.getVisibleInGame_Event()
            )) {
            return false;
         }
      }
