...
public class Menu_InGame_AutoAssimilation extends SliderMenu {
   public int iOnCivID = -1;
   String str;

   public Menu_InGame_AutoAssimilation(int onCivID) {
      ...
      menuElements.add(
         new Button_NS_Opinion(
            ...
            @Override
            public int getWidth() {
               return Menu_InGame_AutoAssimilation.this.getElementW() * 2;
            }
         }
      );
      ...
      int var7;
      menuElements.add(
         new Button_FlagActionSliderStyle(CFG.langManager.get("FullAssimilation"), -1, 2 + CFG.PADDING, var7 = tY + CFG.PADDING, CFG.BUTTON_WIDTH * 2, true) {
            @Override
            public int getWidth() {
               return Menu_InGame_AutoAssimilation.this.getElementW() * 2 - CFG.PADDING * 2;
            }
            ...
            public void buildElementHover() {
               ...
               nData.add(new MenuElement_Hover_v2_Element_Type_Text(CFG.langManager.get("FullAssimilation_Info"), CFG.COLOR_BUTTON_GAME_TEXT_ACTIVE));
      ...
      var7 += menuElements.get(menuElements.size() - 1).getHeight() + CFG.PADDING;
      menuElements.add(
         new Button_FlagActionSliderStyle(CFG.langManager.get("AssimilateAllPeople"), -1, 2 + CFG.PADDING, var7, CFG.BUTTON_WIDTH * 2, true) {
            @Override
            public int getWidth() {
               return Menu_InGame_AutoAssimilation.this.getElementW() * 2 - CFG.PADDING * 2;
            }
            ...
            public void buildElementHover() {
               ArrayList<MenuElement_Hover_v2_Element2> nElements = new ArrayList<>();
               ArrayList<MenuElement_Hover_v2_Element_Type> nData = new ArrayList<>();
               nData.add(new MenuElement_Hover_v2_Element_Type_Text(CFG.langManager.get("AssimilateAllPeople_Info"), CFG.COLOR_BUTTON_GAME_TEXT_ACTIVE));
               nElements.add(new MenuElement_Hover_v2_Element2(nData));
               nData.clear();
               this.menuElementHover = new MenuElement_Hover_v2(nElements);
            }
      ...
      var7 += menuElements.get(menuElements.size() - 1).getHeight() + CFG.PADDING;
      menuElements.add(new Button_FlagActionSliderStyle(CFG.langManager.get("Cancel"), -1, 2 + CFG.PADDING, var7, CFG.BUTTON_WIDTH, true) {
         @Override
         public int getWidth() {
            return Menu_InGame_AutoAssimilation.this.getElementW() - CFG.PADDING - CFG.PADDING / 2;
         }
      });
      menuElements.add(
         new Button_FlagActionSliderStyle(CFG.langManager.get("Confirm"), -1, 2, var7, CFG.BUTTON_WIDTH, true) {
            @Override
            public int getPosX() {
               return Menu_InGame_AutoAssimilation.this.getElementW() + CFG.PADDING / 2;
            }
            ...
            public void buildElementHover() {
               ...
               nData.add(new MenuElement_Hover_v2_Element_Type_Flag(Menu_InGame_AutoAssimilation.this.iOnCivID, CFG.PADDING, CFG.PADDING));
               nData.add(new MenuElement_Hover_v2_Element_Type_Text(CFG.game.getCiv(Menu_InGame_AutoAssimilation.this.iOnCivID).getCivName()));
               nElements.add(new MenuElement_Hover_v2_Element2(nData));
               nData.clear();
            ...
            public void drawText(SpriteBatch oSB, int iTranslateX, int iTranslateY, boolean isActive) {
               ImageManager.getImage(Images.diplo_lord)
                  .draw(
                     oSB,
                     this.getPosX()
                        + this.getWidth() / 2
                        - (int)((this.getTextWidth() * 0.8F + ImageManager.getImage(Images.diplo_lord).getWidth() + CFG.PADDING) / 2.0F)
                        + iTranslateX,
                     this.getPosY() + this.getHeight() / 2 - ImageManager.getImage(Images.diplo_lord).getHeight() / 2 + iTranslateY
                  );
                  ...
                  this.getPosX()
                     + (
                        this.getTextPos() < 0
                           ? this.getWidth() / 2
                              - (int)((this.getTextWidth() * 0.8F + ImageManager.getImage(Images.diplo_lord).getWidth() + CFG.PADDING) / 2.0F)
                              + ImageManager.getImage(Images.diplo_lord).getWidth()
                              + CFG.PADDING
                           : this.getTextPos()
                     )
                     + iTranslateX,
      ...
      this.str = CFG.game.getCiv(onCivID).isAssimilateProvincesPlayer <= 0 ? "StartAssimilation" : "assimilatNo";
      this.initMenu(
         new SliderMenuTitle(CFG.langManager.get(this.str), CFG.BUTTON_HEIGHT * 3 / 5, true, true) {
            @Override
   ...
   public final void actionElement(int paramInt) {
      if (paramInt == 1) {
         ...
         CFG.menuManager.setVisible_Menu_InGame_CurrentWars(true);
         this.setVisible(false);
      } else if (paramInt == this.getMenuElementsSize() - 1) {
         paramInt = CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID();
         if (CFG.game.getCiv(paramInt).isAssimilateProvincesPlayer <= 0) {
            CFG.oAI.getAI_Style(CFG.game.getCiv(paramInt).getAI_Style());
            AI_Style.AllassimilateProvinces(paramInt);
            CFG.game.getCiv(paramInt).isAssimilateProvincesPlayer = 1;
            CFG.menuManager.updateInGame_TOP_All(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID());
         } else {
            CFG.game.getCiv(paramInt).isAssimilateProvincesPlayer = 0;
         }
         ...
         if (paramInt == this.getMenuElementsSize() - 2) {
            this.setVisible(false);
         }
      }
   }
