...
public class Menu_InGame_View_Population extends SliderMenu {
   ...
   public Menu_InGame_View_Population() {
      ...
      for (int i = 0; i < CFG.game.getCiv(this.iCivID).getNumOfProvinces(); i++) {
         if (CFG.FOG_OF_WAR != 2 || CFG.game.getPlayer(CFG.PLAYER_TURNID).getMetProvince(CFG.game.getCiv(this.iCivID).getProvinceID(i))) {
            ...
            totalPopulation += CFG.game.getProvince(CFG.game.getCiv(this.iCivID).getProvinceID(i)).getPopulationData().getPopulation();
         }
      }
      ...
      while (tempProvs.size() > 0) {
         ...
         for (int i2 = 1; i2 < tempProvs.size(); i2++) {
            if (CFG.game.getProvince(tempProvs.get(tBest)).getPopulationData().getPopulation()
               < CFG.game.getProvince(tempProvs.get(i2)).getPopulationData().getPopulation()) {
               tBest = i2;
            }
         }
      ...
      if (tempProvincesSorted.size() > 0) {
         menuElements.add(
            new Button_NS_Population(
               ...
               CFG.getNumberWithSpaces("" + CFG.game.getCiv(this.iCivID).countPopulation()),
               Images.population,
               ...
               public void buildElementHover() {
                  this.menuElementHover = CFG.game.getHover_PopulationOfCiv(Menu_InGame_View_Population.this.iCivID);
               }
            }
         );
         tY += menuElements.get(menuElements.size() - 1).getHeight();

         for (int var10 = 0; var10 < tempProvincesSorted.size(); var10++) {
            menuElements.add(
               new Button_View_Population(
                  var10,
                  (
                        CFG.game.getProvince((Integer)tempProvincesSorted.get(var10)).getName().length() > 0
                           ? CFG.game.getProvince((Integer)tempProvincesSorted.get(var10)).getName()
                           : CFG.game.getCiv(this.iCivID).getCivName()
                     )
                     + ": ",
                  (Integer)tempProvincesSorted.get(var10),
                  totalPopulation,
                  0,
                  tY,
                  CFG.CIV_INFO_MENU_WIDTH,
                  (CFG.SPECTATOR_MODE || CFG.game.isAlly(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(), this.iCivID))
                     && CFG.game.getCiv(this.iCivID).isInConstruction((Integer)tempProvincesSorted.get(var10), ConstructionType.LIBRARY) > 0
               ) {
                  @Override
                  public void actionElement(int iID) {
                     if (CFG.gameAction.getActiveTurnState() != Game_Action.TurnStates.INPUT_ORDERS
                        ...
                        || CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).isInConstruction(this.getCurrent(), ConstructionType.LIBRARY) > 0) {
                        ...
                        CFG.menuManager.rebuildInGame_BuildLibrary(this.getCurrent());
                     }
                  }
               }
            );
         ...
         new SliderMenuTitle(CFG.game.getCiv(this.iCivID).getCivName(), CFG.BUTTON_HEIGHT * 3 / 5, false, false) {
            ...
            public void draw(SpriteBatch oSB, int iTranslateX, int nPosX, int nPosY, int nWidth, boolean sliderMenuIsActive) {
               ImageManager.getImage(Images.new_game_top_edge_title)
                  .draw2(
                     oSB,
                     Menu_InGame_View_Population.this.getPosX() + iTranslateX,
                     Menu_InGame_View_Population.this.getPosY() - ImageManager.getImage(Images.new_game_top_edge_title).getHeight() - this.getHeight(),
                     Menu_InGame_View_Population.this.getWidth() + 2,
                     this.getHeight(),
                     true,
                     false
                  );
               oSB.setColor(new Color(CFG.COLOR_TEXT_POPULATION.r, CFG.COLOR_TEXT_POPULATION.g, CFG.COLOR_TEXT_POPULATION.b, 0.165F));
               ...
               oSB.setColor(new Color(CFG.COLOR_TEXT_POPULATION.r, CFG.COLOR_TEXT_POPULATION.g, CFG.COLOR_TEXT_POPULATION.b, 0.375F));
               ...
               ImageManager.getImage(Images.pix255_255_255)
                  .draw(
                     oSB,
                     Menu_InGame_View_Population.this.getPosX() + iTranslateX,
                     Menu_InGame_View_Population.this.getPosY() - ImageManager.getImage(Images.pix255_255_255).getHeight(),
                     Menu_InGame_View_Population.this.getWidth()
                  );
               ...
               ImageManager.getImage(Images.line_32_off1)
                  .draw(
                     oSB,
                     Menu_InGame_View_Population.this.getPosX() + iTranslateX,
                     Menu_InGame_View_Population.this.getPosY()
               ...
               ImageManager.getImage(Images.slider_gradient)
                  .draw(
                     ...
                     Menu_InGame_View_Population.this.getWidth() / 4,
                     1
                  );
               ImageManager.getImage(Images.slider_gradient)
                  .draw(
                     oSB,
                     Menu_InGame_View_Population.this.getPosX()
                        + Menu_InGame_View_Population.this.getWidth()
                        - Menu_InGame_View_Population.this.getWidth() / 4
                        + iTranslateX,
                     Menu_InGame_View_Population.this.getPosY()
               ...
               ImageManager.getImage(Images.population)
                  .draw(
                     ...
                     Menu_InGame_View_Population.this.getPosY() - this.getHeight() / 2 - ImageManager.getImage(Images.population).getHeight() / 2
                  );
