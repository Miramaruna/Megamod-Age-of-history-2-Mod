...
public class Menu_InGame_View_Goverments extends SliderMenu {
   public static final int ANIMATION_TIME = 175;
   public static long lTime = 0L;
   public static boolean hideAnimation = true;

   public Menu_InGame_View_Goverments() {
      int tempW = CFG.CIV_INFO_MENU_WIDTH;
      int tY = 0;
      ArrayList<MenuElement> menuElements = new ArrayList<>();
      ArrayList<Integer> tempGoverments = new ArrayList<>();
      ArrayList<Integer> tempGovermentsIDs = new ArrayList<>();
      ...
      try {
         ...
         for (int var11 = 1; var11 < CFG.game.getCivsSize(); var11++) {
            if (CFG.game.getCiv(var11).getNumOfProvinces() > 0 && (CFG.FOG_OF_WAR != 2 || CFG.game.getPlayer(CFG.PLAYER_TURNID).getMetCivilization(var11))) {
               tempGoverments.set(CFG.game.getCiv(var11).getIdeologyID(), tempGoverments.get(CFG.game.getCiv(var11).getIdeologyID()) + 1);
            }
         }
         ...
         while (tempGovermentsIDs.size() > 0) {
            ...
            for (int i2 = 1; i2 < tempGovermentsIDs.size(); i2++) {
               if (tempGoverments.get(tempGovermentsIDs.get(i2)) > tempGoverments.get(tempGovermentsIDs.get(tBest))) {
                  tBest = i2;
               }
            }
         ...
         for (int var12 = tSorted.size() - 1; var12 >= 0; var12--) {
            if (tempGoverments.get((Integer)tSorted.get(var12)) == 0) {
               tSorted.remove(var12);
            }
         }

         if (tSorted.size() > 0) {
            for (int var13 = 0; var13 < tSorted.size(); var13++) {
               menuElements.add(
                  new Button_NS_Goverments(
                     CFG.ideologiesManager.getIdeology((Integer)tSorted.get(var13)).getColor(),
                     CFG.ideologiesManager.getIdeology((Integer)tSorted.get(var13)).getName(),
                     (Integer)tSorted.get(var13),
                     CFG.langManager.get("Civilizations") + ": ",
                     CFG.getNumberWithSpaces("" + tempGoverments.get((Integer)tSorted.get(var13))),
                     Images.provinces,
      ...
      this.initMenu(
         new SliderMenuTitle(CFG.langManager.get("Governments"), CFG.BUTTON_HEIGHT * 3 / 5, false, false) {
            @Override
            public void draw(SpriteBatch oSB, int iTranslateX, int nPosX, int nPosY, int nWidth, boolean sliderMenuIsActive) {
               ImageManager.getImage(Images.new_game_top_edge_title)
                  .draw2(
                     oSB,
                     Menu_InGame_View_Goverments.this.getPosX() + iTranslateX,
                     Menu_InGame_View_Goverments.this.getPosY() - ImageManager.getImage(Images.new_game_top_edge_title).getHeight() - this.getHeight(),
                     Menu_InGame_View_Goverments.this.getWidth() + 2,
                     this.getHeight(),
                     true,
                     false
                  );
               ...
               ImageManager.getImage(Images.pix255_255_255)
                  .draw(
                     oSB,
                     Menu_InGame_View_Goverments.this.getPosX() + iTranslateX,
                     Menu_InGame_View_Goverments.this.getPosY() - ImageManager.getImage(Images.pix255_255_255).getHeight(),
                     Menu_InGame_View_Goverments.this.getWidth()
                  );
               ...
               ImageManager.getImage(Images.line_32_off1)
                  .draw(
                     oSB,
                     Menu_InGame_View_Goverments.this.getPosX() + iTranslateX,
                     Menu_InGame_View_Goverments.this.getPosY()
               ...
               ImageManager.getImage(Images.slider_gradient)
                  .draw(
                     ...
                     Menu_InGame_View_Goverments.this.getWidth() / 4,
                     1
                  );
               ImageManager.getImage(Images.slider_gradient)
                  .draw(
                     oSB,
                     Menu_InGame_View_Goverments.this.getPosX()
                        + Menu_InGame_View_Goverments.this.getWidth()
                        - Menu_InGame_View_Goverments.this.getWidth() / 4
                        + iTranslateX,
                     Menu_InGame_View_Goverments.this.getPosY()
