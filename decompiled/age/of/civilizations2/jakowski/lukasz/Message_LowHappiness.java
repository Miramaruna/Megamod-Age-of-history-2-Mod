...
public class Message_LowHappiness extends Message {
   public Message_LowHappiness(int fromCivID, int iValue) {
      super(fromCivID, iValue);
      this.messageType = Message_Type.LOW_HAPPINESS;
   ...
   public void onAccept(int iCivID) {
      if (CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID() == iCivID) {
         ...
         lMess.add(CFG.langManager.get("Happiness: " + CFG.game.getCiv(this.iFromCivID).getHappiness() + "%"));
   ...
   public MenuElement_Hover_v2 getHover() {
      ...
      nData.add(new MenuElement_Hover_v2_Element_Type_Text(CFG.langManager.get("PeopleAreUnhappy") + ": ", CFG.COLOR_TEXT_NUM_OF_PROVINCES));
      nData.add(new MenuElement_Hover_v2_Element_Type_Text("" + CFG.game.getCiv(this.iFromCivID).getHappiness() + "%", CFG.COLOR_TEXT_HAPPINESS_MIN));
      ...
      nData.add(new MenuElement_Hover_v2_Element_Type_Text(CFG.langManager.get("LowerTaxesAndIncreaseSpendingOnGoods"), CFG.COLOR_TEXT_MODIFIER_NEUTRAL));
      nElements.add(new MenuElement_Hover_v2_Element2(nData));
      nData.clear();
