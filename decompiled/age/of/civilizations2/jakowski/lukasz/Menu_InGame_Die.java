...
public class Menu_InGame_Die extends SliderMenu {
   public int iOnCivID = -1;

   public Menu_InGame_Die(int onCivID) {
      ...
      menuElements.add(
         new Button_Build_Building_Destroy(
            ...
            CFG.langManager.get("Destroy") + ": " + CFG.game.getCiv(this.iOnCivID).getCivName(),
            Images.skull,
            0,
            tY,
            CFG.BUTTON_WIDTH * 2,
            ...
            @Override
            public int getWidth() {
               return Menu_InGame_Die.this.getElementW() * 2;
            }
      ...
      menuElements.add(new Button_FlagActionSliderStyle(CFG.langManager.get("Cancel"), -1, 2 + CFG.PADDING, var7 = tY + CFG.PADDING, CFG.BUTTON_WIDTH, true) {
         @Override
         public int getWidth() {
            return Menu_InGame_Die.this.getElementW() - CFG.PADDING - CFG.PADDING / 2;
         }
      });
      menuElements.add(
         new Button_FlagActionSliderStyle(CFG.langManager.get("Destroy"), -1, 2, var7, CFG.BUTTON_WIDTH, true) {
            @Override
            public int getPosX() {
               return Menu_InGame_Die.this.getElementW() + CFG.PADDING / 2;
            }
      ...
      this.initMenu(
         new SliderMenuTitle(CFG.langManager.get("Die"), CFG.BUTTON_HEIGHT * 3 / 5, true, true) {
            @Override
            public void draw(SpriteBatch oSB, int iTranslateX, int nPosX, int nPosY, int nWidth, boolean sliderMenuIsActive) {
               ...
               CFG.game
                  ...
                  .draw(
                     oSB,
                     Menu_InGame_Die.this.getPosX() + CFG.PADDING * 2 + iTranslateX,
                     Menu_InGame_Die.this.getPosY()
                        - this.getHeight() / 2
               ...
               ImageManager.getImage(Images.flag_rect)
                  .draw(
                     oSB,
                     Menu_InGame_Die.this.getPosX() + CFG.PADDING * 2 + iTranslateX,
                     Menu_InGame_Die.this.getPosY() - this.getHeight() / 2 - CFG.CIV_FLAG_HEIGHT / 2
                  );
               CFG.fontMain.getData().setScale(0.8F);
               CFG.drawText(
                  oSB,
   ...
   public final void actionElement(int iID) {
      if (iID != this.getMenuElementsSize() - 1) {
         ...
         int NumOfProvs = CFG.game.getCiv(tCiv).getNumOfProvinces();
         int[] ProvsID = new int[NumOfProvs];

         for (int k = 0; k < NumOfProvs; k++) {
            CFG.game.getProvince(CFG.game.getCiv(tCiv).getProvinceID(k)).setTrueOwnerOfProvince(0);
            ProvsID[k] = CFG.game.getCiv(tCiv).getProvinceID(k);
         }

         for (int l = 0; l < NumOfProvs; l++) {
            CFG.game.getProvince(ProvsID[l]).setCivID(0, false);
         }
