...
public class AI_Build_Invest extends AI_Build {
   public List<Integer> lProvincesToInvest = new ArrayList<>();
   public float fReserve = 1.0F;
   public int iMaxPop = 1;

   public AI_Build_Invest(int nCivID, long nMoney) {
      super(nCivID, nMoney);
      this.fReserve = 1.0F
         - CFG.game.getCiv(nCivID).civGameData.civPersonality.BUILD_RESRVE_RAND / 100.0F
         + CFG.oR.nextInt(CFG.game.getCiv(nCivID).civGameData.civPersonality.BUILD_RESRVE_RAND) / 100.0F;
      ...
      try {
         for (int i = 0; i < CFG.game.getCiv(nCivID).getNumOfProvinces(); i++) {
            if (!CFG.game.getProvince(CFG.game.getCiv(nCivID).getProvinceID(i)).isOccupied()
               ...
               && !CFG.game.getCiv(nCivID).isInvestOrganized(CFG.game.getCiv(nCivID).getProvinceID(i))) {
   ...
   public boolean build(int nCivID, int iteration, boolean out) {
      ...
      if (iBestProvinceID >= 0) {
         int maxInvestmentGold = (int)Math.min(this.getMoney(nCivID), (long)DiplomacyManager.invest_MaxEconomy_Gold(iBestProvinceID, nCivID));
         int minNumOfInvests = (int)Math.min(
            Math.floor(CFG.game.getCiv(nCivID).getMovePoints() / 12), (double)(CFG.game.getCiv(nCivID).getNumOfProvinces() / 10)
         );
         if (iteration == 0
            && minNumOfInvests > 1
            ...
            maxInvestmentGold = (int)(
               maxInvestmentGold
                  * (1.0F - CFG.oR.nextInt(Math.max(2, CFG.game.getCiv(nCivID).civGameData.civPersonality.BUILD_INVEST_SECOND_INVEST_MAX_PERC)) / 100.0F)
            );
         }

         if (DiplomacyManager.invest(iBestProvinceID, nCivID, maxInvestmentGold)) {
            out = true;
            if (this.getMoney(nCivID) > 10L && 12 <= CFG.game.getCiv(nCivID).getMovePoints()) {
               int tSize = 0;

               for (int ix = this.lProvincesToInvest.size() - 1; ix >= 0; ix--) {
                  if (this.lProvincesToInvest.get(ix) == iBestProvinceID) {
                     this.lProvincesToInvest.remove(ix);
                  } else {
                     tSize++;
                  }
               }
      ...
      return (
            (float)CFG.game.getProvince(nProvinceID).getPopulationData().getPopulation()
               ...
               + (
                     1.0F
                        - Math.min(
                           1.0F, (float)CFG.game.getProvince(nProvinceID).getPopulationData().getPopulation() / CFG.game.getProvince(nProvinceID).getEconomy()
                        )
                  )
