...
public class TechnologyManager {
   ...
   public static final int POINTS_PER_LEVEL_BASE = 500;
   public static final float POINTS_PER_LEVEL_PROVINCES = 52.45487F;
   public static float fAverageTechLevel = 0.5F;
   public static final float MIGRATE_RESEARCH_PROGRESS = 0.1F;
   public static final float MIGRATE_RESEARCH_PROGRESS_NOT_CAPITAL = 0.05F;
   ...
   public static final void updateAverageTechLevel() {
      ...
      for (int i = 1; i < CFG.game.getCivsSize(); i++) {
         if (CFG.game.getCiv(i).getNumOfProvinces() > 0) {
            fAverageTechLevel = fAverageTechLevel + CFG.game.getCiv(i).getTechnologyLevel();
            numOfCivs++;
         }
      }

      fAverageTechLevel /= numOfCivs;
   }

   public static final int getResearch_NextLevel(int nCivID) {
      return (int)(
         Math.pow(
               500.0F
                  + 52.45487F * CFG.game.getCiv(nCivID).getNumOfProvinces()
            ...
            * CFG.game.getCiv(nCivID).getTechnologyLevel()
            / fAverageTechLevel
            * CFG.ideologiesManager.getIdeology(CFG.game.getCiv(nCivID).getIdeologyID()).RESEARCH_COST
      );
   }
      ...
      for (int i = 1; i < CFG.game.getCivsSize(); i++) {
         if (CFG.ideologiesManager.getIdeology(CFG.game.getCiv(i).getIdeologyID()).CAN_BECOME_CIVILIZED >= 0
            ...
            CFG.game
               .getCiv(i)
               .addResearchProgress(
                  getResearch_NextLevel(i)
                     * (0.0011254F + CFG.oR.nextInt(545) / 10000.0F)
                     * (
                        0.325F
                           + 0.675F
                              * (
   ...
   public static final void updateResearch(int nCivID) {
      updateReseatchOfCiv(nCivID, 0);
      updateAverageTechLevel();
   }

   public static final void updateReseatchOfCiv(int i, int iterration) {
      if (CFG.game.getCiv(i).getNumOfProvinces() > 0 && CFG.game.getCiv(i).getResearchProgress() > getResearch_NextLevel(i)) {
         ...
         int tID;
         if (CFG.game.getCiv(i).getControlledByPlayer() && (tID = CFG.game.getPlayerID_ByCivID(i)) >= 0 && tID < CFG.game.getPlayersSize()) {
                  ...
                  CFG.game
                     ...
                     .addResearchProgress(
                        (float)CFG.game.getProvince(CFG.game.getCiv(i).getProvinceID(j)).getPopulationData().getPopulation()
                           / BuildingsManager.getLibrary_ResearchPerPopulation(CFG.game.getProvince(CFG.game.getCiv(i).getProvinceID(j)).getLevelOfLibrary())
   ...
   public static final void updateCivs_ResearchProgress_Migrate(int nCivID, int nProvinceID) {
      if (CFG.game.getCiv(nCivID).getNumOfProvinces() > 0) {
         CFG.game.getCiv(nCivID).addResearchProgress(getResearch_NextLevel(nCivID) * (CFG.game.getProvince(nProvinceID).getIsCapital() ? 0.1F : 0.05F));
      }
