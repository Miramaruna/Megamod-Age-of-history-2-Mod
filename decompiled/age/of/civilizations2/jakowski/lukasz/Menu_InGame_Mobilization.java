...
class Menu_InGame_Mobilization
extends SliderMenu {
    private int iOnCivID = -1;

    public Menu_InGame_Mobilization(int onCivID) {
        ArrayList<MenuElement> menuElements = new ArrayList<MenuElement>();
        this.iOnCivID = CFG.game.getCiv(onCivID).getPuppetOfCivID();
        int tempWidth = CFG.CIV_INFO_MENU_WIDTH * 2;
        int tY = 0;
        menuElements.add(new Button_NS_Opinion(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(), CFG.game.getCiv(CFG.getActiveCivInfo()).getPuppetOfCivID(), Images.diplo_army, 0, 2, 2, tY, CFG.BUTTON_WIDTH * 2){

            @Override
            public int getWidth() {
                return Menu_InGame_Mobilization.this.getElementW() * 2;
            }
        });
        ...
        menuElements.add(new Button_FlagActionSliderStyle(CFG.langManager.get("Cancel"), -1, 2 + CFG.PADDING, tY += CFG.PADDING, CFG.BUTTON_WIDTH, true){
            ...
            @Override
            public int getWidth() {
                return Menu_InGame_Mobilization.this.getElementW() - CFG.PADDING - CFG.PADDING / 2;
            }
        });
        menuElements.add(new Button_FlagActionSliderStyle(CFG.langManager.get("Confirm"), -1, 2, tY, CFG.BUTTON_WIDTH, true){
            ...
            @Override
            public int getPosX() {
                return Menu_InGame_Mobilization.this.getElementW() + CFG.PADDING / 2;
            }
            ...
            public void buildElementHover() {
                ...
                nData.add(new MenuElement_Hover_v2_Element_Type_Flag(Menu_InGame_Mobilization.this.iOnCivID, CFG.PADDING, CFG.PADDING));
                nData.add(new MenuElement_Hover_v2_Element_Type_Text(CFG.game.getCiv(Menu_InGame_Mobilization.this.iOnCivID).getCivName()));
        ...
        this.initMenu(new SliderMenuTitle(CFG.langManager.get("DistributeArmyToBorders"), CFG.BUTTON_HEIGHT * 3 / 5, true, true){
   ...
   public final void actionElement(int paramInt) {
      if (paramInt == this.getMenuElementsSize() - 1) {
         ...
         CFG.oAI.getAI_Style(CFG.game.getCiv(civID).getAI_Style()).regroupArmy_AtPeace(civID);
