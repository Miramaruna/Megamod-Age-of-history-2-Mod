...
public class Menu_InGame_DisabledAI extends SliderMenu {
   public int iOnCivID = -1;
   String str;

   public Menu_InGame_DisabledAI(int onCivID) {
      ArrayList<MenuElement> menuElements = new ArrayList<>();
      this.iOnCivID = onCivID;
      int tempWidth = CFG.CIV_INFO_MENU_WIDTH * 2;
      int tY = 0;
      this.str = CFG.game.getCiv(onCivID).disabledAI ? "enabledAI" : "disableAI";
      menuElements.add(
         new Button_Build_Building_Destroy(
            CFG.langManager.get(this.str),
            CFG.langManager.get(this.str) + ": " + CFG.game.getCiv(this.iOnCivID).getCivName(),
            Images.dice,
            0,
            tY,
            CFG.BUTTON_WIDTH * 2,
            ...
            @Override
            public int getWidth() {
               return Menu_InGame_DisabledAI.this.getElementW() * 2;
            }
      ...
      menuElements.add(new Button_FlagActionSliderStyle(CFG.langManager.get("Cancel"), -1, 2 + CFG.PADDING, var7 = tY + CFG.PADDING, CFG.BUTTON_WIDTH, true) {
         @Override
         public int getWidth() {
            return Menu_InGame_DisabledAI.this.getElementW() - CFG.PADDING - CFG.PADDING / 2;
         }
      });
      menuElements.add(
         new Button_FlagActionSliderStyle(CFG.langManager.get("Confirm"), -1, 2, var7, CFG.BUTTON_WIDTH, true) {
            @Override
            public int getPosX() {
               return Menu_InGame_DisabledAI.this.getElementW() + CFG.PADDING / 2;
            }
            ...
            public void buildElementHover() {
               ...
               nData.add(new MenuElement_Hover_v2_Element_Type_Text(CFG.langManager.get("Confirm") + "!", CFG.COLOR_TEXT_MODIFIER_NEGATIVE2));
         ...
         new SliderMenuTitle(CFG.langManager.get(this.str), CFG.BUTTON_HEIGHT * 3 / 5, true, true) {
            ...
            public void draw(SpriteBatch oSB, int iTranslateX, int nPosX, int nPosY, int nWidth, boolean sliderMenuIsActive) {
               ...
               CFG.game
                  ...
                  .draw(
                     oSB,
                     Menu_InGame_DisabledAI.this.getPosX() + CFG.PADDING * 2 + iTranslateX,
                     Menu_InGame_DisabledAI.this.getPosY()
                        - this.getHeight() / 2
               ...
               ImageManager.getImage(Images.flag_rect)
                  .draw(
                     oSB,
                     Menu_InGame_DisabledAI.this.getPosX() + CFG.PADDING * 2 + iTranslateX,
                     Menu_InGame_DisabledAI.this.getPosY() - this.getHeight() / 2 - CFG.CIV_FLAG_HEIGHT / 2
                  );
               CFG.fontMain.getData().setScale(0.8F);
               CFG.drawText(
                  oSB,
   ...
   public final void actionElement(int iID) {
      if (iID == this.getMenuElementsSize() - 1) {
         ...
         int tCiv = CFG.game.getProvince(CFG.game.getActiveProvinceID()).getCivID();
         if (CFG.game.getCiv(tCiv).disabledAI) {
            CFG.game.getCiv(tCiv).disabledAI = !CFG.game.getCiv(tCiv).disabledAI;
         } else {
            CFG.game.getCiv(tCiv).disabledAI = true;
         }
