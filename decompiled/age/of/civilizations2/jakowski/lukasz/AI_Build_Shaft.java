...
class AI_Build_Shaft extends AI_Build {
   private List<Integer> lBuildCost = new ArrayList<>();

   // $VF: Duplicated exception handlers to handle obfuscated exceptions
   protected AI_Build_Shaft(int var1, long var2) {
      super(var1, var2);
      int var4 = 0;
      ...
      while (true) {
         label67: {
            try {
               if (var4 < Shaft.getShaft_MaxLevel()) {
                  this.lBuildCost.add(Shaft.getShaft_BuildCost(var4 + 1, CFG.game.getCiv(var1).getProvinceID(0)));
               ...
               label47: {
                  ...
                  try {
                     ...
                     if (CFG.game.getProvince(CFG.game.getCiv(var1).getProvinceID(var4)).isOccupied()
                        ...
                        || !Shaft.canBuildShaft(CFG.game.getCiv(var1).getProvinceID(var4))) {
                        break label47;
                     }
   ...
   protected boolean build(int var1, int var2, boolean var3) {
      ...
      if (var2 >= 0) {
         var9 = var3;
         if (Shaft.constructShaft(var2, var1)) {
            var9 = true;
         }
      }
   ...
   protected float getProvinceBuildScore(int var1, int var2, int var3) {
      return CFG.game.getProvince(var2).getPopulationData().getPopulation()
         ...
         * (
            1.0F
               - CFG.game.getCiv(var1).civGameData.civPersonality.BUILD_SHAFT_RECRUITABLE_SCORE
               + CFG.game.getCiv(var1).civGameData.civPersonality.BUILD_SHAFT_RECRUITABLE_SCORE * CFG.gameAction.getRecruitableArmy(var2, var1) / var3
         )
