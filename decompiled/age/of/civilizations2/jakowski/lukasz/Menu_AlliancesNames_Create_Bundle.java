...
public class Menu_AlliancesNames_Create_Bundle extends SliderMenu {
   ...
   public Menu_AlliancesNames_Create_Bundle() {
      ...
      for (int i = 0; i < CFG.editorAlliancesNames_GameData.getBundle(CFG.EDIT_ALLIANCE_NAMES_BUNDLE_ID).getWordsSize(); i++) {
         menuElements.add(
            new Button_Menu(
               CFG.editorAlliancesNames_GameData.getBundle(CFG.EDIT_ALLIANCE_NAMES_BUNDLE_ID).getWord(i),
         ...
         menuElements.add(
            new Button_Menu_Remove(
               CFG.GAME_WIDTH - CFG.BUTTON_WIDTH * 2, CFG.BUTTON_HEIGHT * (i + 1) + CFG.PADDING * (i + 2), CFG.BUTTON_WIDTH * 2, CFG.BUTTON_HEIGHT, i > 0
            )
         );
      }
      ...
      switch (iID) {
         ...
         default:
            if (iID % 2 == 0) {
               ...
               for (int i = 0;
                  i * 2 + 2 < this.getMenuElementsSize() && i < CFG.editorAlliancesNames_GameData.getBundle(CFG.EDIT_ALLIANCE_NAMES_BUNDLE_ID).getWordsSize();
                  i++
               ) {
   ...
   public final void onBackPressed() {
      ...
      for (int i = 0; i < CFG.editorAlliancesNames_GameData.getBundle(CFG.EDIT_ALLIANCE_NAMES_BUNDLE_ID).getWordsSize(); i++) {
         if (CFG.editorAlliancesNames_GameData.getBundle(CFG.EDIT_ALLIANCE_NAMES_BUNDLE_ID).getWord(i).equals("")) {
