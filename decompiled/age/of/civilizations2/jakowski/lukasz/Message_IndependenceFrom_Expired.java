...
public class Message_IndependenceFrom_Expired extends Message {
   public Message_IndependenceFrom_Expired(int fromCivID) {
      super(fromCivID, 0);
      this.messageType = Message_Type.GUARANTEE_FROM_EXPIRED;
   ...
   public void onAccept(int iCivID) {
      if (CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID() == iCivID) {
         ...
         lMess.add(CFG.langManager.get("WeNoLongerGuaranteeIndependenceOf") + ": " + CFG.game.getCiv(this.iFromCivID).getCivName());
   ...
   public MenuElement_Hover_v2 getHover() {
      ...
      nData.add(new MenuElement_Hover_v2_Element_Type_Text(CFG.langManager.get("WeNoLongerGuaranteeIndependenceOf") + ": ", CFG.COLOR_TEXT_NUM_OF_PROVINCES));
