...
public class Message_HRE_Elections_NewEmperor extends Message {
   public Message_HRE_Elections_NewEmperor(int byCivID) {
      super(byCivID, 0);
      this.messageType = Message_Type.HRE_NEW_EMPEROR;
   ...
   public void onAccept(int iCivID) {
      if (CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID() == iCivID) {
         CFG.toast
            .setInView(CFG.langManager.get("NextElections") + ": " + Game_Calendar.getDate_ByTurnID(Game_Calendar.TURN_ID + 1), CFG.COLOR_TEXT_NUM_OF_PROVINCES);
      }
   }
   ...
   public MenuElement_Hover_v2 getHover() {
      ...
      nData.add(new MenuElement_Hover_v2_Element_Type_Text(CFG.langManager.get("Emperor") + ": "));
      ...
      nData.add(new MenuElement_Hover_v2_Element_Type_Image(Images.hre_crown, CFG.PADDING, 0));
      nElements.add(new MenuElement_Hover_v2_Element2(nData));
      nData.clear();
