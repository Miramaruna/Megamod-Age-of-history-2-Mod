...
public class Menu_InGame_Destroy_Farm extends SliderMenu {
   ...
   public Menu_InGame_Destroy_Farm(int nProvinceID) {
      ...
      menuElements.add(
         new Button_Build_Building_Destroy(
            ...
            CFG.langManager.get("Destroy") + ": " + CFG.langManager.get(BuildingsManager.getFarm_Name(CFG.game.getProvince(this.iProvinceID).getLevelOfFarm())),
            Images.b_farm,
            0,
            tY,
            CFG.BUTTON_WIDTH * 2,
            ...
            @Override
            public int getWidth() {
               return Menu_InGame_Destroy_Farm.this.getElementW() * 2;
            }
            ...
            public void buildElementHover() {
               ...
               nData.add(
                  new MenuElement_Hover_v2_Element_Type_Text(
                     CFG.langManager.get(BuildingsManager.getFarm_Name(CFG.game.getProvince(Menu_InGame_Destroy_Farm.this.iProvinceID).getLevelOfFarm()))
                        + ": ",
               ...
               nData.add(
                  new MenuElement_Hover_v2_Element_Type_Text(
                     CFG.game.getProvince(Menu_InGame_Destroy_Farm.this.iProvinceID).getName().length() > 0
                        ? CFG.game.getProvince(Menu_InGame_Destroy_Farm.this.iProvinceID).getName()
                        : CFG.langManager.get("Province")
                  )
               );
               nData.add(new MenuElement_Hover_v2_Element_Type_Flag(CFG.game.getProvince(Menu_InGame_Destroy_Farm.this.iProvinceID).getCivID(), CFG.PADDING, 0));
               ...
               nData.add(
                  new MenuElement_Hover_v2_Element_Type_Text(
                     "+"
                        + (int)(
                           BuildingsManager.getFarm_GrowthRateBonus(CFG.game.getProvince(Menu_InGame_Destroy_Farm.this.iProvinceID).getLevelOfFarm()) * 100.0F
                        )
                        + "%",
      ...
      menuElements.add(new Button_FlagActionSliderStyle(CFG.langManager.get("Cancel"), -1, 2 + CFG.PADDING, var7 = tY + CFG.PADDING, CFG.BUTTON_WIDTH, true) {
         @Override
         public int getWidth() {
            return Menu_InGame_Destroy_Farm.this.getElementW() - CFG.PADDING - CFG.PADDING / 2;
         }
      });
      menuElements.add(
         new Button_FlagActionSliderStyle(CFG.langManager.get("Destroy"), -1, 2, var7, CFG.BUTTON_WIDTH, true) {
            @Override
            public int getPosX() {
               return Menu_InGame_Destroy_Farm.this.getElementW() + CFG.PADDING / 2;
            }
      ...
      this.initMenu(
         new SliderMenuTitle(
            CFG.langManager.get(BuildingsManager.getFarm_Name(CFG.game.getProvince(this.iProvinceID).getLevelOfFarm())), CFG.BUTTON_HEIGHT * 3 / 5, true, true
         ) {
            @Override
            public void draw(SpriteBatch oSB, int iTranslateX, int nPosX, int nPosY, int nWidth, boolean sliderMenuIsActive) {
               ...
               CFG.game
                  ...
                  .draw(
                     oSB,
                     Menu_InGame_Destroy_Farm.this.getPosX() + CFG.PADDING * 2 + iTranslateX,
                     Menu_InGame_Destroy_Farm.this.getPosY()
                        - this.getHeight() / 2
               ...
               ImageManager.getImage(Images.flag_rect)
                  .draw(
                     oSB,
                     Menu_InGame_Destroy_Farm.this.getPosX() + CFG.PADDING * 2 + iTranslateX,
                     Menu_InGame_Destroy_Farm.this.getPosY() - this.getHeight() / 2 - CFG.CIV_FLAG_HEIGHT / 2
                  );
               CFG.fontMain.getData().setScale(0.8F);
               CFG.drawText(
                  oSB,
   ...
   public final void actionElement(int iID) {
      if (iID == this.getMenuElementsSize() - 1) {
         if (BuildingsManager.destroyFarm(this.iProvinceID, CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID())) {
