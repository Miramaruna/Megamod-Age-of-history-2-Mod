...
public class Button_Diplomacy_ImprovingRelations extends Button {
   public static final float FONT_SIZE = 0.6F;
   public int iCivID = 0;
   public boolean row = false;
   public String sImprovingRelations;
   public int iImprovingRelationsWidth;
   public String sImprovingRelations2;

   public Button_Diplomacy_ImprovingRelations(
         ...
         ImageManager.getImage(Images.icon_check_true)
            .draw(
               ...
               this.getPosX()
                  + this.getWidth()
                  - CFG.PADDING * 2
                  - (int)(ImageManager.getImage(Images.icon_check_true).getWidth() * this.getImageScale())
                  + iTranslateX,
               this.getPosY()
                  + (int)((this.getHeight() - ImageManager.getImage(Images.icon_check_true).getHeight() * this.getImageScale()) / 2.0F)
                  - ImageManager.getImage(Images.icon_check_true).getHeight()
                  + iTranslateY,
               (int)(ImageManager.getImage(Images.icon_check_true).getWidth() * this.getImageScale()),
               (int)(ImageManager.getImage(Images.icon_check_true).getHeight() * this.getImageScale())
            );
      } else {
         ImageManager.getImage(Images.icon_check_false)
            .draw(
               ...
               this.getPosX()
                  + this.getWidth()
                  - CFG.PADDING * 2
                  - (int)(ImageManager.getImage(Images.icon_check_false).getWidth() * this.getImageScale())
                  + iTranslateX,
               this.getPosY()
                  + (int)((this.getHeight() - ImageManager.getImage(Images.icon_check_false).getHeight() * this.getImageScale()) / 2.0F)
                  - ImageManager.getImage(Images.icon_check_false).getHeight()
                  + iTranslateY,
               (int)(ImageManager.getImage(Images.icon_check_false).getWidth() * this.getImageScale()),
               (int)(ImageManager.getImage(Images.icon_check_false).getHeight() * this.getImageScale())
            );
      }
   }
      ...
      try {
         ...
         nData.add(
            new MenuElement_Hover_v2_Element_Type_Color(
               new Color(
                  CFG.game.getCiv(this.iCivID).getR() / 255.0F, CFG.game.getCiv(this.iCivID).getG() / 255, CFG.game.getCiv(this.iCivID).getB() / 255.0F, 1.0F
               )
            )
         );
         ...
         nData.add(
            new MenuElement_Hover_v2_Element_Type_Text(
               "" + (int)(CFG.game.getCivRelation_OfCivB(this.iCivID, CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()) * 10.0F) / 10.0F,
         ...
         nData.add(
            new MenuElement_Hover_v2_Element_Type_Text(
               "+" + (int)(DiplomacyManager.getImproveRelation(CFG.game.getActiveCivID(), this.iCivID) * 100.0F) / 100.0F, CFG.COLOR_TEXT_MODIFIER_POSITIVE
            )
         );
   ...
   public final float getImageScale() {
      return Math.min((float)CFG.TEXT_HEIGHT / ImageManager.getImage(Images.icon_check_false).getHeight(), 1.0F);
   }
