...
class Menu_InGame_Spy
extends SliderMenu {
    private int iOnCivID = -1;

    public Menu_InGame_Spy() {
    ...
    public Menu_InGame_Spy(final int onCivID) {
        ...
        menuElements.add(new Button_Diplomacy_War_Cost(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(), CFG.getActiveCivInfo(), 2, tY, CFG.BUTTON_WIDTH * 2){
            ...
            @Override
            public int getWidth() {
                return Menu_InGame_Spy.this.getElementW() * 2;
            }
        });
        menuElements.add(new Button_Diplomacy_Demand(CFG.langManager.get("learnarmysize"), CFG.getActiveCivInfo(), 2, tY += ((MenuElement)menuElements.get(menuElements.size() - 1)).getHeight(), CFG.BUTTON_WIDTH * 2){
        ...
        if (CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID() != CFG.game.getCiv(onCivID).getPuppetOfCivID()) {
            menuElements.add(new Button_Diplomacy_Demand(CFG.langManager.get("changegoverment"), CFG.getActiveCivInfo(), 2, tY, CFG.BUTTON_WIDTH * 2){
        ...
        menuElements.add(new Button_Diplomacy_Demand(CFG.langManager.get("supportrebellionsinprovinces") + ": ", CFG.getActiveCivInfo(), 2, tY, CFG.BUTTON_WIDTH * 2){
        ...
        for (int i = 0; i < CFG.game.getCiv((int)onCivID).civGameData.lVassals.size(); ++i) {
            menuElements.add(new Button_Diplomacy_Demand(CFG.langManager.get("SupportLiberationOfVassal") + ": " + CFG.game.getCiv(CFG.game.getCiv((int)onCivID).civGameData.lVassals.get((int)i).iCivID).getCivName(), CFG.game.getCiv((int)onCivID).civGameData.lVassals.get((int)i).iCivID, 2, tY, CFG.BUTTON_WIDTH * 2){
        ...
        menuElements.add(new Slider_CrossTheBorder(CFG.langManager.get("NumberOfSpiesX", 0), CFG.PADDING * 2, ((MenuElement)menuElements.get(menuElements.size() - 1)).getPosY() + ((MenuElement)menuElements.get(menuElements.size() - 1)).getHeight() + CFG.PADDING, tempWidth - CFG.PADDING * 3 - CFG.BUTTON_WIDTH, CFG.TEXT_HEIGHT + CFG.PADDING * 2 + CFG.PADDING * 4, 0, CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getNumOfProvinces(), 0, 0.65f){

            @Override
            public int getWidth() {
                return Menu_InGame_Spy.this.getElementW() * 2 - CFG.PADDING * 4;
            }
        ...
        menuElements.add(new Button_FlagActionSliderStyle(CFG.langManager.get("Cancel"), -1, 2 + CFG.PADDING, tY += CFG.PADDING, CFG.BUTTON_WIDTH, true){
            ...
            @Override
            public int getWidth() {
                return Menu_InGame_Spy.this.getElementW() - CFG.PADDING - CFG.PADDING / 2;
            }
        });
        menuElements.add(new Button_FlagActionSliderStyle(CFG.langManager.get("SendSpy"), -1, 2, tY, CFG.BUTTON_WIDTH, true){

            @Override
            public int getPosX() {
                return Menu_InGame_Spy.this.getElementW() + CFG.PADDING / 2;
            }
            ...
            public void buildElementHover() {
                ...
                if (Game_Calendar.TURN_ID > 4) {
                    ...
                    nData.add(new MenuElement_Hover_v2_Element_Type_Flag(Menu_InGame_Spy.this.iOnCivID, CFG.PADDING, CFG.PADDING));
                    nData.add(new MenuElement_Hover_v2_Element_Type_Text(CFG.game.getCiv(Menu_InGame_Spy.this.iOnCivID).getCivName()));
                    nElements.add(new MenuElement_Hover_v2_Element2(nData));
                    nData.clear();
                    if (CFG.ultimatum.canBeSend()) {
                        ...
                        nData.add(new MenuElement_Hover_v2_Element_Type_Text(CFG.langManager.get("NumberOfSpiesX", Menu_InGame_Spy.this.getMenuElement(Menu_InGame_Spy.this.getMenuElementsSize() - 3).getCurrent()), CFG.COLOR_TEXT_MODIFIER_NEGATIVE2));
            ...
            public boolean getClickable() {
                return CFG.ultimatum.canBeSend() && Menu_InGame_Spy.this.getMenuElement(Menu_InGame_Spy.this.getMenuElementsSize() - 3).getCurrent() > 0 && CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getDiplomacyPoints() >= 24;
            }
        ...
        int tempMenuPosY = ImageManager.getImage(Images.top_left2).getHeight() + CFG.PADDING * 2 + CFG.BUTTON_HEIGHT * 3 / 5;
        this.initMenu(new SliderMenuTitle(CFG.langManager.get("SelectMission"), CFG.BUTTON_HEIGHT * 3 / 5, true, true){
    ...
    public final void actionElement(int iID) {
        if (iID == this.getMenuElementsSize() - 1) {
            if (DiplomacyManager.sendSpy(this.iOnCivID, CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(), CFG.ultimatum, this.getMenuElement(this.getMenuElementsSize() - 3).getCurrent())) {
