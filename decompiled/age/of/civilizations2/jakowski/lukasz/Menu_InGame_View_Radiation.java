...
public class Menu_InGame_View_Radiation extends SliderMenu {
   public static final int ANIMATION_TIME = 175;
   public static long lTime = 0L;
   public static boolean hideAnimation = true;
   public int iCivID = 0;

   public Menu_InGame_View_Radiation() {
      ...
      for (int i = 0; i < CFG.game.getCiv(this.iCivID).getNumOfProvinces(); i++) {
         if (CFG.FOG_OF_WAR != 2 || CFG.game.getPlayer(CFG.PLAYER_TURNID).getMetProvince(CFG.game.getCiv(this.iCivID).getProvinceID(i))) {
            ...
            totalPopulation = (int)(totalPopulation + CFG.game.getProvince(CFG.game.getCiv(this.iCivID).getProvinceID(i)).getZiverts());
         }
      }
      ...
      while (tempProvs.size() > 0) {
         ...
         for (int i2 = 1; i2 < tempProvs.size(); i2++) {
            if (!(CFG.game.getProvince(tempProvs.get(tBest)).getZiverts() >= CFG.game.getProvince(tempProvs.get(i2)).getZiverts())) {
               tBest = i2;
            }
         }
      ...
      if (tempProvincesSorted.size() > 0) {
         menuElements.add(
            new Button_NS_Population(
               ...
               this.iCivID,
               CFG.langManager.get("Ziverts") + ": ",
               CFG.getNumberWithSpaces("" + totalPopulation / CFG.game.getCiv(this.iCivID).getNumOfProvinces()),
               Images.nuclear_icon,
         ...
         for (int var10 = 0; var10 < tempProvincesSorted.size(); var10++) {
            menuElements.add(
               new Button_View_Radiation(
                  ...
                  totalPopulation / CFG.game.getCiv(this.iCivID).getNumOfProvinces(),
                  0,
                  tY,
                  CFG.CIV_INFO_MENU_WIDTH,
                  !CFG.SPECTATOR_MODE && !CFG.game.isAlly(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(), this.iCivID)
                     ? false
                     : CFG.game.getCiv(this.iCivID).isInvestOrganized((Integer)tempProvincesSorted.get(var10))
               ) {
                  @Override
         ...
         new SliderMenuTitle(CFG.game.getCiv(this.iCivID).getCivName(), CFG.BUTTON_HEIGHT * 3 / 5, false, false) {
            ...
            public void draw(SpriteBatch oSB, int iTranslateX, int nPosX, int nPosY, int nWidth, boolean sliderMenuIsActive) {
               ImageManager.getImage(Images.new_game_top_edge_title)
                  .draw2(
                     oSB,
                     Menu_InGame_View_Radiation.this.getPosX() + iTranslateX,
                     Menu_InGame_View_Radiation.this.getPosY() - ImageManager.getImage(Images.new_game_top_edge_title).getHeight() - this.getHeight(),
                     Menu_InGame_View_Radiation.this.getWidth() + 2,
                     this.getHeight(),
                     true,
                     false
                  );
               oSB.setColor(
                  new Color(
                     CFG.game.getCiv(Menu_InGame_View_Radiation.this.iCivID).getR() / 255.0F,
                     CFG.game.getCiv(Menu_InGame_View_Radiation.this.iCivID).getG() / 255.0F,
                     CFG.game.getCiv(Menu_InGame_View_Radiation.this.iCivID).getB() / 255.0F,
                     0.165F
                  )
               );
               ...
               ImageManager.getImage(Images.pix255_255_255)
                  .draw(
                     oSB,
                     Menu_InGame_View_Radiation.this.getPosX() + iTranslateX,
                     Menu_InGame_View_Radiation.this.getPosY() - ImageManager.getImage(Images.pix255_255_255).getHeight(),
                     Menu_InGame_View_Radiation.this.getWidth()
                  );
               ...
               ImageManager.getImage(Images.line_32_off1)
                  .draw(
                     oSB,
                     Menu_InGame_View_Radiation.this.getPosX() + iTranslateX,
                     Menu_InGame_View_Radiation.this.getPosY()
               ...
               ImageManager.getImage(Images.slider_gradient)
                  .draw(
                     ...
                     Menu_InGame_View_Radiation.this.getWidth() / 4,
                     1
                  );
               ImageManager.getImage(Images.slider_gradient)
                  .draw(
                     oSB,
                     Menu_InGame_View_Radiation.this.getPosX()
                        + Menu_InGame_View_Radiation.this.getWidth()
                        - Menu_InGame_View_Radiation.this.getWidth() / 4
                        + iTranslateX,
                     Menu_InGame_View_Radiation.this.getPosY()
               ...
               ImageManager.getImage(Images.nuclear_icon)
                  .draw(
                     ...
                     Menu_InGame_View_Radiation.this.getPosY() - this.getHeight() / 2 - ImageManager.getImage(Images.nuclear_icon).getHeight() / 2
                  );
