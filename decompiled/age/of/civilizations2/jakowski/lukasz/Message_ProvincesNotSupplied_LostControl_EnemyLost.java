...
public class Message_ProvincesNotSupplied_LostControl_EnemyLost extends Message {
   public Message_ProvincesNotSupplied_LostControl_EnemyLost(int fromCivID, int iValue) {
      super(fromCivID, iValue);
      this.messageType = Message_Type.PROVINCES_NOT_SUPPLIED_LOST_CONTROL_ENEMY_LOST;
   ...
   public void onAccept(int iCivID) {
      if (CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID() == iCivID) {
         CFG.toast
            .setInView(CFG.game.getCiv(this.iFromCivID).getCivName() + ": " + CFG.langManager.get("LostControlOverProvince"), CFG.COLOR_TEXT_NUM_OF_PROVINCES);
   ...
   public MenuElement_Hover_v2 getHover() {
      ...
      nData.add(new MenuElement_Hover_v2_Element_Type_Text(CFG.langManager.get("LostControlOverProvince") + ": ", CFG.COLOR_TEXT_NUM_OF_PROVINCES));
