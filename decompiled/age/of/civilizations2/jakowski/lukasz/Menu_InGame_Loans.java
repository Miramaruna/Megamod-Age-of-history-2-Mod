...
public class Menu_InGame_Loans extends SliderMenu {
   ...
   public Menu_InGame_Loans() {
      ...
      this.initMenu(
         new SliderMenuTitle(CFG.langManager.get("Loans"), CFG.BUTTON_HEIGHT * 3 / 5, true, true),
   ...
   public Menu_InGame_Loans(int onCivID) {
      ...
      int tempWidth = CFG.CIV_INFO_MENU_WIDTH * 2;
      int tY = 0;
      if (CFG.game.getCiv(onCivID).getLoansSize() > 0) {
         for (int i = 0; i < CFG.game.getCiv(onCivID).getLoansSize(); i++) {
            menuElements.add(
               new Buton_Diplomacy_Loan(
                  i,
                  onCivID,
                  CFG.game.getCiv(onCivID).getLoan(i).iGoldPerTurn,
                  CFG.game.getCiv(onCivID).getLoan(i).iGoldPerTurn * CFG.game.getCiv(onCivID).getLoan(i).iTurnsLeft,
                  CFG.game.getCiv(onCivID).getLoan(i).iTurnsLeft,
                  2,
                  tY,
                  CFG.BUTTON_WIDTH * 2
               ) {
                  ...
                  public void actionElement(int iID) {
                     CFG.menuManager.rebuildInGame_RepayLoan(Menu_InGame_Loans.this.iOnCivID, iID);
                  }
               }
            );
            tY += menuElements.get(menuElements.size() - 1).getHeight();
      ...
      int var6;
      menuElements.add(new Button_FlagActionSliderStyle(CFG.langManager.get("Close"), -1, CFG.PADDING, var6 = tY + CFG.PADDING, CFG.BUTTON_WIDTH, true) {
         @Override
         public int getWidth() {
            return Menu_InGame_Loans.this.getElementW() * 2 - CFG.PADDING * 2;
         }
      });
      int tempMenuPosY = ImageManager.getImage(Images.top_left2).getHeight() + CFG.PADDING * 2 + CFG.BUTTON_HEIGHT * 3 / 5;
      this.initMenu(
         new SliderMenuTitle(CFG.langManager.get("Loans"), CFG.BUTTON_HEIGHT * 3 / 5, true, true) {
            ...
            public void draw(SpriteBatch oSB, int iTranslateX, int nPosX, int nPosY, int nWidth, boolean sliderMenuIsActive) {
               ...
               ImageManager.getImage(Images.diplo_loan)
                  .draw(
                     oSB,
                     nPosX + (int)(nWidth - (this.getTextWidth() * 0.8F + ImageManager.getImage(Images.diplo_loan).getWidth() + CFG.PADDING)) / 2 + iTranslateX,
                     Menu_InGame_Loans.this.getPosY() - this.getHeight() / 2 - ImageManager.getImage(Images.diplo_loan).getHeight() / 2
                  );
               ...
               CFG.drawText(
                  ...
                  this.getText(),
                  nPosX
                     + (int)(nWidth - (this.getTextWidth() * 0.8F + ImageManager.getImage(Images.diplo_loan).getWidth() + CFG.PADDING)) / 2
