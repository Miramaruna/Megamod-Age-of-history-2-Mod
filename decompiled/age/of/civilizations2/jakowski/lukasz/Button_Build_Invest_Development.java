...
public class Button_Build_Invest_Development extends Button_Build {
   ...
   public Button_Build_Invest_Development(String sText, String sProvinceID, int nImageID, int nCost, int nMovementCost, int iPosX, int iPosY, int iWidth) {
      super(sText, nImageID, nCost, nMovementCost, iPosX, iPosY, iWidth, true, false, 0, 0.0F);
      this.sProvinceName = sProvinceID;
      this.sDate = CFG.langManager.get("Development") + ": ";
   ...
   public void setMin(int iMin) {
      float nMin = iMin / 10000.0F;

      try {
         this.sEconomy = "+"
            + nMin
            + " [+"
            + CFG.getPercentage_Max100_X100(nMin, CFG.game.getProvince(Menu_InGame_Invest_Development.iProvinceID).getDevelopmentLevel(), 4)
            + "%]";
