...
public class Menu_InGame_View_Recruitable extends SliderMenu {
   ...
   public Menu_InGame_View_Recruitable() {
      ...
      for (int i = 0; i < CFG.game.getCiv(this.iCivID).getNumOfProvinces(); i++) {
         if (CFG.FOG_OF_WAR != 2 || CFG.game.getPlayer(CFG.PLAYER_TURNID).getMetProvince(CFG.game.getCiv(this.iCivID).getProvinceID(i))) {
            ...
            tempRecruitableTotal += CFG.gameAction.getRecruitableArmy(CFG.game.getCiv(this.iCivID).getProvinceID(i));
         }
      }
      ...
      while (tempProvs.size() > 0) {
         ...
         for (int i2 = 1; i2 < tempProvs.size(); i2++) {
            if (CFG.gameAction.getRecruitableArmy(tempProvs.get(tBest), this.iCivID) < CFG.gameAction.getRecruitableArmy(tempProvs.get(i2), this.iCivID)) {
               tBest = i2;
            }
         }
      ...
      if (tempProvincesSorted.size() > 0) {
         menuElements.add(
            new Button_NS_Population(
               ...
               CFG.getNumberWithSpaces("" + tempRecruitableTotal),
               Images.diplo_army,
            ...
            menuElements.add(
               new Button_View_Recruitable(
                  ...
                  (CFG.SPECTATOR_MODE || CFG.game.isAlly(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(), this.iCivID))
                     && CFG.game.getCiv(this.iCivID).isInConstruction((Integer)tempProvincesSorted.get(var10), ConstructionType.ARMOURY) > 0
               ) {
                  @Override
                  public void actionElement(int iID) {
                     if (CFG.gameAction.getActiveTurnState() != Game_Action.TurnStates.INPUT_ORDERS
                        ...
                        || CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).isInConstruction(this.getCurrent(), ConstructionType.ARMOURY) > 0) {
                        ...
                        CFG.menuManager.rebuildInGame_BuildArmoury(this.getCurrent());
                     }
                  }
               }
            );
         ...
         new SliderMenuTitle(CFG.game.getCiv(this.iCivID).getCivName(), CFG.BUTTON_HEIGHT * 3 / 5, false, false) {
            ...
            public void draw(SpriteBatch oSB, int iTranslateX, int nPosX, int nPosY, int nWidth, boolean sliderMenuIsActive) {
               ImageManager.getImage(Images.new_game_top_edge_title)
                  .draw2(
                     oSB,
                     Menu_InGame_View_Recruitable.this.getPosX() + iTranslateX,
                     Menu_InGame_View_Recruitable.this.getPosY() - ImageManager.getImage(Images.new_game_top_edge_title).getHeight() - this.getHeight(),
                     Menu_InGame_View_Recruitable.this.getWidth() + 2,
                     this.getHeight(),
                     true,
                     false
                  );
               ...
               ImageManager.getImage(Images.pix255_255_255)
                  .draw(
                     oSB,
                     Menu_InGame_View_Recruitable.this.getPosX() + iTranslateX,
                     Menu_InGame_View_Recruitable.this.getPosY() - ImageManager.getImage(Images.pix255_255_255).getHeight(),
                     Menu_InGame_View_Recruitable.this.getWidth()
                  );
               ...
               ImageManager.getImage(Images.line_32_off1)
                  .draw(
                     oSB,
                     Menu_InGame_View_Recruitable.this.getPosX() + iTranslateX,
                     Menu_InGame_View_Recruitable.this.getPosY()
               ...
               ImageManager.getImage(Images.slider_gradient)
                  .draw(
                     ...
                     Menu_InGame_View_Recruitable.this.getWidth() / 4,
                     1
                  );
               ImageManager.getImage(Images.slider_gradient)
                  .draw(
                     oSB,
                     Menu_InGame_View_Recruitable.this.getPosX()
                        + Menu_InGame_View_Recruitable.this.getWidth()
                        - Menu_InGame_View_Recruitable.this.getWidth() / 4
                        + iTranslateX,
                     Menu_InGame_View_Recruitable.this.getPosY()
               ...
               ImageManager.getImage(Images.diplo_army)
                  .draw(
                     ...
                     Menu_InGame_View_Recruitable.this.getPosY() - this.getHeight() / 2 - ImageManager.getImage(Images.diplo_army).getHeight() / 2
                  );
