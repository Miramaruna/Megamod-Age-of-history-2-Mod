...
class Button_PeaceTreaty_Demands_ReleaseVassal extends Button {
   ...
   protected Button_PeaceTreaty_Demands_ReleaseVassal(
      ...
      MAX_WDITH_LEFT = Math.max(
            (int)(ImageManager.getImage(Images.city).getWidth() * this.getImageScale(ImageManager.getImage(Images.city).getHeight())),
            (int)(ImageManager.getImage(Images.flag_rect).getWidth() * this.getImageScale2(Images.flag_rect))
         )
         + CFG.PADDING * 4;
   }
   ...
   public void drawButtonBG(SpriteBatch oSB, int iTranslateX, int iTranslateY, boolean isActive) {
      ...
      oSB.setColor(
         new Color(
            CFG.game.getCiv(this.iReleaseCivID).getR() / 255.0F,
            CFG.game.getCiv(this.iReleaseCivID).getG() / 255.0F,
            CFG.game.getCiv(this.iReleaseCivID).getB() / 255.0F,
            0.125F
         )
      );
   ...
   protected final float getImageScale(int nHeight) {
      return CFG.TEXT_HEIGHT * 0.8F / nHeight;
   }
         ...
         if (this.iCivID > 0) {
            CFG.game
               ...
               .draw(
                  ...
                  this.getPosX()
                     + MAX_WDITH_LEFT / 2
                     - (int)(ImageManager.getImage(Images.flag_rect).getWidth() * this.getImageScale2(Images.flag_rect)) / 2
                     + iTranslateX,
                  this.getPosY()
                     + this.getHeight() / 2
                     - (int)(ImageManager.getImage(Images.flag_rect).getHeight() * this.getImageScale2(Images.flag_rect)) / 2
                     - CFG.game.getCiv(this.iCivID).getFlag().getHeight()
                     + iTranslateY,
                  (int)(ImageManager.getImage(Images.flag_rect).getWidth() * this.getImageScale2(Images.flag_rect)),
                  (int)(ImageManager.getImage(Images.flag_rect).getHeight() * this.getImageScale2(Images.flag_rect))
               );
            ...
            ImageManager.getImage(Images.diplo_vassal)
               .draw(
                  ...
                  this.getPosX()
                     + MAX_WDITH_LEFT / 2
                     - (int)(ImageManager.getImage(Images.diplo_vassal).getWidth() * this.getImageScale(ImageManager.getImage(Images.diplo_vassal).getHeight()))
                        / 2
                     + iTranslateX,
                  this.getPosY()
                     + 1
                     + this.getHeight() / 2
                     - (int)(
                           ImageManager.getImage(Images.diplo_vassal).getHeight() * this.getImageScale(ImageManager.getImage(Images.diplo_vassal).getHeight())
                        )
                        / 2
                     - ImageManager.getImage(Images.diplo_vassal).getHeight()
                     + iTranslateY,
                  (int)(ImageManager.getImage(Images.diplo_vassal).getWidth() * this.getImageScale(ImageManager.getImage(Images.diplo_vassal).getHeight())),
                  (int)(ImageManager.getImage(Images.diplo_vassal).getHeight() * this.getImageScale(ImageManager.getImage(Images.diplo_vassal).getHeight()))
               );
         }
   ...
   public void buildElementHover() {
      ...
      for (int var8 = 0; var8 < CFG.peaceTreatyData.peaceTreatyGameData.lCivsDemands_Aggressors.size(); var8++) {
         if (CFG.peaceTreatyData.peaceTreatyGameData.lCivsDemands_Aggressors.get(var8).iCivID == this.toReleaseByCivID) {
            for (int jx = 0; jx < CFG.peaceTreatyData.peaceTreatyGameData.lCivsDemands_Aggressors.get(var8).lReleasableCivs.size(); jx++) {
               if (CFG.peaceTreatyData.peaceTreatyGameData.lCivsDemands_Aggressors.get(var8).lReleasableCivs.get(jx).iCivID == this.iReleaseCivID) {
                  for (int k = 0;
                     k < CFG.peaceTreatyData.peaceTreatyGameData.lCivsDemands_Aggressors.get(var8).lReleasableCivs.get(jx).lProvinces.size() && k < 8;
                     k++
                  ) {
                     nData.add(new MenuElement_Hover_v2_Element_Type_Flag(this.iReleaseCivID));
                     nData.add(
                        new MenuElement_Hover_v2_Element_Type_Text(
                           CFG.game
                              .getProvince(CFG.peaceTreatyData.peaceTreatyGameData.lCivsDemands_Aggressors.get(var8).lReleasableCivs.get(jx).lProvinces.get(k))
                              .getName(),
                           CFG.COLOR_TEXT_MODIFIER_NEUTRAL
                        )
                     );
                     nData.add(
                        new MenuElement_Hover_v2_Element_Type_Text(
                           " "
                              + CFG.game
                                 .getProvinceValue(
                                    CFG.peaceTreatyData.peaceTreatyGameData.lCivsDemands_Aggressors.get(var8).lReleasableCivs.get(jx).lProvinces.get(k)
                                 ),
                           CFG.COLOR_TEXT_NUM_OF_PROVINCES
                        )
                     );
