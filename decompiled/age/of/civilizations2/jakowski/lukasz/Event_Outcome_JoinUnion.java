...
public class Event_Outcome_JoinUnion extends Event_Outcome {
   public int iCivID = -1;
   public int iCivID2 = -1;

   Event_Outcome_JoinUnion() {
   }
   ...
   public void outcomeAction() {
      if (this.canMakeAction()) {
         CFG.createUnion(this.getCivID(), this.getCivID2());
      }
   }
   ...
   public List<MenuElement_Hover_v2_Element2> getHoverText() {
      try {
         ...
         if (this.canMakeAction()) {
            tData.add(new MenuElement_Hover_v2_Element_Type_Text(CFG.langManager.get("Union") + ":", CFG.COLOR_BUTTON_GAME_TEXT_ACTIVE));
   ...
   public String getConditionText() {
      try {
         return CFG.langManager.get("CreateUnion")
            + ": "
