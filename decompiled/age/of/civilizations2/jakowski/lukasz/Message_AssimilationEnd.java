...
public class Message_AssimilationEnd extends Message {
   ...
   public void onAccept(int iCivID) {
      if (CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID() == iCivID) {
         ...
         lMess.add(CFG.langManager.get("TheProcessOfAssimilationHasComeToAnEnd"));
   ...
   public MenuElement_Hover_v2 getHover() {
      ...
      nData.add(new MenuElement_Hover_v2_Element_Type_Text(CFG.langManager.get("TheProcessOfAssimilationHasComeToAnEnd")));
      ...
      nData.add(
         new MenuElement_Hover_v2_Element_Type_Text(
            "" + (int)(CFG.game.getProvince(this.iValue).getProvinceStability() * 100.0F) + "%", CFG.COLOR_TEXT_PROVINCE_STABILITY_MAX
         )
      );
