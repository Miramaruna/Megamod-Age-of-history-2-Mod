...
public class Menu_InGame_SendGiftSandBox extends SliderMenu {
   public int iOnCivID = -1;

   public Menu_InGame_SendGiftSandBox() {
   ...
   public Menu_InGame_SendGiftSandBox(int onCivID) {
      ArrayList<MenuElement> menuElements = new ArrayList<>();
      ...
      menuElements.add(
         new Button_NS_Opinion(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(), CFG.getActiveCivInfo(), Images.diplo_gift, 0, 8, 2, tY, CFG.BUTTON_WIDTH * 2) {
            @Override
            public int getWidth() {
               return Menu_InGame_SendGiftSandBox.this.getElementW() * 2;
            }
         }
      );
      tY += menuElements.get(menuElements.size() - 1).getHeight();
      menuElements.add(
         new Slider_FlagAction_Gold(
            ...
            10000,
            (int)((float)CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getMoney()) * 5,
            0,
            0.65F,
            Images.top_gold
         ) {
            @Override
            public int getWidth() {
               return Menu_InGame_SendGiftSandBox.this.getElementW() * 2 - CFG.PADDING * 4;
            }
      ...
      menuElements.add(
         new Slider_FlagAction_Gold(
            ...
            CFG.TEXT_HEIGHT + CFG.PADDING * 2 + CFG.PADDING * 4,
            1000,
            (int)CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getManPower() * 5,
            0,
            0.65F,
      ...
      menuElements.add(
         new Slider_FlagAction_Gold(
            CFG.langManager.get("SelectTheAmountOfNuclearWeaponsToSend"),
            CFG.PADDING * 2,
            menuElements.get(menuElements.size() - 1).getPosY() + menuElements.get(menuElements.size() - 1).getHeight() + CFG.PADDING,
            tempWidth - CFG.PADDING * 3 - CFG.BUTTON_WIDTH,
            CFG.TEXT_HEIGHT + CFG.PADDING * 2 + CFG.PADDING * 4,
      ...
      menuElements.add(new Button_FlagActionSliderStyle(CFG.langManager.get("Cancel"), -1, 2 + CFG.PADDING, var10 = tY + CFG.PADDING, CFG.BUTTON_WIDTH, true) {
         @Override
         public int getWidth() {
            return Menu_InGame_SendGiftSandBox.this.getElementW() - CFG.PADDING - CFG.PADDING / 2;
         }
      });
      menuElements.add(
         new Button_FlagActionSliderStyle(CFG.langManager.get("Confirm"), -1, 2, var10, CFG.BUTTON_WIDTH, true) {
            @Override
            public int getPosX() {
               return Menu_InGame_SendGiftSandBox.this.getElementW() + CFG.PADDING / 2;
            }
            ...
            public void buildElementHover() {
               ...
               nData.add(new MenuElement_Hover_v2_Element_Type_Flag(Menu_InGame_SendGiftSandBox.this.iOnCivID, 0, CFG.PADDING));
               nData.add(new MenuElement_Hover_v2_Element_Type_Text(CFG.game.getCiv(Menu_InGame_SendGiftSandBox.this.iOnCivID).getCivName()));
            ...
            public void draw(SpriteBatch oSB, int iTranslateX, int nPosX, int nPosY, int nWidth, boolean sliderMenuIsActive) {
               ...
               ImageManager.getImage(Images.top_gold)
                  .draw(
                     ...
                     Menu_InGame_SendGiftSandBox.this.getPosY() - this.getHeight() / 2 - ImageManager.getImage(Images.top_gold).getHeight() / 2
                  );
               CFG.fontMain.getData().setScale(0.8F);
               CFG.drawText(
                  oSB,
   ...
   public final void actionElement(int iID) {
      if (iID == this.getMenuElementsSize() - 1) {
         DiplomacyManager.sendGiftSandBox(
            ...
            this.getMenuElement(this.getMenuElementsSize() - 5).getCurrent() * 5,
            this.getMenuElement(this.getMenuElementsSize() - 4).getCurrent() * 5,
