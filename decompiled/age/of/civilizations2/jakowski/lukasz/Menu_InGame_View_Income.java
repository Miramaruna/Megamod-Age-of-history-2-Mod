...
public class Menu_InGame_View_Income extends SliderMenu {
   ...
   public Menu_InGame_View_Income() {
      ...
      for (int i = 0; i < CFG.game.getCiv(this.iCivID).getNumOfProvinces(); i++) {
         if (CFG.FOG_OF_WAR != 2 || CFG.game.getPlayer(CFG.PLAYER_TURNID).getMetProvince(CFG.game.getCiv(this.iCivID).getProvinceID(i))) {
            ...
            tempIncome = (int)(tempIncome + CFG.game_NextTurnUpdate.getProvinceIncomeAndExpenses_Total(CFG.game.getCiv(this.iCivID).getProvinceID(i)));
         }
      }
      ...
      while (tempProvs.size() > 0) {
         ...
         for (int i2 = 1; i2 < tempProvs.size(); i2++) {
            if (CFG.game_NextTurnUpdate.getProvinceIncomeAndExpenses_Total(tempProvs.get(tBest))
               < CFG.game_NextTurnUpdate.getProvinceIncomeAndExpenses_Total(tempProvs.get(i2))) {
               tBest = i2;
            }
         }
      ...
      if (tempProvincesSorted.size() > 0) {
         menuElements.add(
            new Button_NS_Population(
               ...
               "" + (tempIncome > 0 ? "+" : "") + CFG.getNumberWithSpaces("" + tempIncome),
               Images.top_gold,
               tempIncome >= 0 ? CFG.COLOR_INGAME_GOLD : CFG.COLOR_TEXT_MODIFIER_NEGATIVE2,
               0,
               tY,
               tempW
            ) {}
            ...
            menuElements.add(
               new Button_View_Income(
                  ...
                  (CFG.SPECTATOR_MODE || CFG.game.isAlly(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(), this.iCivID))
                     && CFG.game.getCiv(this.iCivID).isInConstruction((Integer)tempProvincesSorted.get(var10), ConstructionType.WORKSHOP) > 0
               ) {
                  @Override
                  public void actionElement(int iID) {
                     if (CFG.gameAction.getActiveTurnState() != Game_Action.TurnStates.INPUT_ORDERS
                        ...
                        || CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).isInConstruction(this.getCurrent(), ConstructionType.WORKSHOP) > 0
                        )
                      {
                        ...
                        CFG.menuManager.rebuildInGame_BuildWorkshop(this.getCurrent());
                     }
                  }
               }
            );
         ...
         new SliderMenuTitle(CFG.game.getCiv(this.iCivID).getCivName(), CFG.BUTTON_HEIGHT * 3 / 5, false, false) {
            ...
            public void draw(SpriteBatch oSB, int iTranslateX, int nPosX, int nPosY, int nWidth, boolean sliderMenuIsActive) {
               ImageManager.getImage(Images.new_game_top_edge_title)
                  .draw2(
                     oSB,
                     Menu_InGame_View_Income.this.getPosX() + iTranslateX,
                     Menu_InGame_View_Income.this.getPosY() - ImageManager.getImage(Images.new_game_top_edge_title).getHeight() - this.getHeight(),
                     Menu_InGame_View_Income.this.getWidth() + 2,
                     this.getHeight(),
                     true,
                     false
                  );
               oSB.setColor(new Color(CFG.COLOR_INGAME_GOLD.r, CFG.COLOR_INGAME_GOLD.g, CFG.COLOR_INGAME_GOLD.b, 0.165F));
               ...
               ImageManager.getImage(Images.pix255_255_255)
                  .draw(
                     oSB,
                     Menu_InGame_View_Income.this.getPosX() + iTranslateX,
                     Menu_InGame_View_Income.this.getPosY() - ImageManager.getImage(Images.pix255_255_255).getHeight(),
                     Menu_InGame_View_Income.this.getWidth()
                  );
               ...
               ImageManager.getImage(Images.line_32_off1)
                  .draw(
                     oSB,
                     Menu_InGame_View_Income.this.getPosX() + iTranslateX,
                     Menu_InGame_View_Income.this.getPosY()
               ...
               ImageManager.getImage(Images.slider_gradient)
                  .draw(
                     ...
                     Menu_InGame_View_Income.this.getWidth() / 4,
                     1
                  );
               ImageManager.getImage(Images.slider_gradient)
                  .draw(
                     oSB,
                     Menu_InGame_View_Income.this.getPosX()
                        + Menu_InGame_View_Income.this.getWidth()
                        - Menu_InGame_View_Income.this.getWidth() / 4
                        + iTranslateX,
                     Menu_InGame_View_Income.this.getPosY()
               ...
               try {
                  CFG.game
                     .getCiv(Menu_InGame_View_Income.this.iCivID)
                     .getFlag()
                     .draw(
                        oSB,
                        ...
                        nPosY
                           ...
                           - CFG.game.getCiv(Menu_InGame_View_Income.this.iCivID).getFlag().getHeight(),
                        CFG.CIV_FLAG_WIDTH,
                        CFG.CIV_FLAG_HEIGHT
                     );
