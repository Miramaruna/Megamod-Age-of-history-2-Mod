...
public class Menu_MapEditor_FormableCivs_SelectFormable_List extends SliderMenu {
   ...
   public Menu_MapEditor_FormableCivs_SelectFormable_List() {
      ...
      if (CFG.sSearch != null && CFG.sSearch.length() > 0) {
         ...
         for (int var39 = 0; var39 < iSize; var39++) {
            if (CFG.langManager.getCiv(tagsSPLITED_ED[var39]).toLowerCase().indexOf(CFG.sSearch.toLowerCase()) >= 0
               && !CFG.isInFormableCivs(tagsSPLITED_ED[var39])) {
               boolean add = true;

               for (int jx = 0; jx < tagsSPLITED_ADDEDLength; jx++) {
                  if (tagsSPLITED_ADDED[jx].equals(tagsSPLITED_ED[var39])) {
                     add = false;
                     break;
                  }
               }
               ...
               if (add) {
                  lTempNames.add(CFG.langManager.getCiv(tagsSPLITED_ED[var39]));
                  lTempTags.add(tagsSPLITED_ED[var39]);
               }
            }
         }
         ...
         for (int ix = 0; ix < iSize; ix++) {
            if (!CFG.isInFormableCivs(tagsSPLITED[ix])) {
               ...
               for (int jxx = 0; jxx < tagsSPLITED_ADDEDLength; jxx++) {
                  if (tagsSPLITED_ADDED[jxx].equals(tagsSPLITED[ix])) {
                     add = false;
                     break;
                  }
               }
         ...
         for (int var35 = 0; var35 < iSize; var35++) {
            if (!CFG.isInFormableCivs(tagsSPLITED_ED[var35])) {
               boolean add = true;

               for (int jxxx = 0; jxxx < tagsSPLITED_ADDEDLength; jxxx++) {
                  if (tagsSPLITED_ADDED[jxxx].equals(tagsSPLITED_ED[var35])) {
                     add = false;
                     break;
                  }
               }
               ...
               if (add) {
                  lTempNames.add(CFG.langManager.getCiv(tagsSPLITED_ED[var35]));
                  lTempTags.add(tagsSPLITED_ED[var35]);
               }
            }
         }
         ...
         for (int ixx = 0; ixx < iSize; ixx++) {
            if (CFG.langManager.getCiv(tagsSPLITED[ixx]).charAt(0) == CFG.chosen_AlphabetCharachter.charAt(0) && !CFG.isInFormableCivs(tagsSPLITED[ixx])) {
               ...
               for (int jxxxx = 0; jxxxx < tagsSPLITED_ADDEDLength; jxxxx++) {
                  if (tagsSPLITED_ADDED[jxxxx].equals(tagsSPLITED[ixx])) {
                     add = false;
                     break;
                  }
               }
         ...
         for (int var37 = 0; var37 < iSize; var37++) {
            if (CFG.langManager.getCiv(tagsSPLITED_ED[var37]).charAt(0) == CFG.chosen_AlphabetCharachter.charAt(0)
               && !CFG.isInFormableCivs(tagsSPLITED_ED[var37])) {
               boolean add = true;

               for (int jxxxxx = 0; jxxxxx < tagsSPLITED_ADDEDLength; jxxxxx++) {
                  if (tagsSPLITED_ADDED[jxxxxx].equals(tagsSPLITED_ED[var37])) {
                     add = false;
                     break;
                  }
               }
               ...
               if (add) {
                  lTempNames.add(CFG.langManager.getCiv(tagsSPLITED_ED[var37]));
                  lTempTags.add(tagsSPLITED_ED[var37]);
               }
            }
         }
