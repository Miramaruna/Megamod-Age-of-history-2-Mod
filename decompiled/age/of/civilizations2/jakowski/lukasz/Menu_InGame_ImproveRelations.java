...
public class Menu_InGame_ImproveRelations extends SliderMenu {
   ...
   public Menu_InGame_ImproveRelations(int onCivID) {
      ...
      menuElements.add(
         new Button_NS_Opinion(
            CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(), CFG.getActiveCivInfo(), Images.diplo_relations_inc, 0, 5, 2, tY, CFG.BUTTON_WIDTH * 2
         ) {
            @Override
            public int getWidth() {
      ...
      menuElements.add(
         new Button_FlagActionSliderStyle(CFG.langManager.get("ImproveRelation"), -1, 2, var9, CFG.BUTTON_WIDTH, true) {
            @Override
            public int getPosX() {
            ...
            public void drawText(SpriteBatch oSB, int iTranslateX, int iTranslateY, boolean isActive) {
               ImageManager.getImage(Images.diplo_relations_inc)
                  .draw(
                     oSB,
                     this.getPosX()
                        + this.getWidth() / 2
                        - (int)((this.getTextWidth() * 0.8F + ImageManager.getImage(Images.diplo_relations_inc).getWidth() + CFG.PADDING) / 2.0F)
                        + iTranslateX,
                     this.getPosY() + this.getHeight() / 2 - ImageManager.getImage(Images.diplo_relations_inc).getHeight() / 2 + iTranslateY
                  );
                  ...
                  this.getPosX()
                     + (
                        this.getTextPos() < 0
                           ? this.getWidth() / 2
                              - (int)((this.getTextWidth() * 0.8F + ImageManager.getImage(Images.diplo_relations_inc).getWidth() + CFG.PADDING) / 2.0F)
                              + ImageManager.getImage(Images.diplo_relations_inc).getWidth()
                              + CFG.PADDING
                           : this.getTextPos()
                     )
                     + iTranslateX,
      ...
      if (iID == this.getMenuElementsSize() - 1) {
         if (this.getMenuElement(1).getCurrent() > 1) {
            CFG.game
               ...
               .addImproveRelations(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(), this.iOnCivID, this.getMenuElement(1).getCurrent());
         }
