...
public class Menu_Leader_Edit_SelectCiv_List extends SliderMenu {
   ...
   public Menu_Leader_Edit_SelectCiv_List() {
      ...
      if (CFG.sSearch != null && CFG.sSearch.length() > 0) {
         ...
         for (int i = 0; i < iSize; i++) {
            if (CFG.langManager.getCiv(tagsSPLITED[i]).toLowerCase().indexOf(CFG.sSearch.toLowerCase()) >= 0 && !CFG.isInLeaderCivs(tagsSPLITED[i])) {
         ...
         for (int var21 = 0; var21 < iSize; var21++) {
            if (CFG.langManager.getCiv(tagsSPLITED_ED[var21]).toLowerCase().indexOf(CFG.sSearch.toLowerCase()) >= 0
               && !CFG.isInLeaderCivs(tagsSPLITED_ED[var21])) {
         ...
         for (int ix = 0; ix < iSize; ix++) {
            if (!CFG.isInLeaderCivs(tagsSPLITED[ix])) {
         ...
         for (int var17 = 0; var17 < iSize; var17++) {
            if (!CFG.isInLeaderCivs(tagsSPLITED_ED[var17])) {
         ...
         for (int ixx = 0; ixx < iSize; ixx++) {
            if (CFG.langManager.getCiv(tagsSPLITED[ixx]).charAt(0) == CFG.chosen_AlphabetCharachter.charAt(0) && !CFG.isInLeaderCivs(tagsSPLITED[ixx])) {
         ...
         for (int var19 = 0; var19 < iSize; var19++) {
            if (CFG.langManager.getCiv(tagsSPLITED_ED[var19]).charAt(0) == CFG.chosen_AlphabetCharachter.charAt(0)
               && !CFG.isInLeaderCivs(tagsSPLITED_ED[var19])) {
   ...
   public final void actionElement(int iID) {
      if (iID % 2 == 0) {
         CFG.leader_GameData.addCiv(this.lCivsTags.get(iID / 2));
         this.onBack();
