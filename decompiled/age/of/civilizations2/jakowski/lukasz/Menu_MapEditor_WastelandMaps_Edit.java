...
public class Menu_MapEditor_WastelandMaps_Edit extends SliderMenu {
   ...
   public Menu_MapEditor_WastelandMaps_Edit() {
      ...
      menuElements.add(
         new Button_Game(
            CFG.langManager.get("Min") + " X",
            -1,
            CFG.GAME_WIDTH - CFG.BUTTON_WIDTH - CFG.PADDING,
      ...
      menuElements.add(
         new Button_Game(
            CFG.langManager.get("Max") + " X",
            -1,
            CFG.GAME_WIDTH - CFG.BUTTON_WIDTH - CFG.PADDING,
            CFG.GAME_HEIGHT - (CFG.BUTTON_HEIGHT + CFG.PADDING) * 3,
            true
         )
      );
      menuElements.add(
         new Button_Game(
            CFG.langManager.get("Min") + " Y",
            -1,
            CFG.GAME_WIDTH - CFG.BUTTON_WIDTH - CFG.PADDING,
            CFG.GAME_HEIGHT - (CFG.BUTTON_HEIGHT + CFG.PADDING) * 4,
            true
         )
      );
      menuElements.add(
         new Button_Game(
            CFG.langManager.get("Max") + " Y",
            -1,
            CFG.GAME_WIDTH - CFG.BUTTON_WIDTH - CFG.PADDING,
            CFG.GAME_HEIGHT - (CFG.BUTTON_HEIGHT + CFG.PADDING) * 5,
            true
         )
      );
   ...
   public void draw(SpriteBatch oSB, int iTranslateX, int iTranslateY, boolean sliderMenuIsActive) {
      ...
      CFG.drawEditorButtons_Bot_Edge_R_Reflected(
         ...
         this.getMenuElement(10).getPosY() - CFG.PADDING + iTranslateY,
         this.getMenuElement(6).getWidth() + CFG.PADDING * 2,
         CFG.GAME_HEIGHT - this.getMenuElement(10).getPosY() + CFG.PADDING
      );
      ...
      switch (iID) {
         ...
         case 7:
            ...
            for (int ixxx = 0; ixxx < CFG.game.getProvincesSize(); ixxx++) {
               if (!CFG.game.getProvince(ixxx).getSeaProvince()
                  && CFG.game.getProvince(ixxx).getWasteland() < 0
                  && CFG.game.getProvince(ixxx).getMinX() < tMinX) {
                  tMinX = CFG.game.getProvince(ixxx).getMinX();
                  nProvinceID = ixxx;
               }
            }
         ...
         case 8:
            ...
            for (int ixx = 0; ixx < CFG.game.getProvincesSize(); ixx++) {
               if (!CFG.game.getProvince(ixx).getSeaProvince() && CFG.game.getProvince(ixx).getWasteland() < 0 && CFG.game.getProvince(ixx).getMaxX() > tMaxX) {
                  tMaxX = CFG.game.getProvince(ixx).getMaxX();
                  nProvinceID = ixx;
               }
            }
         ...
         case 9:
            ...
            for (int ix = 0; ix < CFG.game.getProvincesSize(); ix++) {
               if (!CFG.game.getProvince(ix).getSeaProvince() && CFG.game.getProvince(ix).getWasteland() < 0 && CFG.game.getProvince(ix).getMinY() < tMinY) {
         ...
         case 10:
            ...
            for (int i = 0; i < CFG.game.getProvincesSize(); i++) {
               if (!CFG.game.getProvince(i).getSeaProvince() && CFG.game.getProvince(i).getWasteland() < 0 && CFG.game.getProvince(i).getMaxY() > tMaxY) {
   ...
   public final void saveData() {
      ...
      try {
         FileHandle file = CFG.readLocalFiles()
            ? Gdx.files.local("map/" + CFG.map.getFile_ActiveMap_Path() + "data/wasteland_maps/Age_of_Civilizations")
            : Gdx.files.internal("map/" + CFG.map.getFile_ActiveMap_Path() + "data/wasteland_maps/Age_of_Civilizations");
