...
public class Message_Liberation extends Message {
   public Message_Liberation(int byCivID) {
      super(byCivID, 0);
      this.messageType = Message_Type.LIBERATION_OF_VASSAL;
   ...
   public void onAccept(int iCivID) {
      if (CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID() == iCivID) {
         CFG.toast.setInView(CFG.langManager.get("Liberation"), CFG.COLOR_TEXT_NUM_OF_PROVINCES);
      }
   }
   ...
   public MenuElement_Hover_v2 getHover() {
      ...
      nData.add(new MenuElement_Hover_v2_Element_Type_Text(CFG.langManager.get("Liberation"), CFG.COLOR_TEXT_NUM_OF_PROVINCES));
      nElements.add(new MenuElement_Hover_v2_Element2(nData));
      nData.clear();
      nData.add(new MenuElement_Hover_v2_Element_Type_Text(CFG.langManager.get("WeAreNotLongerAVassal")));
