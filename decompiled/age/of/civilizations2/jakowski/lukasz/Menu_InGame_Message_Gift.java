...
public class Menu_InGame_Message_Gift extends SliderMenu {
   ...
   public Menu_InGame_Message_Gift() {
      ...
      this.initMenu(
         new SliderMenuTitle(CFG.langManager.get("NonAggressionPact"), CFG.BUTTON_HEIGHT * 3 / 5, true, true),
   ...
   public Menu_InGame_Message_Gift(int onCivID, int iMessageID, int iValue) {
      ArrayList<MenuElement> menuElements = new ArrayList<>();
      ...
      int tY = 0;
      menuElements.add(
         new Button_Diplomacy_Message_Gift(
            CFG.langManager.get("AGiftFromCivA", CFG.game.getCiv(this.iOnCivID).getCivName()), this.iOnCivID, "" + iValue, 2, tY, CFG.BUTTON_WIDTH * 2
         ) {
            @Override
            public int getWidth() {
      ...
      tY += menuElements.get(menuElements.size() - 1).getHeight();
      int var9;
      menuElements.add(new Button_FlagActionSliderStyle(CFG.langManager.get("Refuse"), -1, 2 + CFG.PADDING, var9 = tY + CFG.PADDING, CFG.BUTTON_WIDTH, true) {
         @Override
         public int getWidth() {
            return Menu_InGame_Message_Gift.this.getElementW() - CFG.PADDING - CFG.PADDING / 2;
         }
         ...
         public void buildElementHover() {
            ...
            nData.add(new MenuElement_Hover_v2_Element_Type_Flag(Menu_InGame_Message_Gift.this.iOnCivID));
            nData.add(new MenuElement_Hover_v2_Element_Type_Text(CFG.langManager.get("SendGidtBack"), CFG.COLOR_BUTTON_GAME_TEXT_ACTIVE));
            nElements.add(new MenuElement_Hover_v2_Element2(nData));
            nData.clear();
            nData.add(new MenuElement_Hover_v2_Element_Type_Text(CFG.langManager.get("Relation") + ": "));
            nData.add(new MenuElement_Hover_v2_Element_Type_Text("-8", CFG.COLOR_TEXT_MODIFIER_NEGATIVE2));
            nData.add(new MenuElement_Hover_v2_Element_Type_Flag(Menu_InGame_Message_Gift.this.iOnCivID, CFG.PADDING, 0));
      ...
      menuElements.add(new Button_FlagActionSliderStyle(CFG.langManager.get("Accept"), -1, 2, var9, CFG.BUTTON_WIDTH, true) {
         @Override
         public int getPosX() {
            return Menu_InGame_Message_Gift.this.getElementW() + CFG.PADDING / 2;
         }
         ...
         public void buildElementHover() {
            ArrayList<MenuElement_Hover_v2_Element2> nElements = new ArrayList<>();
            ArrayList<MenuElement_Hover_v2_Element_Type> nData = new ArrayList<>();
            nData.add(new MenuElement_Hover_v2_Element_Type_Text(CFG.langManager.get("Thanks") + "!", CFG.COLOR_BUTTON_GAME_TEXT_ACTIVE));
            nData.add(new MenuElement_Hover_v2_Element_Type_Flag(Menu_InGame_Message_Gift.this.iOnCivID, CFG.PADDING, 0));
      ...
      this.initMenu(
         new SliderMenuTitle(CFG.langManager.get("Gift"), CFG.BUTTON_HEIGHT * 3 / 5, true, true) {
            @Override
            public void draw(SpriteBatch oSB, int iTranslateX, int nPosX, int nPosY, int nWidth, boolean sliderMenuIsActive) {
               ...
               ImageManager.getImage(Images.diplo_message)
                  .draw(
                     oSB,
                     Menu_InGame_Message_Gift.this.getPosX() + CFG.PADDING * 2 + iTranslateX,
                     Menu_InGame_Message_Gift.this.getPosY() - this.getHeight() / 2 - ImageManager.getImage(Images.diplo_message).getHeight() / 2
                  );
   ...
   public final void actionElement(int iID) {
      if (iID == this.getMenuElementsSize() - 1) {
         ...
         CFG.toast.setInView(CFG.langManager.get("GiftSentBack"), CFG.COLOR_TEXT_NUM_OF_PROVINCES);
         CFG.toast.setTimeInView(4500);
         this.setVisible(false);
      } else {
