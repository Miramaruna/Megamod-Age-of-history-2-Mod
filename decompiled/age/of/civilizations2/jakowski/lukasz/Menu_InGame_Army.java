...
public class Menu_InGame_Army extends SliderMenu {
   public static int iSort = 1;

   public final int getRecruitedArmy(int nCivID) {
      return !CFG.SPECTATOR_MODE
            && CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID() != CFG.game.getSortedCivsAZ(nCivID - 1)
            && (
               CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getAllianceID() <= 0
                  || CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getAllianceID()
                     != CFG.game.getCiv(CFG.game.getSortedCivsAZ(nCivID - 1)).getAllianceID()
            )
            && CFG.game.getCiv(CFG.game.getSortedCivsAZ(nCivID - 1)).getPuppetOfCivID() != CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()
         ? -1
         : CFG.game.getCiv(CFG.game.getSortedCivsAZ(nCivID - 1)).getNumOfUnits();
   }

   public final int getRecruitedArmy_2(int nCivID) {
      return !CFG.SPECTATOR_MODE
            && CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID() != nCivID
            && (
               CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getAllianceID() <= 0
                  || CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getAllianceID() != CFG.game.getCiv(nCivID).getAllianceID()
            )
            && CFG.game.getCiv(nCivID).getPuppetOfCivID() != CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()
         ? -1
   ...
   public Menu_InGame_Army(int tInit) {
      ...
      this.initMenu(null, CFG.GAME_WIDTH * 3 / 4 - tempWidth / 2 - CFG.PADDING, tempMenuPosY, tempWidth, CFG.GAME_HEIGHT * 3 / 5, menuElements, false, false);
   }
   ...
   public Menu_InGame_Army() {
      ...
      menuElements.add(new Button_Statistics_Title(CFG.langManager.get("Civilizations"), CFG.PADDING * 2, 2, 0, CFG.BUTTON_WIDTH * 2, tElemHeight) {
         @Override
         public int getWidth() {
            return Menu_InGame_Army.this.getElementW() * 2 + CFG.PADDING * 2 - 2;
         }

         @Override
         public Color getColor(boolean isActive) {
            return Menu_InGame_Army.iSort == 0 ? CFG.COLOR_TEXT_NUM_OF_PROVINCES : super.getColor(isActive);
         }
      ...
      menuElements.add(
         new Button_Statistics_Title(CFG.langManager.get("Army"), CFG.PADDING, CFG.PADDING * 2 + CFG.BUTTON_WIDTH * 5, 0, CFG.BUTTON_WIDTH, tElemHeight) {
            @Override
            public int getPosX() {
               return Menu_InGame_Army.this.getElementW() * 2 + CFG.PADDING * 2;
            }

            @Override
            public int getWidth() {
               return Menu_InGame_Army.this.getW() - Menu_InGame_Army.this.getElementW() * 2 + CFG.PADDING * 2 - 2;
            }

            @Override
            public Color getColor(boolean isActive) {
               return Menu_InGame_Army.iSort == 1 ? CFG.COLOR_TEXT_NUM_OF_PROVINCES : super.getColor(isActive);
            }
      ...
      for (int i = 0; i < iSize; i++) {
         ...
         menuElements.add(
            new Button_Statistics(
               this.getRecruitedArmy_2(tSorted.get(i)) >= 0
                  ? CFG.getNumberWithSpaces("" + this.getRecruitedArmy_2(tSorted.get(i)))
