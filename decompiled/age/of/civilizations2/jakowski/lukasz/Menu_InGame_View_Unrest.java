...
public class Menu_InGame_View_Unrest extends SliderMenu {
   ...
   public Menu_InGame_View_Unrest() {
      ...
      for (int i2 = 0; i2 < CFG.game.getCiv(this.iCivID).getNumOfProvinces(); i2++) {
         if (CFG.game.getProvince(CFG.game.getCiv(this.iCivID).getProvinceID(i2)).getRevolutionaryRisk() >= 0.01F
            && (CFG.FOG_OF_WAR != 2 || CFG.game.getPlayer(CFG.PLAYER_TURNID).getMetProvince(CFG.game.getCiv(this.iCivID).getProvinceID(i2)))) {
            tempProvs.add(CFG.game.getCiv(this.iCivID).getProvinceID(i2));
         }
      }
      ...
      while (tempProvs.size() > 0) {
         ...
         for (int i = 1; i < tempProvs.size(); i++) {
            if (CFG.game.getProvince(tempProvs.get(tBest)).getRevolutionaryRisk() < CFG.game.getProvince(tempProvs.get(i)).getRevolutionaryRisk()) {
               tBest = i;
            }
         }
      ...
      SupportRebels_Data supportData = DiplomacyManager.supportRebels(this.iCivID);
      if (supportData.lMovements.size() > 0) {
         menuElements.add(
            new Text_AlliesNotInWar(CFG.langManager.get("RevolutionaryMovements"), -1, 0, tY, CFG.CIV_INFO_MENU_WIDTH, CFG.TEXT_HEIGHT + CFG.PADDING * 3) {
               @Override
         ...
         for (int ix = 0; ix < supportData.lMovements.size(); ix++) {
            menuElements.add(
               new Button_Diplomacy_SupportRebelsWithoutProvinces(
                  ix,
                  supportData.lMovements.get(ix),
                  supportData.lPopulation.get(ix),
                  (int)((float)supportData.lUnrest.get(ix).intValue() / supportData.lProvinces.get(ix).intValue()),
                  supportData.lProvinces.get(ix),
                  0,
                  tY,
                  ...
                  public void actionElement(int iID) {
                     ...
                     if (this.getCheckboxState() && CFG.gameAction.getActiveTurnState() == Game_Action.TurnStates.INPUT_ORDERS) {
                        canCenter = false;
                        CFG.menuManager.rebuildInGame_SupportRebels(Menu_InGame_View_Unrest.this.iCivID, this.getCurrent());
                     } else {
                        for (int ixx = 0; ixx < Menu_InGame_View_Unrest.this.getMenuElementsSize(); ixx++) {
                           Menu_InGame_View_Unrest.this.getMenuElement(ixx).setCheckboxState(false);
                        }
                     ...
                     if (canCenter
                        && (rebelsProvinces = DiplomacyManager.supportRebels_Provinces(Menu_InGame_View_Unrest.this.iCivID, this.getCurrent())).size() > 0) {
                        int tBest = 0;

                        for (int ixx = 1; ixx < rebelsProvinces.size(); ixx++) {
                           if (CFG.game.getPlayer(CFG.PLAYER_TURNID).getMetProvince(rebelsProvinces.get(ixx))
                              && CFG.game.getProvince(rebelsProvinces.get(tBest)).getPopulationData().getPopulationOfCivID(this.getCurrent())
                                 < CFG.game.getProvince(rebelsProvinces.get(ixx)).getPopulationData().getPopulationOfCivID(this.getCurrent())) {
                              tBest = ixx;
                           }
                        }
      ...
      if (tempProvincesSorted.size() > 0) {
         for (int ix = 0; ix < tempProvincesSorted.size(); ix++) {
            menuElements.add(
               new Button_View_Unrest(
                  ix,
                  (
                        CFG.game.getProvince((Integer)tempProvincesSorted.get(ix)).getName().length() > 0
                           ? CFG.game.getProvince((Integer)tempProvincesSorted.get(ix)).getName()
                           : CFG.game.getCiv(this.iCivID).getCivName()
                     )
                     + ": ",
                  (Integer)tempProvincesSorted.get(ix),
                  0,
                  tY,
                  CFG.CIV_INFO_MENU_WIDTH
               ) {
         ...
         new SliderMenuTitle(CFG.game.getCiv(this.iCivID).getCivName(), CFG.BUTTON_HEIGHT * 3 / 5, false, false) {
            ...
            public void draw(SpriteBatch oSB, int iTranslateX, int nPosX, int nPosY, int nWidth, boolean sliderMenuIsActive) {
               ImageManager.getImage(Images.new_game_top_edge_title)
                  .draw2(
                     oSB,
                     Menu_InGame_View_Unrest.this.getPosX() + iTranslateX,
                     Menu_InGame_View_Unrest.this.getPosY() - ImageManager.getImage(Images.new_game_top_edge_title).getHeight() - this.getHeight(),
                     Menu_InGame_View_Unrest.this.getWidth() + 2,
                     this.getHeight(),
                     true,
                     false
                  );
               oSB.setColor(
                  new Color(
                     CFG.game.getCiv(Menu_InGame_View_Unrest.this.iCivID).getR() / 255.0F,
                     CFG.game.getCiv(Menu_InGame_View_Unrest.this.iCivID).getG() / 255.0F,
                     CFG.game.getCiv(Menu_InGame_View_Unrest.this.iCivID).getB() / 255.0F,
                     0.165F
                  )
               );
               ...
               ImageManager.getImage(Images.pix255_255_255)
                  .draw(
                     oSB,
                     Menu_InGame_View_Unrest.this.getPosX() + iTranslateX,
                     Menu_InGame_View_Unrest.this.getPosY() - ImageManager.getImage(Images.pix255_255_255).getHeight(),
                     Menu_InGame_View_Unrest.this.getWidth()
                  );
               ...
               ImageManager.getImage(Images.line_32_off1)
                  .draw(
                     oSB,
                     Menu_InGame_View_Unrest.this.getPosX() + iTranslateX,
                     Menu_InGame_View_Unrest.this.getPosY()
               ...
               ImageManager.getImage(Images.slider_gradient)
                  .draw(
                     ...
                     Menu_InGame_View_Unrest.this.getWidth() / 4,
                     1
                  );
               ImageManager.getImage(Images.slider_gradient)
                  .draw(
                     oSB,
                     Menu_InGame_View_Unrest.this.getPosX()
                        + Menu_InGame_View_Unrest.this.getWidth()
                        - Menu_InGame_View_Unrest.this.getWidth() / 4
                        + iTranslateX,
                     Menu_InGame_View_Unrest.this.getPosY()
