...
public class Message_Plunder extends Message {
   public int iTreasury;
   public int iEconomy;
   public int iPopulation;
   public float fHappiness;
   public float fDevelopment;

   public Message_Plunder(int fromCivID, int iValue, int iTreasury, int iEconomy, float fDevelopment, float fHappiness, int iPopulation) {
      ...
      this.messageType = Message_Type.PLUNDER_REPROT;
   ...
   public void onAccept(int iCivID) {
      if (CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID() == iCivID) {
         ...
         lMess.add(CFG.langManager.get("Treasury") + ": +" + this.iTreasury);
         lColors.add(CFG.COLOR_INGAME_GOLD);
   ...
   public MenuElement_Hover_v2 getHover() {
      ...
      nData.add(new MenuElement_Hover_v2_Element_Type_Text("+" + this.iTreasury, CFG.COLOR_INGAME_GOLD));
      ...
      if ((int)(this.fDevelopment * 100.0F) > 0) {
         ...
         nData.add(new MenuElement_Hover_v2_Element_Type_Text("-" + (int)(this.fDevelopment * 100.0F) / 100.0F, CFG.COLOR_TEXT_MODIFIER_NEGATIVE2));
