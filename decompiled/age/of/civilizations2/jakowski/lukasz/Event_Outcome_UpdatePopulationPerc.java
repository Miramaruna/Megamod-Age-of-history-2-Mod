...
public class Event_Outcome_UpdatePopulationPerc extends Event_Outcome {
   public int iCivID = -1;
   public List<Integer> lProvinces = new ArrayList<>();
   public int iValue = 0;

   Event_Outcome_UpdatePopulationPerc() {
   }
               ...
               CFG.game
                  ...
                  .setPopulationOfCivID(
                     ...
                     CFG.game.getProvince(this.getProvinces().get(i)).getPopulationData().getPopulationID(j)
                        + (int)(CFG.game.getProvince(this.getProvinces().get(i)).getPopulationData().getPopulationID(j) * (this.getValue() / 100.0F))
                  );
            }
         }
      }
   ...
   public boolean canMakeAction(int i) {
      try {
         return !CFG.game.getProvince(this.getProvinces().get(i)).getSeaProvince()
            ...
            && (this.getCivID() <= 0 || this.getCivID() == CFG.game.getProvince(this.getProvinces().get(i)).getCivID());
   ...
   public String getConditionText() {
      try {
         return CFG.langManager.get("UpdatePopulationPercentage") + ": " + CFG.game.getCiv(this.getCivID()).getCivName() + ", " + this.getValue() + "%";
      } catch (IndexOutOfBoundsException var2) {
         return CFG.langManager.get("UpdatePopulationPercentage");
      }
   }
   ...
   public List<MenuElement_Hover_v2_Element2> getHoverText() {
      try {
         ...
         for (int i = 0; i < this.getProvinces().size(); i++) {
            if (this.canMakeAction(i)) {
               tData.add(new MenuElement_Hover_v2_Element_Type_Flag(CFG.game.getProvince(this.getProvinces().get(i)).getCivID()));
