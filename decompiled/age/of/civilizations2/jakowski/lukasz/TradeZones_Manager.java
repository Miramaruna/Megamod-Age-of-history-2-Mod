...
public class TradeZones_Manager {
   public List<TradeZone> lTradeZones = new ArrayList<>();
   public List<String> lTags = new ArrayList<>();
   public int iTradeZonesSize;

   public final void loadTradeZones_Age() {
      this.clearData();
   }

   public final void loadTradeZones_All() {
      this.clearData();
      ...
      for (int i = 0; i < CFG.gameAges.getAgesSize(); i++) {
         ...
         for (int j = 0; j < sFiles.size(); j++) {
            FileHandle fileData = Gdx.files.internal("map/" + CFG.map.getFile_ActiveMap_Path() + "data/trade_zones/zones/" + i + "/" + sFiles.get(j));
   ...
   public final int isInAnotherTradeZone(int nProvinceIDToCheck) {
   ...
   public final void addTradeZone(int nCenterOfTrade) {
   ...
   public final void removeTradeZone(int nTradeZoneID) {
      Gdx.files
         .local(
            "map/" + CFG.map.getFile_ActiveMap_Path() + "data/trade_zones/zones/" + this.getTradeZone(nTradeZoneID).getAge() + "/" + this.getTag(nTradeZoneID)
         )
         .delete();
   ...
   public final void saveTradeZone_GameData(int nTradeZoneID) {
   ...
   public final TradeZone getTradeZone(int i) {
   ...
   public final int getTradeZonesSize() {
