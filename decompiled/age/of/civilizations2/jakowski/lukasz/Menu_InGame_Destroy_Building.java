...
class Menu_InGame_Destroy_Building extends SliderMenu {
   private int iProvinceID = -1;
   private int iN = 0;

   public Menu_InGame_Destroy_Building() {
   ...
   public Menu_InGame_Destroy_Building(int nProvinceID, final int n) {
      this.iN = n;
      ArrayList<MenuElement> menuElements = new ArrayList<>();
      ...
      menuElements.add(
         new Button_Build_Building_Destroy(
            ...
            CFG.langManager.get("Destroy")
               + ": "
               + CFG.langManager.get(CustomBuildingsManager.getBuilding_Name(CFG.game.getProvince(this.iProvinceID).getLevelOfBuilding(n), n)),
            ...
            @Override
            public int getWidth() {
               return Menu_InGame_Destroy_Building.this.getElementW() * 2;
            }
            ...
            public void buildElementHover() {
               ...
               nData.add(
                  new MenuElement_Hover_v2_Element_Type_Text(
                     CFG.langManager
                           .get(
                              CustomBuildingsManager.getBuilding_Name(
                                 CFG.game.getProvince(Menu_InGame_Destroy_Building.this.iProvinceID).getLevelOfBuilding(n), n
                              )
                           )
                        + ": ",
               ...
               nData.add(
                  new MenuElement_Hover_v2_Element_Type_Text(
                     CFG.game.getProvince(Menu_InGame_Destroy_Building.this.iProvinceID).getName().length() > 0
                        ? CFG.game.getProvince(Menu_InGame_Destroy_Building.this.iProvinceID).getName()
                        : CFG.langManager.get("Province")
                  )
               );
               nData.add(
                  new MenuElement_Hover_v2_Element_Type_Flag(CFG.game.getProvince(Menu_InGame_Destroy_Building.this.iProvinceID).getCivID(), CFG.PADDING, 0)
               );
      ...
      menuElements.add(new Button_FlagActionSliderStyle(CFG.langManager.get("Cancel"), -1, 2 + CFG.PADDING, var8 = tY + CFG.PADDING, CFG.BUTTON_WIDTH, true) {
         @Override
         public int getWidth() {
            return Menu_InGame_Destroy_Building.this.getElementW() - CFG.PADDING - CFG.PADDING / 2;
         }
      });
      menuElements.add(
         new Button_FlagActionSliderStyle(CFG.langManager.get("Destroy"), -1, 2, var8, CFG.BUTTON_WIDTH, true) {
            @Override
            public int getPosX() {
               return Menu_InGame_Destroy_Building.this.getElementW() + CFG.PADDING / 2;
            }
         ...
         new SliderMenuTitle(
            ...
            public void draw(SpriteBatch oSB, int iTranslateX, int nPosX, int nPosY, int nWidth, boolean sliderMenuIsActive) {
               ...
               CFG.game
                  ...
                  .draw(
                     oSB,
                     Menu_InGame_Destroy_Building.this.getPosX() + CFG.PADDING * 2 + iTranslateX,
                     Menu_InGame_Destroy_Building.this.getPosY()
                        - this.getHeight() / 2
               ...
               ImageManager.getImage(Images.flag_rect)
                  .draw(
                     oSB,
                     Menu_InGame_Destroy_Building.this.getPosX() + CFG.PADDING * 2 + iTranslateX,
                     Menu_InGame_Destroy_Building.this.getPosY() - this.getHeight() / 2 - CFG.CIV_FLAG_HEIGHT / 2
                  );
               CFG.fontMain.getData().setScale(0.8F);
               CFG.drawText(
                  oSB,
   ...
   public final void actionElement(int iID) {
      if (iID == this.getMenuElementsSize() - 1) {
         if (CustomBuildingsManager.destroyBuilding(this.iProvinceID, CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(), this.iN)) {
