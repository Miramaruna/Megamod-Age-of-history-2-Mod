...
public class Menu_CreateScenario_Civilizations_Select_List extends SliderMenu {
   ...
   public Menu_CreateScenario_Civilizations_Select_List() {
      ...
      if (CFG.sSearch != null && CFG.sSearch.length() > 0) {
         ...
         for (int ix = 0; ix < iSize; ix++) {
            if (CFG.langManager.getCiv(tagsSPLITED[ix]).toLowerCase().indexOf(CFG.sSearch.toLowerCase()) >= 0 && !CFG.isInTheGame(tagsSPLITED[ix])) {
         ...
         for (int var28 = 0; var28 < iSize; var28++) {
            if (CFG.langManager.getCiv(tagsSPLITED_ED[var28]).toLowerCase().indexOf(CFG.sSearch.toLowerCase()) >= 0 && !CFG.isInTheGame(tagsSPLITED_ED[var28])) {
               lTempNames.add(CFG.langManager.getCiv(tagsSPLITED_ED[var28]));
               lTempTags.add(tagsSPLITED_ED[var28]);
            }
         }

         CFG.menuManager.menuCreateScenario_SelectCivilizations_UpdateTitle(lTempNames.size());
         int nPosY = 0;
                     ...
                     try {
                        ...
                        nData.add(
                           new MenuElement_Hover_v2_Element_Type_Text(
                              CFG.getWikiInormationsLink_Clear(Menu_CreateScenario_Civilizations_Select_List.this.lCivsTags.get(this.getTextPos())),
         ...
         for (int ixx = 0; ixx < iSize; ixx++) {
            if (!CFG.isInTheGame(tagsSPLITED[ixx])) {
         ...
         for (int var24 = 0; var24 < iSize; var24++) {
            if (!CFG.isInTheGame(tagsSPLITED_ED[var24])) {
               lTempNames.add(CFG.langManager.getCiv(tagsSPLITED_ED[var24]));
               lTempTags.add(tagsSPLITED_ED[var24]);
            }
         }
         ...
         for (int ixxx = 0; ixxx < iSize; ixxx++) {
            if (CFG.langManager.getCiv(tagsSPLITED[ixxx]).charAt(0) == CFG.chosen_AlphabetCharachter.charAt(0) && !CFG.isInTheGame(tagsSPLITED[ixxx])) {
         ...
         for (int var26 = 0; var26 < iSize; var26++) {
            if (CFG.langManager.getCiv(tagsSPLITED_ED[var26]).charAt(0) == CFG.chosen_AlphabetCharachter.charAt(0) && !CFG.isInTheGame(tagsSPLITED_ED[var26])) {
               lTempNames.add(CFG.langManager.getCiv(tagsSPLITED_ED[var26]));
               lTempTags.add(tagsSPLITED_ED[var26]);
            }
         }
   ...
   public final void actionElement(int iID) {
      switch (iID) {
         case 0:
            if (CFG.chosen_AlphabetCharachter == null && CFG.sSearch == null) {
               this.onBack();
               CFG.game.createScenarioAddCivilization("ran", CFG.game.getActiveProvinceID(), true);
               CFG.game.randomCivilizationColor(CFG.game.getCivsSize() - 1);
               int iRandomCivID = 1;
               ...
               CFG.game.getCiv(CFG.game.getCivsSize() - 1).setCivName(CFG.game.getCiv(CFG.game.getCivsSize() - 1).getCivName() + " " + iRandomCivID);
      ...
      if ((iID = iID - (CFG.chosen_AlphabetCharachter == null && CFG.sSearch == null ? 1 : 0)) % 2 == 0) {
         this.onBack();
         CFG.game.createScenarioAddCivilization(this.lCivsTags.get(iID / 2), CFG.game.getActiveProvinceID(), true);
