...
public class Event_Outcome_UpdateEconomyPerc extends Event_Outcome {
   public int iCivID = -1;
   public List<Integer> lProvinces = new ArrayList<>();
   public int iValue = 0;

   Event_Outcome_UpdateEconomyPerc() {
   }
         ...
         for (int i = 0; i < this.getProvinces().size(); i++) {
            if (this.canMakeAction(i)) {
               CFG.game
                  ...
                  .setEconomy(
                     CFG.game.getProvince(this.getProvinces().get(i)).getEconomy()
                        + (int)(CFG.game.getProvince(this.getProvinces().get(i)).getEconomy() * (this.getValue() / 100.0F))
                  );
            }
         }
      }
   ...
   public String getConditionText() {
      try {
         return CFG.langManager.get("UpdateEconomyPercentage") + ": " + CFG.game.getCiv(this.getCivID()).getCivName() + ", " + this.getValue();
      } catch (IndexOutOfBoundsException var2) {
         return CFG.langManager.get("UpdateEconomyPercentage");
      }
   }
