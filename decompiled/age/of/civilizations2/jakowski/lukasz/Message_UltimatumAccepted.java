...
public class Message_UltimatumAccepted extends Message {
   public Message_UltimatumAccepted(int fromCivID) {
      super(fromCivID, 0);
      this.messageType = Message_Type.ULTIMATUM_ACCEPTED;
   ...
   public void onAccept(int iCivID) {
      if (CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID() == iCivID) {
         ...
         lMess.add(CFG.game.getCiv(this.iFromCivID).getCivName() + ": " + CFG.langManager.get("AcceptedOurUltimatum"));
   ...
   public MenuElement_Hover_v2 getHover() {
      ...
      nData.add(new MenuElement_Hover_v2_Element_Type_Text(CFG.langManager.get("AcceptedOurUltimatum"), CFG.COLOR_TEXT_NUM_OF_PROVINCES));
