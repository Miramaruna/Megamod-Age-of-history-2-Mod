...
public class Event_Conditions_WatchTower extends Event_Conditions {
      ...
      try {
         if (this.getValue() == 0) {
            for (int i = 0; i < this.getProvinces().size(); i++) {
               if (CFG.game.getProvince(this.getProvinces().get(i)).getLevelOfWatchTower() == 0) {
                  return false;
               }
            }
         } else {
            for (int ix = 0; ix < this.getProvinces().size(); ix++) {
               if (CFG.game.getProvince(this.getProvinces().get(ix)).getLevelOfWatchTower() > 0) {
                  return false;
               }
            }
         }
   ...
   public String getConditionText() {
      try {
         return CFG.langManager.get("WatchTower") + ": " + (this.getValue() == 0);
      } catch (IndexOutOfBoundsException var2) {
