...
public class Menu_InGame_CallToArms extends SliderMenu {
   ...
   public Menu_InGame_CallToArms(int onCivID) {
      ...
      int tY = 0;
      menuElements.add(
         new Button_Diplomacy_CallToArms(Images.diplo_rivals, CFG.langManager.get("RequestHelpInTheWarAgainst") + ": ", onCivID, 2, tY, CFG.BUTTON_WIDTH * 2, 0) {
            @Override
            public int getWidth() {
      ...
      List<Integer> alliesToCall = DiplomacyManager.callToArmsListOfCivs(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(), onCivID);
      if (alliesToCall.size() > 0) {
         for (int i = 0; i < alliesToCall.size(); i++) {
            menuElements.add(new Buton_Diplomacy_CallAllies(i, alliesToCall.get(i), onCivID, 2, tY, CFG.BUTTON_WIDTH * 2) {
               @Override
               public int getWidth() {
                  return Menu_InGame_CallToArms.this.getElementW() * 2;
               }
      ...
      menuElements.add(new Button_FlagActionSliderStyle(CFG.langManager.get("Cancel"), -1, 2 + CFG.PADDING, var9 = tY + CFG.PADDING, CFG.BUTTON_WIDTH, true) {
         @Override
         public int getWidth() {
            return Menu_InGame_CallToArms.this.getElementW() - CFG.PADDING - CFG.PADDING / 2;
         }
      });
      menuElements.add(
         new Button_FlagActionSliderStyle(CFG.langManager.get("Confirm"), -1, 2, var9, CFG.BUTTON_WIDTH, canSend) {
            @Override
            public int getPosX() {
               return Menu_InGame_CallToArms.this.getElementW() + CFG.PADDING / 2;
            }
   ...
   public final void actionElement(int iID) {
      if (iID != this.getMenuElementsSize() - 1) {
         ...
         if (canSend) {
            for (int var4 = 1; var4 < this.getMenuElementsSize() - 2; var4++) {
               if (this.getMenuElement(var4).getCurrent() > 0 && this.getMenuElement(var4).getCheckboxState()) {
                  DiplomacyManager.sendCallToArms(this.getMenuElement(var4).getCurrent(), CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(), this.iOnCivID);
               }
            }
