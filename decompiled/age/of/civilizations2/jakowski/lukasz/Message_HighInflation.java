...
public class Message_HighInflation extends Message {
   public Message_HighInflation(int fromCivID, int iValue) {
      super(fromCivID, iValue);
      this.messageType = Message_Type.HIGH_INFLATION;
   ...
   public void onAccept(int iCivID) {
      if (CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID() == iCivID) {
         ...
         lMess.add(CFG.langManager.get("InflationH1"));
         lColors.add(CFG.COLOR_TEXT_MODIFIER_NEGATIVE2);
         lMess.add(CFG.langManager.get("InflationH2"));
         lColors.add(CFG.COLOR_TEXT_MODIFIER_NEGATIVE2);
   ...
   public MenuElement_Hover_v2 getHover() {
      ...
      nData.add(new MenuElement_Hover_v2_Element_Type_Text(CFG.langManager.get("HighInflation") + ": ", CFG.COLOR_TEXT_NUM_OF_PROVINCES));
      nData.add(
