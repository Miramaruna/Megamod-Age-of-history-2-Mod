...
public class Message_GoodsLow extends Message {
   public Message_GoodsLow(int fromCivID, int iValue) {
      super(fromCivID, iValue);
      this.messageType = Message_Type.GOODS_LOW;
   ...
   public void onAction(int iMessageID) {
      CFG.toast
         .setInView(
            CFG.langManager
               .get(
                  "hGoods3", (int)(CFG.ideologiesManager.getIdeology(CFG.game.getCiv(this.iFromCivID).getIdeologyID()).getMin_Goods(this.iFromCivID) * 100.0F)
               ),
         ...
         CFG.toast
            .setInView(
               CFG.langManager
                  .get(
                     "hGoods3",
                     (int)(CFG.ideologiesManager.getIdeology(CFG.game.getCiv(this.iFromCivID).getIdeologyID()).getMin_Goods(this.iFromCivID) * 100.0F)
                  ),
               CFG.COLOR_TEXT_MODIFIER_NEGATIVE2
            );
         CFG.toast.setTimeInView(4500);
   ...
   public MenuElement_Hover_v2 getHover() {
      ...
      nData.add(new MenuElement_Hover_v2_Element_Type_Text(CFG.langManager.get("Goods"), CFG.COLOR_TEXT_NUM_OF_PROVINCES));
               ...
               .get(
                  ...
                  (int)(
                     CFG.ideologiesManager
                           .getIdeology(CFG.game.getCiv(this.iFromCivID).getIdeologyID())
