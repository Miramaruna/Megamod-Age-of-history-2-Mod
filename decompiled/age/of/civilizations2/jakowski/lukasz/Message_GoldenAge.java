...
public class Message_GoldenAge extends Message {
   public Message_GoldenAge(int fromCivID, int iNumOfTurns) {
      super(fromCivID, 0);
      this.messageType = Message_Type.GOLDEN_AGE;
      this.iNumOfTurnsLeft = iNumOfTurns;
   }

   @Override
   public void onAction(int iMessageID) {
      CFG.toast.setInView(CFG.langManager.get("OurCivilizationIsInAGoldenAgeOf") + ": " + CFG.langManager.get("GAProsperity"), CFG.COLOR_TEXT_GOLDEN_AGE);
   }
   ...
   public MenuElement_Hover_v2 getHover() {
      ...
      nData.add(new MenuElement_Hover_v2_Element_Type_Text(CFG.langManager.get("GAProsperity")));
      ...
      for (int i = 0; i < CFG.game.getCiv(this.iFromCivID).getBonusesSize(); i++) {
         if (CFG.game.getCiv(this.iFromCivID).getBonus(i).BONUS_TYPE == CivBonus_Type.GOLDEN_AGE_PROSPERITY) {
            nID = i;
            break;
         }
      }
