...
public class Message_JoinedAWar extends Message {
   public Message_JoinedAWar(int fromCivID, int iValue) {
      super(fromCivID, iValue);
      this.messageType = Message_Type.JOINED_A_WAR;
   ...
   public void onAccept(int iCivID) {
      if (CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID() == iCivID) {
         ...
         lMess.add(CFG.langManager.get("XJoinedTheWar", CFG.game.getCiv(this.iFromCivID).getCivName()));
   ...
   public MenuElement_Hover_v2 getHover() {
      ...
      nData.add(
         new MenuElement_Hover_v2_Element_Type_Text(
            CFG.langManager.get("XJoinedTheWar", CFG.game.getCiv(this.iFromCivID).getCivName()), CFG.COLOR_TEXT_MODIFIER_POSITIVE
         )
      );
