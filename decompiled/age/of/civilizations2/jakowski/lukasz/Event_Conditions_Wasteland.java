...
public class Event_Conditions_Wasteland extends Event_Conditions {
   public List<Integer> lProvinces = new ArrayList<>();
   public int iValue = 0;

   Event_Conditions_Wasteland() {
   }
      ...
      try {
         if (this.getValue() == 0) {
            for (int i = 0; i < this.getProvinces().size(); i++) {
               if (CFG.game.getProvince(this.getProvinces().get(i)).getWasteland() < 0) {
                  return false;
               }
            }
         } else {
            for (int ix = 0; ix < this.getProvinces().size(); ix++) {
               if (CFG.game.getProvince(this.getProvinces().get(ix)).getWasteland() >= 0) {
                  return false;
               }
            }
         }
   ...
   public String getConditionText() {
      try {
         return CFG.langManager.get("IsWasteland") + ": " + (this.getValue() == 0);
      } catch (IndexOutOfBoundsException var2) {
         return CFG.langManager.get("IsWasteland");
      }
   }
