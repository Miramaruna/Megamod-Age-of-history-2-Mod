...
class Event_Outcome_AddAttackBonus_Ya
extends Event_Outcome {
    public int iCivID = -1;
    public int iValue = 0;

    Event_Outcome_AddAttackBonus_Ya() {
    }
    ...
    public void outcomeAction() {
        if (this.canMakeAction()) {
            CFG.game.getCiv(this.getCivID()).setModifier_AttackBonus(CFG.game.getCiv(this.getCivID()).getModifier_AttackBonus() + (float)this.getValue() / 100.0f);
        }
    }
    ...
    public String getConditionText() {
        try {
            return CFG.langManager.get("AddAttackBonus") + ": " + CFG.game.getCiv(this.getCivID()).getCivName() + ", " + (this.getValue() > 0 ? "+" : "") + (float)this.getValue() / 10.0f;
        }
        catch (IndexOutOfBoundsException var2) {
            return CFG.langManager.get("AddAttackBonus");
        }
    }
    ...
    public List<MenuElement_Hover_v2_Element2> getHoverText() {
        try {
            ...
            if (this.canMakeAction()) {
                ...
                tData.add(new MenuElement_Hover_v2_Element_Type_Text(CFG.langManager.get("AttackBonus") + ": ", CFG.COLOR_BUTTON_GAME_TEXT_ACTIVE));
