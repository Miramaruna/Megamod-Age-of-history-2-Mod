...
public class CivArmyMission_Expand_BuildPort extends CivArmyMission {
   ...
   public final void lockTreasury_Port(int nCivID, int nProvinceID) {
      ...
      if (CFG.game.getCiv(nCivID).iBudget > 0) {
         this.iObsolate = CFG.game.getCiv(nCivID).getMoney() > 0L && CFG.game.getCiv(nCivID).getMoney() < colonizeCost
            ? Math.max(this.iObsolate, Math.max(2, (int)Math.ceil((float)CFG.game.getCiv(nCivID).getMoney() / colonizeCost)))
