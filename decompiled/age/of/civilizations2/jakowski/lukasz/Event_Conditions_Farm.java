...
public class Event_Conditions_Farm extends Event_Conditions {
   public List<Integer> lProvinces = new ArrayList<>();
   public int iValue = 0;

   Event_Conditions_Farm() {
   }
      ...
      try {
         if (this.getValue() == 0) {
            for (int i = 0; i < this.getProvinces().size(); i++) {
               if (CFG.game.getProvince(this.getProvinces().get(i)).getLevelOfFarm() == 0) {
                  return false;
               }
            }
         } else {
            for (int ix = 0; ix < this.getProvinces().size(); ix++) {
               if (CFG.game.getProvince(this.getProvinces().get(ix)).getLevelOfFarm() > 0) {
                  return false;
               }
            }
         }
   ...
   public String getConditionText() {
      try {
         return CFG.langManager.get("Farm") + ": " + (this.getValue() == 0);
      } catch (IndexOutOfBoundsException var2) {
         return CFG.langManager.get("Farm");
      }
   }
