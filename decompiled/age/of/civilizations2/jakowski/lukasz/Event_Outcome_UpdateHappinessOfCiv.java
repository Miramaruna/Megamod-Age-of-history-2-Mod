...
public class Event_Outcome_UpdateHappinessOfCiv extends Event_Outcome {
   public int iCivID = -1;
   public int iValue = 0;

   Event_Outcome_UpdateHappinessOfCiv() {
   }
      ...
      if (this.canMakeAction()) {
         for (int i = 0; i < CFG.game.getCiv(this.getCivID()).getNumOfProvinces(); i++) {
            CFG.game
               ...
               .setHappiness(CFG.game.getProvince(CFG.game.getCiv(this.getCivID()).getProvinceID(i)).getHappiness() + this.getValue() / 100.0F);
         }
      }
   }
