...
public class Event_Outcome_UpdateEconomy extends Event_Outcome {
   public int iCivID = -1;
   public List<Integer> lProvinces = new ArrayList<>();
   public int iValue = 0;

   Event_Outcome_UpdateEconomy() {
   }
      ...
      if (this.getCivID() >= 0 && this.getCivID() < CFG.game.getCivsSize()) {
         for (int i = 0; i < this.getProvinces().size(); i++) {
            if (this.canMakeAction(i)) {
               CFG.game.getProvince(this.getProvinces().get(i)).setEconomy(CFG.game.getProvince(this.getProvinces().get(i)).getEconomy() + this.getValue());
            }
         }
      }
   }
