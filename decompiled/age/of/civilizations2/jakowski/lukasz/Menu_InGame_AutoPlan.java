...
public class Menu_InGame_AutoPlan extends SliderMenu {
   public int iOnCivID = -1;
   String str;

   public Menu_InGame_AutoPlan(int onCivID) {
      ...
      menuElements.add(
         new Button_NS_Opinion(
            ...
            @Override
            public int getWidth() {
               return Menu_InGame_AutoPlan.this.getElementW() * 2;
            }
         }
      );
      ...
      menuElements.add(new Button_FlagActionSliderStyle(CFG.langManager.get("Cancel"), -1, 2 + CFG.PADDING, var7 = tY + CFG.PADDING, CFG.BUTTON_WIDTH, true) {
         @Override
         public int getWidth() {
            return Menu_InGame_AutoPlan.this.getElementW() - CFG.PADDING - CFG.PADDING / 2;
         }
      });
      menuElements.add(
         new Button_FlagActionSliderStyle(CFG.langManager.get("Confirm"), -1, 2, var7, CFG.BUTTON_WIDTH, true) {
            @Override
            public int getPosX() {
               return Menu_InGame_AutoPlan.this.getElementW() + CFG.PADDING / 2;
            }
            ...
            public void buildElementHover() {
               ...
               nData.add(new MenuElement_Hover_v2_Element_Type_Flag(Menu_InGame_AutoPlan.this.iOnCivID, CFG.PADDING, CFG.PADDING));
               nData.add(new MenuElement_Hover_v2_Element_Type_Text(CFG.game.getCiv(Menu_InGame_AutoPlan.this.iOnCivID).getCivName()));
      ...
      this.str = CFG.game.getCiv(onCivID).isMoveAtWarPlayer <= 0 ? "autoplan" : "noautoplan";
      this.initMenu(
   ...
   public final void actionElement(int paramInt) {
      if (paramInt == this.getMenuElementsSize() - 1) {
         ...
         if (CFG.game.getCiv(paramInt).isMoveAtWarPlayer <= 0) {
            CFG.oAI.getAI_Style(CFG.game.getCiv(paramInt).getAI_Style()).moveAtWarPlayer(paramInt);
            CFG.game.getCiv(paramInt).isMoveAtWarPlayer = 1;
            CFG.menuManager.updateInGame_TOP_All(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID());
         } else {
            CFG.game.getCiv(paramInt).isMoveAtWarPlayer = 0;
         }
