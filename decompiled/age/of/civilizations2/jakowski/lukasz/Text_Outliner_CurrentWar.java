...
public class Text_Outliner_CurrentWar extends Text {
   public static final float FONT_SCALE = 0.7F;
   public boolean row = false;
   public int iWarID;
   public int iCivID;
   public Color colorWarScore;
   public boolean sentPeaceTreaty = false;

   public Text_Outliner_CurrentWar(int nWarID, int iPosX, int iPosY, int iWidth) {
      ...
      try {
         for (int i2 = 0; i2 < CFG.game.lPeaceTreaties.size(); i2++) {
            if (CFG.game.getWar(this.iWarID).WAR_TAG.equals(CFG.game.lPeaceTreaties.get(i2).peaceTreaty_GameData.WAR_TAG)) {
      ...
      if (CFG.game.getWar(nWarID).getIsAggressor(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID())) {
         ...
         for (int ix = 1; ix < CFG.game.getWar(nWarID).getAggressorsSize(); ix++) {
            if (CFG.game.getCiv(CFG.game.getWar(nWarID).getAggressorID(ix).getCivID()).getRankScore()
      ...
      this.colorWarScore = Math.abs(tempWarScore) >= 100
         ? CFG.COLOR_TEXT_NUM_OF_PROVINCES
         : (tempWarScore > 0 ? CFG.COLOR_TEXT_MODIFIER_POSITIVE : (tempWarScore == 0 ? CFG.COLOR_TEXT_MODIFIER_NEUTRAL2 : CFG.COLOR_TEXT_MODIFIER_NEGATIVE2));
   ...
   public void buildElementHover() {
      ...
      for (int var6 = 0; var6 < CFG.game.getWar(this.iWarID).getDefendersSize() && var6 < 5; var6++) {
         nData.add(new MenuElement_Hover_v2_Element_Type_Flag(CFG.game.getWar(this.iWarID).getDefenderID(var6).getCivID(), 0, 0));
      }
