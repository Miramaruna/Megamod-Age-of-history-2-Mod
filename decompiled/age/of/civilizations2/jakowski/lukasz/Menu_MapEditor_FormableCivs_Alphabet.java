...
public class Menu_MapEditor_FormableCivs_Alphabet extends SliderMenu {
   ...
   public Menu_MapEditor_FormableCivs_Alphabet() {
      ...
      try {
         ...
         for (int var11 = 0; var11 < this.lCharacters.size() - 1; var11++) {
            for (int j = var11 + 1; j < this.lCharacters.size(); j++) {
               if (this.lCharacters.get(var11) > this.lCharacters.get(j)) {
                  char temp = this.lCharacters.get(var11);
                  this.lCharacters.set(var11, this.lCharacters.get(j));
                  this.lCharacters.set(j, temp);
               }
            }
         }

         for (int var12 = 0; var12 < this.lCharacters.size(); var12++) {
            if (CFG.chosen_AlphabetCharachter != null && this.lCharacters.get(var12) == CFG.chosen_AlphabetCharachter.charAt(0)) {
               menuElements.add(
                  new Button_Menu_Active(
                     "[" + this.lCharacters.get(var12) + "]",
                     -1,
                     CFG.BUTTON_HEIGHT * (var12 + 1) + CFG.BUTTON_WIDTH * 2,
                     CFG.PADDING,
                     CFG.BUTTON_HEIGHT,
                     CFG.BUTTON_HEIGHT,
                     true
