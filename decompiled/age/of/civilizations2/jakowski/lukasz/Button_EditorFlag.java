...
public class Button_EditorFlag extends Button {
   public int nCivID = 0;
   ...
   public final void drawButtonBG(SpriteBatch oSB, int iTranslateX, int iTranslateY, boolean isActive) {
      if (CFG.game.getCiv(this.nCivID).getCivTag().equals("ran")) {
         oSB.setColor(
            new Color(
               CFG.game.getCiv(this.nCivID).getR() / 255.0F, CFG.game.getCiv(this.nCivID).getG() / 255.0F, CFG.game.getCiv(this.nCivID).getB() / 255.0F, 1.0F
            )
         );
         CFG.game
            .getCiv(this.nCivID)
