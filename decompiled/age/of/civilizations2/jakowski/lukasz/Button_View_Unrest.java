...
public class Button_View_Unrest extends Button {
      ...
      ImageManager.getImage(Images.diplo_revolution)
         .draw(
            ...
            this.getPosY()
               ...
               + iTranslateY
               - ImageManager.getImage(Images.diplo_revolution).getHeight(),
            (int)(ImageManager.getImage(Images.diplo_revolution).getWidth() * this.getImageScale(ImageManager.getImage(Images.diplo_revolution).getHeight())),
            (int)(ImageManager.getImage(Images.diplo_revolution).getHeight() * this.getImageScale(ImageManager.getImage(Images.diplo_revolution).getHeight()))
         );
      CFG.fontMain.getData().setScale(0.7F);
   ...
   public void buildElementHover() {
      ...
      if (CFG.game.getProvince(this.iProvinceID).saveProvinceData.iSupportRebelsSize > 0) {
         ...
         for (int i = 0; i < CFG.game.getProvince(this.iProvinceID).saveProvinceData.iSupportRebelsSize; i++) {
            ...
            for (int j = lCivs.size() - 1; j >= 0; j--) {
               if (lCivs.get(j) == tAddID) {
                  ...
                  ((List)lSupportedByCivs.get(j)).add(CFG.game.getProvince(this.iProvinceID).saveProvinceData.lSupportRebels.get(i).iByCivID);
                  break;
               }
            }
            ...
            if (!wasAdded) {
               ...
               ((List)lSupportedByCivs.get(lSupportedByCivs.size() - 1))
                  .add(CFG.game.getProvince(this.iProvinceID).saveProvinceData.lSupportRebels.get(i).iByCivID);
               if (lCivs.size() >= 4) {
                  break;
               }
            }
         ...
         for (int var11 = 0; var11 < iCivsSize; var11++) {
            nData.add(new MenuElement_Hover_v2_Element_Type_Flag(lCivs.get(var11)));
            nData.add(
               new MenuElement_Hover_v2_Element_Type_Text(
                  lCivs.get(var11) > 0 ? CFG.game.getCiv(lCivs.get(var11)).getCivName() : CFG.langManager.get("Undiscovered"),
                  CFG.COLOR_BUTTON_GAME_TEXT_ACTIVE
               )
            );

            for (int k = 0; k < ((List)lSupportedByCivs.get(var11)).size() && k < 10; k++) {
               nData.add(
                  new MenuElement_Hover_v2_Element_Type_Flag(
                     !CFG.SPECTATOR_MODE
                           && !CFG.game.isAlly(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(), (Integer)((List)lSupportedByCivs.get(var11)).get(k))
                        ? -(Integer)((List)lSupportedByCivs.get(var11)).get(k)
                        : (Integer)((List)lSupportedByCivs.get(var11)).get(k),
                     k == 0 ? CFG.PADDING : 0,
                     0
                  )
            ...
            nData.add(
               new MenuElement_Hover_v2_Element_Type_Text(
                  " " + Game_Calendar.getDate_ByTurnID(Game_Calendar.TURN_ID + lCivsTurnsLeft.get(var11)), CFG.COLOR_TEXT_MODIFIER_NEUTRAL
               )
            );
            nData.add(
               new MenuElement_Hover_v2_Element_Type_Text(" [" + CFG.langManager.get("TurnsX", lCivsTurnsLeft.get(var11)) + "]", CFG.COLOR_TEXT_RANK_HOVER)
            );
            nData.add(new MenuElement_Hover_v2_Element_Type_Image(Images.time, CFG.PADDING, 0));
            nElements.add(new MenuElement_Hover_v2_Element2(nData));
            nData.clear();
