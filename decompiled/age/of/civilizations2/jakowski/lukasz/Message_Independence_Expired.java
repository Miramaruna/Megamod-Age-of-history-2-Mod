...
public class Message_Independence_Expired extends Message {
   public Message_Independence_Expired(int fromCivID) {
      super(fromCivID, 0);
      this.messageType = Message_Type.GUARANTEE_EXPIRED;
   ...
   public void onAccept(int iCivID) {
      if (CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID() == iCivID) {
         ...
         lMess.add(CFG.langManager.get("OurIndependenceIsNoLongerGuaranteedBy") + ": " + CFG.game.getCiv(this.iFromCivID).getCivName());
   ...
   public MenuElement_Hover_v2 getHover() {
      ...
      nData.add(
         new MenuElement_Hover_v2_Element_Type_Text(CFG.langManager.get("OurIndependenceIsNoLongerGuaranteedBy") + ": ", CFG.COLOR_TEXT_NUM_OF_PROVINCES)
      );
