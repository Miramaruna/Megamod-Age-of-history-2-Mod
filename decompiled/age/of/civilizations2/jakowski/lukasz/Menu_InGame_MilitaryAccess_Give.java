...
public class Menu_InGame_MilitaryAccess_Give extends SliderMenu {
   ...
   public Menu_InGame_MilitaryAccess_Give() {
      ...
      this.initMenu(
         new SliderMenuTitle(CFG.langManager.get("OfferMilitaryAccess"), CFG.BUTTON_HEIGHT * 3 / 5, true, true),
   ...
   public Menu_InGame_MilitaryAccess_Give(int onCivID) {
      ...
      menuElements.add(
         new Button_NS_Opinion(
            CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(), CFG.getActiveCivInfo(), Images.diplo_access_gives, 0, 4, 2, tY, CFG.BUTTON_WIDTH * 2
         ) {
            @Override
            public int getWidth() {
      ...
      menuElements.add(
         new Slider_FlagAction_Date(
            ...
            @Override
            public int getWidth() {
               return Menu_InGame_MilitaryAccess_Give.this.getElementW() * 2 - CFG.PADDING * 4;
            }
      ...
      menuElements.add(new Button_FlagActionSliderStyle(CFG.langManager.get("Cancel"), -1, 2 + CFG.PADDING, var8 = tY + CFG.PADDING, CFG.BUTTON_WIDTH, true) {
         @Override
         public int getWidth() {
            return Menu_InGame_MilitaryAccess_Give.this.getElementW() - CFG.PADDING - CFG.PADDING / 2;
         }
      });
      menuElements.add(
         new Button_FlagActionSliderStyle(CFG.langManager.get("Confirm"), -1, 2, var8, CFG.BUTTON_WIDTH, true) {
            @Override
            public int getPosX() {
               return Menu_InGame_MilitaryAccess_Give.this.getElementW() + CFG.PADDING / 2;
            }
            ...
            public void buildElementHover() {
               ...
               nData.add(new MenuElement_Hover_v2_Element_Type_Flag(Menu_InGame_MilitaryAccess_Give.this.iOnCivID, CFG.PADDING, CFG.PADDING));
               nData.add(new MenuElement_Hover_v2_Element_Type_Text(CFG.game.getCiv(Menu_InGame_MilitaryAccess_Give.this.iOnCivID).getCivName()));
               ...
               nData.add(new MenuElement_Hover_v2_Element_Type_Text("-0.4", CFG.COLOR_TEXT_MODIFIER_NEGATIVE2));
            ...
            public void drawText(SpriteBatch oSB, int iTranslateX, int iTranslateY, boolean isActive) {
               ImageManager.getImage(Images.diplo_access_gives)
                  .draw(
                     oSB,
                     this.getPosX()
                        + this.getWidth() / 2
                        - (int)((this.getTextWidth() * 0.8F + ImageManager.getImage(Images.diplo_access_gives).getWidth() + CFG.PADDING) / 2.0F)
                        + iTranslateX,
                     this.getPosY() + this.getHeight() / 2 - ImageManager.getImage(Images.diplo_access_gives).getHeight() / 2 + iTranslateY
                  );
                  ...
                  this.getPosX()
                     + (
                        this.getTextPos() < 0
                           ? this.getWidth() / 2
                              - (int)((this.getTextWidth() * 0.8F + ImageManager.getImage(Images.diplo_access_gives).getWidth() + CFG.PADDING) / 2.0F)
                              + ImageManager.getImage(Images.diplo_access_gives).getWidth()
                              + CFG.PADDING
                           : this.getTextPos()
                     )
                     + iTranslateX,
            ...
            public boolean getClickable() {
               return CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getDiplomacyPoints() >= 4;
            }
      ...
      this.updateLanguage();
      this.getMenuElement(1).setCurrent(10);
   ...
   public final void actionElement(int iID) {
      if (iID == this.getMenuElementsSize() - 1) {
         DiplomacyManager.sendMilitaryAccess_GiveProposal(this.iOnCivID, CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(), this.getMenuElement(1).getCurrent());
