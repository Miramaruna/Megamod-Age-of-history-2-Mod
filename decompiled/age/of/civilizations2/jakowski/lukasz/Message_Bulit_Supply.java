...
public class Message_Bulit_Supply extends Message {
   public Message_Bulit_Supply(int fromCivID, int iValue) {
      super(fromCivID, iValue);
      this.messageType = Message_Type.BUILT_SUPPLY;
      ...
      if (CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID() == iCivID) {
         ...
         lMess.add(
            (CFG.game.getProvince(this.iValue).getName().length() > 0 ? CFG.game.getProvince(this.iValue).getName() + ": " : "")
               + CFG.langManager.get(BuildingsManager.getSupply_Name(CFG.game.getProvince(this.iValue).getLevelOfSupply()))
         );
   ...
   public MenuElement_Hover_v2 getHover() {
      ...
      nData.add(
         new MenuElement_Hover_v2_Element_Type_Text(
            CFG.langManager.get(BuildingsManager.getSupply_Name(CFG.game.getProvince(this.iValue).getLevelOfSupply())), CFG.COLOR_TEXT_NUM_OF_PROVINCES
         )
      );
      ...
      nData.add(
         new MenuElement_Hover_v2_Element_Type_Text(
            "-" + (int)(BuildingsManager.getSupply_Bonus(CFG.game.getProvince(this.iValue).getLevelOfSupply()) * 100.0F) + "%",
