...
public class Message_WarReparationsRepaid_Green extends Message {
   public Message_WarReparationsRepaid_Green(int fromCivID) {
      super(fromCivID, 0);
      this.messageType = Message_Type.WAR_REPARATIONS_REPAID;
   ...
   public void onAccept(int iCivID) {
      if (CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID() == iCivID) {
         ...
         lMess.add(CFG.langManager.get("WarReparationsRepaid") + ": " + CFG.game.getCiv(this.iFromCivID).getCivName());
   ...
   public MenuElement_Hover_v2 getHover() {
      ...
      nData.add(new MenuElement_Hover_v2_Element_Type_Text(CFG.langManager.get("WarReparationsRepaid") + ": ", CFG.COLOR_TEXT_NUM_OF_PROVINCES));
