...
public class Editor_Wasteland extends Editor {
   Editor_Wasteland() {
   }
   ...
   public static final void actionSave() {
      ...
      fileSave.writeString("" + (CFG.game.getProvince(0).getWasteland() >= 0 ? 49 : 48) + ";", false);

      for (int i = 1; i < CFG.game.getProvincesSize(); i++) {
         if (!CFG.game.getProvince(i).getSeaProvince()) {
            fileSave.writeString("" + (CFG.game.getProvince(i).getWasteland() >= 0 ? 49 : 48) + ";", true);
         }
      }
   }
