...
public class Menu_InGame_RepayLoan extends SliderMenu {
   public int iOnCivID = -1;
   public int iLoanValue = 0;
   public int iLoanID;
   ...
   public Menu_InGame_RepayLoan() {
      ...
      this.initMenu(
         new SliderMenuTitle(CFG.langManager.get("Loan"), CFG.BUTTON_HEIGHT * 3 / 5, true, true),
   ...
   public Menu_InGame_RepayLoan(int onCivID, int iLoanID) {
      ...
      int tempWidth = CFG.CIV_INFO_MENU_WIDTH * 2;
      int tY = 0;
      if (iLoanID >= 0 && iLoanID < CFG.game.getCiv(onCivID).getLoansSize()) {
         this.iLoanValue = CFG.game.getCiv(onCivID).getLoan(iLoanID).iGoldPerTurn * CFG.game.getCiv(onCivID).getLoan(iLoanID).iTurnsLeft;
         menuElements.add(
            new Buton_Diplomacy_Loan(
               0,
               onCivID,
               CFG.game.getCiv(onCivID).getLoan(iLoanID).iGoldPerTurn,
               CFG.game.getCiv(onCivID).getLoan(iLoanID).iGoldPerTurn * CFG.game.getCiv(onCivID).getLoan(iLoanID).iTurnsLeft,
               CFG.game.getCiv(onCivID).getLoan(iLoanID).iTurnsLeft,
               2,
               tY,
               CFG.BUTTON_WIDTH * 2
            ) {
         ...
         menuElements.add(new Text_Scale(CFG.langManager.get("-----"), -1, 2, tY, tempWidth - 4, CFG.BUTTON_HEIGHT * 3 / 4, 0.75F) {
            @Override
            public int getWidth() {
               return Menu_InGame_RepayLoan.this.getElementW() * 2;
            }
      ...
      menuElements.add(new Button_FlagActionSliderStyle(CFG.langManager.get("Cancel"), -1, 2 + CFG.PADDING, var8 = tY + CFG.PADDING, CFG.BUTTON_WIDTH, true) {
         @Override
         public int getWidth() {
            return Menu_InGame_RepayLoan.this.getElementW() - CFG.PADDING - CFG.PADDING / 2;
         }
      });
      menuElements.add(
         new Button_FlagActionSliderStyle(CFG.langManager.get("Repay"), -1, 2, var8, CFG.BUTTON_WIDTH, true) {
            @Override
            public int getPosX() {
               return Menu_InGame_RepayLoan.this.getElementW() + CFG.PADDING / 2;
            }
            ...
            public void buildElementHover() {
               ...
               nData.add(new MenuElement_Hover_v2_Element_Type_Flag(Menu_InGame_RepayLoan.this.iOnCivID, 0, CFG.PADDING));
               nData.add(new MenuElement_Hover_v2_Element_Type_Text(CFG.langManager.get("Repay") + ": ", CFG.COLOR_BUTTON_GAME_TEXT_ACTIVE));
               nData.add(new MenuElement_Hover_v2_Element_Type_Text("" + Menu_InGame_RepayLoan.this.iLoanValue, CFG.COLOR_INGAME_GOLD));
            ...
            public void drawText(SpriteBatch oSB, int iTranslateX, int iTranslateY, boolean isActive) {
               ImageManager.getImage(Images.diplo_loan2)
                  .draw(
                     oSB,
                     this.getPosX()
                        + this.getWidth() / 2
                        - (int)((this.getTextWidth() * 0.8F + ImageManager.getImage(Images.diplo_loan2).getWidth() + CFG.PADDING) / 2.0F)
                        + iTranslateX,
                     this.getPosY() + this.getHeight() / 2 - ImageManager.getImage(Images.diplo_loan2).getHeight() / 2 + iTranslateY
                  );
                  ...
                  this.getPosX()
                     + (
                        this.getTextPos() < 0
                           ? this.getWidth() / 2
                              - (int)((this.getTextWidth() * 0.8F + ImageManager.getImage(Images.diplo_loan2).getWidth() + CFG.PADDING) / 2.0F)
                              + ImageManager.getImage(Images.diplo_loan2).getWidth()
                              + CFG.PADDING
                           : this.getTextPos()
                     )
                     + iTranslateX,
            ...
            public boolean getClickable() {
               return CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getMoney() >= Menu_InGame_RepayLoan.this.iLoanValue;
            }
            ...
            public void draw(SpriteBatch oSB, int iTranslateX, int nPosX, int nPosY, int nWidth, boolean sliderMenuIsActive) {
               ...
               ImageManager.getImage(Images.diplo_loan2)
                  .draw(
                     oSB,
                     nPosX
                        + (int)(nWidth - (this.getTextWidth() * 0.8F + ImageManager.getImage(Images.diplo_loan2).getWidth() + CFG.PADDING)) / 2
                        + iTranslateX,
                     Menu_InGame_RepayLoan.this.getPosY() - this.getHeight() / 2 - ImageManager.getImage(Images.diplo_loan2).getHeight() / 2
                  );
               CFG.fontMain.getData().setScale(0.8F);
               CFG.drawText(
                  oSB,
   ...
   public final void actionElement(int iID) {
      if (iID == this.getMenuElementsSize() - 1) {
         DiplomacyManager.repayLoan(this.iOnCivID, this.iLoanID);
