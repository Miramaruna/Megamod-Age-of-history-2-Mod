...
public class Message_Bulit_Workshop extends Message {
   public Message_Bulit_Workshop(int fromCivID, int iValue) {
      super(fromCivID, iValue);
      this.messageType = Message_Type.BUILT_WORKSHOP;
      ...
      if (CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID() == iCivID) {
         ...
         lMess.add(
            (CFG.game.getProvince(this.iValue).getName().length() > 0 ? CFG.game.getProvince(this.iValue).getName() + ": " : "")
               + CFG.langManager.get(BuildingsManager.getWorkshop_Name(CFG.game.getProvince(this.iValue).getLevelOfWorkshop()))
         );
   ...
   public MenuElement_Hover_v2 getHover() {
      ...
      nData.add(
         new MenuElement_Hover_v2_Element_Type_Text(
            CFG.langManager.get(BuildingsManager.getWorkshop_Name(CFG.game.getProvince(this.iValue).getLevelOfWorkshop())), CFG.COLOR_TEXT_NUM_OF_PROVINCES
         )
      );
      ...
      nData.add(
         new MenuElement_Hover_v2_Element_Type_Text(
            "+" + (int)(BuildingsManager.getWorkshop_IncomeProduction(CFG.game.getProvince(this.iValue).getLevelOfWorkshop()) * 100.0F) + "%",
