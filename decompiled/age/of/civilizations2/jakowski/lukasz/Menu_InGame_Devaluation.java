...
class Menu_InGame_Devaluation
extends SliderMenu {
    private int iOnCivID = -1;

    public Menu_InGame_Devaluation() {
        ...
        this.initMenu(new SliderMenuTitle(CFG.langManager.get("SelectForcePlan"), CFG.BUTTON_HEIGHT * 3 / 5, true, true), CFG.GAME_WIDTH / 2 - tempWidth / 2, tempMenuPosY, tempWidth, ((MenuElement)menuElements.get(menuElements.size() - 1)).getPosY() + ((MenuElement)menuElements.get(menuElements.size() - 1)).getHeight() + CFG.PADDING + tempMenuPosY > CFG.GAME_HEIGHT - CFG.BUTTON_HEIGHT - CFG.PADDING * 2 ? Math.max(CFG.GAME_HEIGHT - CFG.BUTTON_HEIGHT - CFG.PADDING * 2 - tempMenuPosY, (CFG.TEXT_HEIGHT + CFG.PADDING * 2) * 6) : ((MenuElement)menuElements.get(menuElements.size() - 1)).getPosY() + ((MenuElement)menuElements.get(menuElements.size() - 1)).getHeight() + CFG.PADDING, menuElements, false, true);
        this.updateLanguage();
    }

    public Menu_InGame_Devaluation(int onCivID) {
        ArrayList<MenuElement> menuElements = new ArrayList<MenuElement>();
        this.iOnCivID = onCivID;
        int tempWidth = CFG.CIV_INFO_MENU_WIDTH * 2;
        int tY = 0;
        menuElements.add(new Button_NS_Opinion(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(), CFG.getActiveCivInfo(), Images.diplo_trade, 0, 16, 2, tY, CFG.BUTTON_WIDTH * 2){

            @Override
            public int getWidth() {
                return Menu_InGame_Devaluation.this.getElementW() * 2;
            }
        });
        tY += ((MenuElement)menuElements.get(menuElements.size() - 1)).getHeight();
        menuElements.add(new Button_FlagActionSliderStyle(CFG.langManager.get("AutoAssimilation"), -1, 2 + CFG.PADDING, tY += CFG.PADDING, CFG.BUTTON_WIDTH, true){

            @Override
            public int getWidth() {
                return Menu_InGame_Devaluation.this.getElementW() - CFG.PADDING - CFG.PADDING / 2;
            }
        ...
        menuElements.add(new Button_FlagActionSliderStyle(CFG.langManager.get("AutoPlan"), -1, 2, tY, CFG.BUTTON_WIDTH, true){

            @Override
            public int getPosX() {
                return Menu_InGame_Devaluation.this.getElementW() + CFG.PADDING / 2;
            }
        ...
        tY += ((MenuElement)menuElements.get(menuElements.size() - 1)).getHeight();
        menuElements.add(new Button_FlagActionSliderStyle(CFG.langManager.get("AutoDistrubitation"), -1, 2 + CFG.PADDING, tY += CFG.PADDING, CFG.BUTTON_WIDTH, true){
        ...
        menuElements.add(new Button_FlagActionSliderStyle(CFG.langManager.get("AutoMobilization"), -1, 2, tY, CFG.BUTTON_WIDTH, true){
        ...
        int tempMenuPosY = ImageManager.getImage(Images.top_left2).getHeight() + CFG.PADDING * 2 + CFG.BUTTON_HEIGHT * 3 / 5;
        this.initMenu(new SliderMenuTitle(CFG.langManager.get("SelectAutomaticProcess"), CFG.BUTTON_HEIGHT * 3 / 5, true, true){
    ...
    public final void actionElement(int iID) {
        if (iID == 1) {
            CFG.menuManager.rebuildInGame_AutoAssimilation(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID());
            return;
        }
        if (iID == 2) {
            CFG.menuManager.rebuildInGame_AutoPlan(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID());
            return;
        }
        if (iID == 3) {
            CFG.menuManager.rebuildInGame_Mobilization(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID());
            this.setVisible(false);
        }
        if (iID == 4) {
            CFG.menuManager.rebuildInGame_Rec(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID());
            this.setVisible(false);
        }
    }
