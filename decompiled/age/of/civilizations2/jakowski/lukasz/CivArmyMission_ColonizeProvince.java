...
public class CivArmyMission_ColonizeProvince extends CivArmyMission {
   public int iCivID;
   ...
   public final boolean generateColonizeData() {
      ...
      if (CFG.game.getCiv(this.iCivID).iBudget < 0) {
         ...
         ArrayList<Boolean> haveAccessToBasins = new ArrayList<>();

         for (int i3 = 0; i3 < CFG.map.iNumOfBasins; i3++) {
            haveAccessToBasins.add(false);
         }

         ArrayList<Integer> lPossibleProvincesFrom = new ArrayList<>();
         ArrayList<Integer> lPossibleProvincesTo = new ArrayList<>();

         for (int i2 = 0; i2 < CFG.game.getProvince(this.iColonizeProvinceID).getNeighboringSeaProvincesSize(); i2++) {
            haveAccessToBasins.set(CFG.game.getProvince(CFG.game.getProvince(this.iColonizeProvinceID).getNeighboringSeaProvinces(i2)).getBasinID(), true);
            lPossibleProvincesTo.add(CFG.game.getProvince(this.iColonizeProvinceID).getNeighboringSeaProvinces(i2));
         }

         for (int var20 = 0; var20 < CFG.game.getCiv(this.iCivID).getNumOfProvinces(); var20++) {
            if (!CFG.game.getProvince(CFG.game.getCiv(this.iCivID).getProvinceID(var20)).isOccupied()
               && CFG.game.getProvince(CFG.game.getCiv(this.iCivID).getProvinceID(var20)).getLevelOfPort() > 0) {
               for (int j2 = 0; j2 < CFG.game.getProvince(CFG.game.getCiv(this.iCivID).getProvinceID(var20)).getNeighboringSeaProvincesSize(); j2++) {
                  if (haveAccessToBasins.get(
                     CFG.game.getProvince(CFG.game.getProvince(CFG.game.getCiv(this.iCivID).getProvinceID(var20)).getNeighboringSeaProvinces(j2)).getBasinID()
                  )) {
                     lPossibleProvincesFrom.add(CFG.game.getCiv(this.iCivID).getProvinceID(var20));
                     break;
                  }
               }
            }
         ...
         if (lPossibleProvincesFrom.size() > 0) {
            ...
            if (tBestID >= 0) {
               this.iArmy = CFG.game.getProvince(lPossibleProvincesFrom.get(tBestID)).getArmyCivID(this.iCivID) > 0
                  ? Math.min(CFG.game.getProvince(lPossibleProvincesFrom.get(tBestID)).getArmyCivID(this.iCivID), 1 + CFG.oR.nextInt(9))
                  : Math.max(2, 1 + CFG.oR.nextInt(9));
               RegroupArmy_Data tryRegroupArmy2 = new RegroupArmy_Data(this.iCivID, lPossibleProvincesFrom.get(tBestID), lPossibleProvincesTo.get(tBestID_To));
               if (tryRegroupArmy2.getRouteSize() > 0) {
                  ...
                  this.iObsolate = Math.max(this.iObsolate, (int)(tryRegroupArmy2.getRouteSize() * 1.25F));
                  this.lockTreasury();
                  return true;
               }
            }
         ...
         for (int var21 = 0; var21 < CFG.game.getCiv(this.iCivID).getNumOfProvinces(); var21++) {
            if (!CFG.game.getProvince(CFG.game.getCiv(this.iCivID).getProvinceID(var21)).isOccupied()) {
               for (int j2x = 0; j2x < CFG.game.getProvince(CFG.game.getCiv(this.iCivID).getProvinceID(var21)).getNeighboringSeaProvincesSize(); j2x++) {
                  if (haveAccessToBasins.get(
                     CFG.game.getProvince(CFG.game.getProvince(CFG.game.getCiv(this.iCivID).getProvinceID(var21)).getNeighboringSeaProvinces(j2x)).getBasinID()
                  )) {
                     lPossibleProvincesFrom.add(CFG.game.getCiv(this.iCivID).getProvinceID(var21));
                     break;
                  }
               }
            }
         ...
         if (lPossibleProvincesFrom.size() > 0) {
            ...
            for (int i = lPossibleProvincesFrom.size() - 1; i >= 0; i--) {
               for (int jx = lPossibleProvincesTo.size() - 1; jx >= 0; jx--) {
                  RegroupArmy_Data tryRegroupArmy = new RegroupArmy_Data_PortToBuild(this.iCivID, lPossibleProvincesFrom.get(i), lPossibleProvincesTo.get(jx));
                  if (tryRegroupArmy.getRouteSize() > 0) {
                     if (tBestID < 0) {
                        tBestID = i;
                        tBestID_To = jx;
            ...
            if (tBestID >= 0) {
               this.iArmy = CFG.game.getProvince(lPossibleProvincesFrom.get(tBestID)).getArmyCivID(this.iCivID) > 0
                  ? Math.min(CFG.game.getProvince(lPossibleProvincesFrom.get(tBestID)).getArmyCivID(this.iCivID), 1 + CFG.oR.nextInt(9))
                  : Math.max(2, 1 + CFG.oR.nextInt(9));
               RegroupArmy_Data_PortToBuild tryRegroupArmy3 = new RegroupArmy_Data_PortToBuild(
                  this.iCivID, lPossibleProvincesFrom.get(tBestID), lPossibleProvincesTo.get(tBestID_To)
               );
               if (tryRegroupArmy3.getRouteSize() > 0) {
                  this.iProvinceID = lPossibleProvincesFrom.get(tBestID);
                  this.toProvinceID = lPossibleProvincesTo.get(tBestID_To);
                  this.iObsolate = Math.max(this.iObsolate, (int)(tryRegroupArmy3.getRouteSize() * 1.25F));
                  this.lockTreasury();
                  return true;
               }
            }
   ...
   public boolean action(int nCivID) {
      if (CFG.game.getProvince(this.iColonizeProvinceID).getCivID() != 0) {
         CFG.game
            .setCivRelation_OfCivB(
               ...
               CFG.game.getCivRelation_OfCivB(this.iCivID, CFG.game.getProvince(this.iColonizeProvinceID).getCivID()) - 0.25F - CFG.oR.nextInt(425) / 100.0F
            );
         ArrayList<Integer> tProv = new ArrayList<>();
         ...
         for (int ix = tProv.size() - 1; ix > 0; ix--) {
            if (CFG.game.getProvince(tProv.get(tBest)).getGrowthRate_Population() < CFG.game.getProvince(tProv.get(ix)).getGrowthRate_Population()) {
               tBest = ix;
            }
         }
      ...
      if (this.iProvinceID != this.toProvinceID) {
         RegroupArmy_Data_PortToBuild tryRegroupArmy;
         if (this.action_RecruitArmy() && (tryRegroupArmy = new RegroupArmy_Data_PortToBuild(nCivID, this.iProvinceID, this.toProvinceID)).getRouteSize() > 0) {
