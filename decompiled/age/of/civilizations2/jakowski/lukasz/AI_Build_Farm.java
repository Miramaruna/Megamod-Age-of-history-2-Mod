...
public class AI_Build_Farm extends AI_Build {
   ...
   public AI_Build_Farm(int nCivID, long nMoney) {
      ...
      try {
         for (int i = 0; i < BuildingsManager.getFarm_MaxLevel(); i++) {
            this.lBuildCost.add(BuildingsManager.getFarm_BuildCost(i + 1, CFG.game.getCiv(nCivID).getProvinceID(0)));
         ...
         if (nMoney >= this.lBuildCost.get(0).intValue()) {
            for (int var8 = 0; var8 < CFG.game.getCiv(nCivID).getNumOfProvinces(); var8++) {
               if (!CFG.game.getProvince(CFG.game.getCiv(nCivID).getProvinceID(var8)).isOccupied()
                  ...
                  && BuildingsManager.canBuildFarm(CFG.game.getCiv(nCivID).getProvinceID(var8))
                  && CFG.game.getCiv(nCivID).isInConstruction(CFG.game.getCiv(nCivID).getProvinceID(var8), ConstructionType.FARM) == 0) {
                  try {
                     if (nMoney >= this.lBuildCost.get(CFG.game.getProvince(CFG.game.getCiv(nCivID).getProvinceID(var8)).getLevelOfFarm()).intValue()) {
                        this.lProvincesToBuild
                           .get(CFG.game.getProvince(CFG.game.getCiv(nCivID).getProvinceID(var8)).getLevelOfFarm())
   ...
   public int getNumOfAlreadyBuilt(int nCivID) {
      return CFG.game.getCiv(nCivID).iNumOf_Farms;
   }
   ...
   public boolean build(int nCivID, int iteration, boolean out) {
      ...
      if (iBestProvinceID >= 0 && BuildingsManager.constructFarm(iBestProvinceID, nCivID)) {
         ...
         if (this.getMoney(nCivID) > this.lBuildCost.get(0).intValue()
            && BuildingsManager.getFarm_BuildMovementCost(1) <= CFG.game.getCiv(nCivID).getMovePoints()) {
            int tSize = 0;
