...
class Sweep {
   ...
   static boolean EdgeLeq(GLUtessellatorImpl tess, ActiveRegion reg1, ActiveRegion reg2) {
      ...
      if (e1.Sym.Org == event) {
         if (e2.Sym.Org == event) {
            return Geom.VertLeq(e1.Org, e2.Org) ? Geom.EdgeSign(e2.Sym.Org, e1.Org, e2.Org) <= 0.0 : Geom.EdgeSign(e1.Sym.Org, e2.Org, e1.Org) >= 0.0;
         } else {
   ...
   static void GetIntersectData(GLUtessellatorImpl tess, GLUvertex isect, GLUvertex orgUp, GLUvertex dstUp, GLUvertex orgLo, GLUvertex dstLo) {
      ...
      data[2] = orgLo.data;
      data[3] = dstLo.data;
      isect.coords[0] = isect.coords[1] = isect.coords[2] = 0.0;
   ...
   static boolean CheckForLeftSplice(GLUtessellatorImpl tess, ActiveRegion regUp) {
      ...
      if (Geom.VertLeq(eUp.Sym.Org, eLo.Sym.Org)) {
         ...
         regUp.dirty = regLo.dirty = true;
         GLUhalfEdge ex = Mesh.__gl_meshSplitEdge(eLo);
         if (ex == null) {
            throw new RuntimeException();
         }
         ...
         ex.Sym.Lface.inside = regUp.inside;
      }
      ...
      if (orgUp == orgLo) {
         ...
         if (tMinUp > tMaxLo) {
            ...
            if (Geom.VertEq(isect, orgUp) || Geom.VertEq(isect, orgLo)) {
               ...
               return false;
            } else if ((Geom.VertEq(dstUp, tess.event) || !(Geom.EdgeSign(dstUp, tess.event, isect) >= 0.0))
               && (Geom.VertEq(dstLo, tess.event) || !(Geom.EdgeSign(dstLo, tess.event, isect) <= 0.0))) {
               if (Mesh.__gl_meshSplitEdge(eUp.Sym) == null) {
                  throw new RuntimeException();
               ...
               if (Mesh.__gl_meshSplitEdge(eLo.Sym) == null) {
                  ...
                  ActiveRegion var16 = TopRightRegion(regUp);
                  GLUhalfEdge e = RegionBelow(var16).eUp.Sym.Onext;
                  regUp.eUp = eLo.Sym.Lnext;
                  eLo = FinishLeftRegions(tess, regUp, null);
                  AddRightEdges(tess, var16, eLo.Onext, eUp.Sym.Onext, e, true);
                  return true;
               }
            } else {
