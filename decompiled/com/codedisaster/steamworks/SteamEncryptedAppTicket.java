...
public class SteamEncryptedAppTicket extends SteamInterface {
   ...
   public boolean decryptTicket(ByteBuffer ticketEncrypted, ByteBuffer ticketDecrypted, byte[] key, int[] ticketDecryptedOutputSize) throws SteamException {
      ...
      return SteamEncryptedAppTicketNative.decryptTicket(
         ticketEncrypted,
         ticketEncrypted.position(),
         ticketEncrypted.remaining(),
         ticketDecrypted,
         ticketDecrypted.position(),
         ticketDecrypted.remaining(),
         key,
         32,
         ticketDecryptedOutputSize
      );
   }
   ...
   public int getUserVariableData(ByteBuffer ticketDecrypted, ByteBuffer userData) throws SteamException {
      ...
      return SteamEncryptedAppTicketNative.getUserVariableData(
         ticketDecrypted, ticketDecrypted.position(), ticketDecrypted.remaining(), userData, userData.position(), userData.remaining()
      );
   }
}
