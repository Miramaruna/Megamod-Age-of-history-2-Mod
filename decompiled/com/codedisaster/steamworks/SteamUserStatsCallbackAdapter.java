...
class SteamUserStatsCallbackAdapter extends SteamCallbackAdapter<SteamUserStatsCallback> {
   ...
   void onUserStatsReceived(long gameId, long steamIDUser, int result) {
      this.callback.onUserStatsReceived(gameId, new SteamID(steamIDUser), SteamResult.byValue(result));
   }

   void onUserStatsStored(long gameId, int result) {
      this.callback.onUserStatsStored(gameId, SteamResult.byValue(result));
   }

   void onUserStatsUnloaded(long steamIDUser) {
      this.callback.onUserStatsUnloaded(new SteamID(steamIDUser));
   }
   ...
   void onUserAchievementStored(long gameId, boolean isGroupAchievement, String achievementName, int curProgress, int maxProgress) {
      this.callback.onUserAchievementStored(gameId, isGroupAchievement, achievementName, curProgress, maxProgress);
   }

   void onLeaderboardFindResult(long handle, boolean found) {
      this.callback.onLeaderboardFindResult(new SteamLeaderboardHandle(handle), found);
   }

   void onLeaderboardScoresDownloaded(long handle, long entries, int numEntries) {
      this.callback.onLeaderboardScoresDownloaded(new SteamLeaderboardHandle(handle), new SteamLeaderboardEntriesHandle(entries), numEntries);
   }
   ...
   void onLeaderboardScoreUploaded(boolean success, long handle, int score, boolean changed, int globalRankNew, int globalRankPrevious) {
      this.callback.onLeaderboardScoreUploaded(success, new SteamLeaderboardHandle(handle), score, changed, globalRankNew, globalRankPrevious);
   }

   void onGlobalStatsReceived(long gameId, int result) {
      this.callback.onGlobalStatsReceived(gameId, SteamResult.byValue(result));
   }
}
