...
public class ParticleEmitter {
   ...
   public void draw(Batch batch, float delta) {
      ...
      if (this.accumulator < 1.0F) {
         ...
         if (this.delayTimer < this.delay) {
            ...
            float emissionTime = this.emission + this.emissionDiff * this.emissionValue.getScale(this.durationTimer / this.duration);
            if (emissionTime > 0.0F) {
               float var11 = 1000.0F / emissionTime;
               if (this.emissionDelta >= var11) {
                  int emitCount = (int)(this.emissionDelta / var11);
                  emitCount = Math.min(emitCount, this.maxParticleCount - activeCount);
                  this.emissionDelta = (int)(this.emissionDelta - emitCount * var11);
                  this.emissionDelta = (int)(this.emissionDelta % var11);
                  this.addParticles(emitCount);
               }
            }
   ...
   private void activateParticle(int index) {
      ...
      particle.currentLife = particle.life = this.life + (int)(this.lifeDiff * this.lifeValue.getScale(percent));
      if (this.velocityValue.active) {
      ...
      if (!this.scaleValue.isRelative()) {
         particle.scaleDiff = particle.scaleDiff - particle.scale;
      }
   ...
   private boolean updateParticle(ParticleEmitter.Particle particle, float delta, int deltaMillis) {
      ...
      if (life <= 0) {
         ...
         float percent = 1.0F - (float)particle.currentLife / particle.life;
         int updateFlags = this.updateFlags;
         if ((updateFlags & 1) != 0) {
