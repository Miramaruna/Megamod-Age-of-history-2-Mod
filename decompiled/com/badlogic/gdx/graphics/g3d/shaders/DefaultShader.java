...
public class DefaultShader extends BaseShader {
   ...
   public DefaultShader(Renderable renderable, DefaultShader.Config config, ShaderProgram shaderProgram) {
      ...
      if (!config.ignoreUnimplemented && (implementedFlags & this.attributesMask) != this.attributesMask) {
         ...
         this.u_bones = renderable.bones != null && config.numBones > 0
            ? this.register(DefaultShader.Inputs.bones, new DefaultShader.Setters.Bones(config.numBones))
            : -1;
         ...
         this.u_ambientCubemap = this.lighting
            ? this.register(DefaultShader.Inputs.ambientCube, new DefaultShader.Setters.ACubemap(config.numDirectionalLights, config.numPointLights))
            : -1;
         this.u_environmentCubemap = this.environmentCubemap
            ? this.register(DefaultShader.Inputs.environmentCubemap, DefaultShader.Setters.environmentCubemap)
            : -1;
      }
   }
   ...
   public boolean canRender(Renderable renderable) {
      ...
      return this.attributesMask == (renderableMask | optionalAttributes)
         && this.vertexMask == renderable.meshPart.mesh.getVertexAttributes().getMask()
   ...
   public void begin(Camera camera, RenderContext context) {
      ...
      if (this.has(this.u_time)) {
         this.set(this.u_time, this.time = this.time + Gdx.graphics.getDeltaTime());
      }
   }
   ...
   protected void bindLights(Renderable renderable, Attributes attributes) {
      ...
      if (this.dirLightsLoc >= 0) {
         for (int i = 0; i < this.directionalLights.length; i++) {
            if (dirs != null && i < dirs.size) {
               ...
               if (this.lightsSet
                  && this.directionalLights[i].color.r == 0.0F
                  && this.directionalLights[i].color.g == 0.0F
                  && this.directionalLights[i].color.b == 0.0F) {
                  continue;
               }
            ...
            int idx = this.dirLightsLoc + i * this.dirLightsSize;
            this.program
               .setUniformf(
                  idx + this.dirLightsColorOffset, this.directionalLights[i].color.r, this.directionalLights[i].color.g, this.directionalLights[i].color.b
               );
            this.program
               .setUniformf(
      ...
      if (this.pointLightsLoc >= 0) {
         for (int i = 0; i < this.pointLights.length; i++) {
            ...
            int idx = this.pointLightsLoc + i * this.pointLightsSize;
            this.program
               .setUniformf(
                  idx + this.pointLightsColorOffset,
            ...
            this.program
               .setUniformf(
                  idx + this.pointLightsPositionOffset, this.pointLights[i].position.x, this.pointLights[i].position.y, this.pointLights[i].position.z
               );
      ...
      if (this.spotLightsLoc >= 0) {
         for (int i = 0; i < this.spotLights.length; i++) {
            ...
            int idxx = this.spotLightsLoc + i * this.spotLightsSize;
            this.program
               .setUniformf(
                  idxx + this.spotLightsColorOffset,
            ...
            this.program.setUniformf(idxx + this.spotLightsPositionOffset, this.spotLights[i].position);
            this.program.setUniformf(idxx + this.spotLightsDirectionOffset, this.spotLights[i].direction);
            this.program.setUniformf(idxx + this.spotLightsCutoffAngleOffset, this.spotLights[i].cutoffAngle);
            this.program.setUniformf(idxx + this.spotLightsExponentOffset, this.spotLights[i].exponent);
            if (this.spotLightsIntensityOffset >= 0) {
               this.program.setUniformf(idxx + this.spotLightsIntensityOffset, this.spotLights[i].intensity);
            }
   ...
   public static class Setters {
      ...
      public static final BaseShader.Setter environmentCubemap = new BaseShader.LocalSetter() {
         ...
         public void set(BaseShader shader, int inputID, Renderable renderable, Attributes combinedAttributes) {
            if (combinedAttributes.has(CubemapAttribute.EnvironmentMap)) {
               shader.set(
                  inputID, shader.context.textureBinder.bind(((CubemapAttribute)combinedAttributes.get(CubemapAttribute.EnvironmentMap)).textureDescription)
               );
            }
         }
      };
      ...
      public static class ACubemap extends BaseShader.LocalSetter {
         private static final float[] ones = new float[]{
         ...
         public void set(BaseShader shader, int inputID, Renderable renderable, Attributes combinedAttributes) {
            if (renderable.environment == null) {
               ...
               if (combinedAttributes.has(PointLightsAttribute.Type)) {
                  ...
                  for (int i = this.pointLightsOffset; i < lights.size; i++) {
                     this.cacheAmbientCubemap.add(lights.get(i).color, lights.get(i).position, tmpV1, lights.get(i).intensity);
                  }
               }
      ...
      public static class Bones extends BaseShader.LocalSetter {
         ...
         public void set(BaseShader shader, int inputID, Renderable renderable, Attributes combinedAttributes) {
            for (int i = 0; i < this.bones.length; i++) {
               int idx = i / 16;
               this.bones[i] = renderable.bones != null && idx < renderable.bones.length && renderable.bones[idx] != null
